...
- `python -m randomizer.scripts.sprite_pixel_shift <sprite_id> <dx> <dy>` - Shifts all pixel data in a gridplane sprite's molds by the specified offset. Outputs new subtile_bytes that can be copied into the sprite file. This is useful for when you have converted a small tilemap sprite into a gridplane but need to fix its centering within the grid. Use `--visualize <sprite_id> <mold_index> [dx] [dy]` to preview a specific mold before/after shifting.
