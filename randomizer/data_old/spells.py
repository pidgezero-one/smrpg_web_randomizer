...
class CloneSpell(CharacterSpell):
    ...
    def __init__(self, world, title, spell):
        ...
        self.check_stats = spell.check_stats
        self.ignore_defense = spell.ignore_defense
        self.check_ohko = spell.check_ohko
        self.usable_outside_of_battle = spell.usable_outside_of_battle
        self.spell_type = spell.spell_type
        self.effect_type = spell.effect_type
        self.quad9s = spell.quad9s
        self.target_others = spell.target_others
        self.target_enemies = spell.target_enemies
        self.target_party = spell.target_party
        self.target_wounded = spell.target_wounded
        self.target_one_party = spell.target_one_party
        self.target_not_self = spell.target_not_self
        self.element = spell.element
        self.status_effects = spell.status_effects
        self.boosts = spell.boosts
        self.inflict = spell.inflict
        self.hide_num = spell.hide_num
...
class Thunderbolt(CharacterSpell):
    ...
    def get_patch(self):
        ...
        if self.element == Element.ICE:
            patch.add_data(
                0x33CB1F, bytearray([0x7E, 0x40, 0xC0, 0x51, 0x20, 0x67, 0xF3, 0x7F])
            )
        elif self.element == Element.FIRE:
            patch.add_data(
                0x33CB1F, bytearray([0x7E, 0x40, 0x11, 0x00, 0x99, 0x01, 0xFF, 0x03])
            )
        elif self.element == Element.JUMP:
            patch.add_data(
                0x33CB1F, bytearray([0x7E, 0x40, 0x00, 0x06, 0x20, 0x13, 0xF8, 0x03])
            )
...
class Snowy(CharacterSpell):
    ...
    def get_patch(self):
        ...
        if self.element == Element.THUNDER:
            patch.add_data(
                0x33C141, bytearray([0x85, 0x02, 0xFF, 0x7F, 0xF8, 0x7F, 0xED, 0x7F])
            )
            patch.add_data(0x33C400, bytearray([00, 0x00, 0xFF, 0x7F]))
        elif self.element == Element.FIRE:
            patch.add_data(
                0x33C141, bytearray([0x85, 0x02, 0xDB, 0x02, 0x97, 0x01, 0x13, 0x00])
            )
            patch.add_data(0x33C400, bytearray([00, 0x00, 0xDB, 0x02]))
        elif self.element == Element.JUMP:
            patch.add_data(
                0x33C141, bytearray([0x85, 0x02, 0x40, 0x03, 0xA0, 0x02, 0x00, 0x02])
            )
