...
def sanitize_animation_script(boss, boss_sprite_location, script, model):
    # leave script alone if character is vanilla
    if not is_vanilla(boss, boss_sprite_location):
        new_script = []
            ...
            if subscript_command["command"] == 'set_sprite_sequence':
                ...
                if _0x08Flags.READ_AS_MOLD in subscript_command["args"][2]:
                    ...
                    # otherwise, it's subject to animation-specific rules
                        ...
                        if utils.isclass_or_instance(boss_sprite_location, data.bosses.Booster):
                ...
                # sequences
                    ...
                    if utils.isclass_or_instance(boss_sprite_location, data.bosses.Croco1) and model.animations is not None and model.animations.bandits_way_distracted is not None:
                    ...
                    # moleville mines punch
                    elif utils.isclass_or_instance(boss_sprite_location, data.bosses.Punchinello):
                        if model.animations is not None and model.animations.mines_punch is not None:
                            if subscript_command["args"][0] == 3:
                                ...
                                new_script.append(subscript_command)
                    # chapel laughing
                    elif utils.isclass_or_instance(boss_sprite_location, data.bosses.Booster):
                        if model.animations is not None and model.animations.chapel_laugh is not None:
                            if subscript_command["args"][0] == 2:
                                ...
                                new_script.append(subscript_command)
                    # marrymore kitchen
                    elif utils.isclass_or_instance(boss_sprite_location, data.bosses.Bundt):
                        if model.animations is not None and model.animations.kitchen_prep is not None:
                            if subscript_command["args"][0] == 3:
                                ...
                                new_script.append(subscript_command)
                    # ship beckon
                    elif utils.isclass_or_instance(boss_sprite_location, data.bosses.KingCalamari):
                        if model.animations is not None and model.animations.ship_beckon is not None:
                            if subscript_command["args"][0] == 1:
                                ...
                                new_script.append(subscript_command)
                    # ship chair
                    elif utils.isclass_or_instance(boss_sprite_location, data.bosses.Johnny):
                        if model.animations is not None and model.animations.ship_chair is not None:
                            if subscript_command["args"][0] == 10:
                                ...
                                new_script.append(subscript_command)
                    # jagger
                    elif utils.isclass_or_instance(boss_sprite_location, data.bosses.Jagger):
                        if model.animations is not None and model.animations.dojo_challenge is not None:
                            if subscript_command["args"][0] == 4:
                                ...
                                new_script.append(subscript_command)
                    # jinx
                    elif utils.isclass_or_instance(boss_sprite_location, data.bosses.Jinx1) or utils.isclass_or_instance(boss_sprite_location, data.bosses.Jinx2) or utils.isclass_or_instance(boss_sprite_location, data.bosses.Jinx3):
                        if model.animations is not None and model.animations.dojo_challenge is not None:
                            if subscript_command["args"][0] == 3:
                                subscript_command["args"][0] = model.animations.dojo_challenge.sequence_id
                                new_script.append(subscript_command)
                    # magikoopa - challenge only. sequence #10 also used in battle doors, which will be handled separately
                    elif utils.isclass_or_instance(boss_sprite_location, data.bosses.Magikoopa):
...
class GameWorld:
    ...
    def build_patch(self):
        ...
        for boss_location in self.boss_locations:
            ...
            if not is_vanilla(boss, boss_location):
                for boss_sprite_location in boss_location.boss_locations:
                    ...
                    for index, obj in enumerate(self.rooms[boss_sprite_location.room_id]["objects"]):
                    ...
                    for index, obj in enumerate(flattened_object_array):
                        if index == boss_sprite_location.npc_id:
                            ...
                            elif boss_sprite_location.preferred_size == SpriteSize.Large:
                                if occupant.big_model is not None:
                                    preferred_size = SpriteSize.Large
                                else:
                                    preferred_size = SpriteSize.Small
                            elif boss_sprite_location.preferred_size == SpriteSize.Statue:
                            ...
                            if "original_index" in obj:
                                ...
                                if preferred_size == SpriteSize.Small:
                                    self.rooms[boss_sprite_location.room_id]["objects"][obj["original_index"]]["model"] = model.model_id
                                    ...
                                    model_num = self.rooms[boss_sprite_location.room_id]["objects"][obj["original_index"]]["model"]
                                ...
                                current_direction = self.rooms[boss_sprite_location.room_id]["objects"][obj["original_index"]]["direction"]

                            else:
                                current_direction = self.rooms[boss_sprite_location.room_id]["objects"][obj["parent_index"]]["clones"][obj["clone_index"]]["direction"]
                            ...
                            if "original_index" in obj:
                                self.rooms[boss_sprite_location.room_id]["objects"][obj["original_index"]]["direction"] = new_direction
                            else:
                                self.rooms[boss_sprite_location.room_id]["objects"][obj["parent_index"]]["clones"][obj["clone_index"]]["direction"] = new_direction

                            # statues: flip directions where necessary
                            if boss_sprite_location.preferred_size == SpriteSize.Statue:

                                if "original_index" in obj:
                                    self.rooms[boss_sprite_location.room_id]["objects"][obj["original_index"]]["set_sequence_playback"] = False
                                    model_num = self.rooms[boss_sprite_location.room_id]["objects"][obj["original_index"]]["model"]
                                else:
                                    self.rooms[boss_sprite_location.room_id]["objects"][obj["parent_index"]]["clones"][obj["clone_index"]]["set_sequence_playback"] = False
                                    model_num = self.rooms[boss_sprite_location.room_id]["objects"][obj["parent_index"]]["model"] + self.rooms[boss_sprite_location.room_id]["objects"][obj["parent_index"]]["clones"][obj["clone_index"]]["npc_id_offset"]
                                ...
                                cmd = new_animation(boss_sprite_location.sequence_setter, 'action_queue_async', boss_sprite_location.npc_id, [{"identifier": "dummy", "command": "sequence_playback_off"}])
                                ...
                                if (new_direction == RadialDirection.SOUTHEAST or new_direction == RadialDirection.SOUTHWEST) and (model.horizontal_pixel_shift > 0 or model.vertical_pixel_shift > 0):
                                    ...
                                    cmd = new_animation(boss_sprite_location.sequence_setter, 'action_queue_async', boss_sprite_location.npc_id, [{"identifier": "dummy", "command": "shift_xy_pixels", "args": [horizontal_shift, vertical_shift]}])
                                    sequence_setters[boss_sprite_location.sequence_setter].append(cmd)
                            ...
                            if model.model_details is not None and model.model_details["sprite"] == SpriteName._221_YARIDOVICH_OUT_OF_BATTLE and (utils.isclass_or_instance(boss_location, data.bosses.Boomer) or utils.isclass_or_instance(boss_location, data.bosses.Smithy)):
                                ...
                                if model.sequence_type == SequenceType.Mold:
                                    cmd = new_animation(boss_sprite_location.sequence_setter, 'action_queue_async', boss_sprite_location.npc_id, [{"identifier": "dummy", "command": "set_sprite_sequence", "args": [model.mold, sprite_offset, [_0x08Flags.LOOPING_OFF, _0x08Flags.READ_AS_MOLD]]}])
                                else:
                                    cmd = new_animation(boss_sprite_location.sequence_setter, 'action_queue_async', boss_sprite_location.npc_id, [{"identifier": "dummy", "command": "set_sprite_sequence", "args": [model.sequence, sprite_offset, [_0x08Flags.READ_AS_SEQUENCE]]}])
                                sequence_setters[boss_sprite_location.sequence_setter].append(cmd)
                                ...
                                for script_id in boss_sprite_location.target_scripts:
                                    ...
                                    for command_index, command in enumerate(script):
                                        if is_animation_header(command, boss_sprite_location.npc_id):
                            ...
                            if model.animations is not None:
                                if utils.isclass_or_instance(boss_location, data.bosses.Croco1) and model.animations.bandits_way_distracted is not None and model.animations.bandits_way_distracted.new_sprite_id is not None:
                                    self.models[model.model_id]["sprite"] = model.animations.bandits_way_distracted.new_sprite_id
                                elif utils.isclass_or_instance(boss_location, data.bosses.Punchinello) and model.animations.mines_punch is not None and model.animations.mines_punch.new_sprite_id is not None:
                                    self.models[model.model_id]["sprite"] = model.animations.mines_punch.new_sprite_id
                                elif utils.isclass_or_instance(boss_location, data.bosses.Booster) and model.animations.chapel_laugh is not None and model.animations.chapel_laugh.new_sprite_id is not None:
                                    self.models[model.model_id]["sprite"] = model.animations.chapel_laugh.new_sprite_id
                                elif utils.isclass_or_instance(boss_location, data.bosses.KingCalamari) and model.animations.ship_beckon is not None and model.animations.ship_beckon.new_sprite_id is not None:
                                    self.models[model.model_id]["sprite"] = model.animations.ship_beckon.new_sprite_id
                                elif utils.isclass_or_instance(boss_location, data.bosses.Johnny) and model.animations.ship_chair is not None and model.animations.ship_chair.new_sprite_id is not None:
                                    self.models[model.model_id]["sprite"] = model.animations.ship_chair.new_sprite_id
                                elif (utils.isclass_or_instance(boss_location, data.bosses.Jinx1) or utils.isclass_or_instance(boss_location, data.bosses.Jinx2) or utils.isclass_or_instance(boss_location, data.bosses.Jinx3) or utils.isclass_or_instance(boss_location, data.bosses.Jagger)) and model.animations.dojo_challenge is not None and model.animations.dojo_challenge.new_sprite_id is not None:
                                    self.models[model.model_id]["sprite"] = model.animations.dojo_challenge.new_sprite_id
                                elif utils.isclass_or_instance(boss_location, data.bosses.Dodo) and model.animations.statue_peck is not None and model.animations.statue_peck.new_sprite_id is not None:
                                    self.models[model.model_id]["sprite"] = model.animations.statue_peck.new_sprite_id
                                elif utils.isclass_or_instance(boss_location, data.bosses.Magikoopa) and model.animations.keep_challenge is not None and model.animations.keep_challenge.new_sprite_id is not None:
                                    self.models[model.model_id]["sprite"] = model.animations.keep_challenge.new_sprite_id
                                elif utils.isclass_or_instance(boss_location, data.bosses.Magikoopa) and model.animations.keep_summon is not None and model.animations.keep_summon.new_sprite_id is not None:
                                    self.models[model.model_id]["sprite"] = model.animations.keep_summon.new_sprite_id
                                elif utils.isclass_or_instance(boss_location, data.bosses.Boomer) and model.animations.chandelier_challenge is not None and model.animations.chandelier_challenge.new_sprite_id is not None:
                                    self.models[model.model_id]["sprite"] = model.animations.chandelier_challenge.new_sprite_id
                                elif utils.isclass_or_instance(boss_location, data.bosses.Smithy) and model.animations.endgame_challenge is not None and model.animations.endgame_challenge.new_sprite_id is not None:
                            ...
                            if not is_vanilla(boss, boss_sprite_location):
                                ...
                                if utils.isclass_or_instance(boss_location, data.bosses.Gunyolk):
                                ...
                                # hide composite NPCs that aren't used if shuffled
                                if utils.isclass_or_instance(boss_location, data.bosses.Smithy):
                                    self.rooms[509]["objects"] = copy.deepcopy(non_smithy_509_objects)

                                # move NPCs in megasmilax's room
                                if utils.isclass_or_instance(boss_location, data.bosses.MegaSmilax):
                            ...
                            for script_id in boss_sprite_location.target_scripts:
                                ...
                                if not is_vanilla(boss, boss_sprite_location):
                                    ...
                                    if utils.isclass_or_instance(boss_location, data.bosses.Punchinello) and script_id == 860:
                                        pause = 10
                                        for command_index, command in enumerate(script):
                                            if is_animation_header(command, boss_sprite_location.npc_id):
                                                if model.animations is not None and model.animations.mines_punch is not None:
                                    ...
                                    if utils.isclass_or_instance(boss_location, data.bosses.Magikoopa) and model.animations is not None and model.animations.keep_summon is not None and script_id == 941:
                                    ...
                                    # adjust dojo pause
                                    if utils.isclass_or_instance(boss_location, data.bosses.Jagger) and model.animations is not None and model.animations.dojo_challenge is not None and script_id == 861:
                                        for command_index, command in enumerate(script):
                                            if is_animation_header(command, boss_sprite_location.npc_id):
                                                ...
                                                for subscript_command_index, subscript_command in enumerate(command["subscript"]):
                                                    if subscript_command["command"] == 'pause' and subscript_command["args"] == 45:
                                                        ...
                                                        command["subscript"][subscript_command_index] = subscript_command
                                                self.eventscripts[script_id][command_index]["subscript"] = command["subscript"]
                                    elif utils.isclass_or_instance(boss_location, data.bosses.Jinx1) and model.animations is not None and model.animations.dojo_challenge is not None:
                                        for command_index, command in enumerate(script):
                                            if is_animation_header(command, boss_sprite_location.npc_id):
                                                for subscript_command_index, subscript_command in enumerate(command["subscript"]):
                                                    if (script_id == 862):
                                                        ...
                                                        if subscript_command["command"] == 'pause' and subscript_command["args"] == 18:
                                                            ...
                                                            command["subscript"][subscript_command_index] = subscript_command
                                                self.eventscripts[script_id][command_index]["subscript"] = command["subscript"]
                                    elif utils.isclass_or_instance(boss_location, data.bosses.Jinx2) and model.animations is not None and model.animations.dojo_challenge is not None:
                                        for command_index, command in enumerate(script):
                                            if is_animation_header(command, boss_sprite_location.npc_id):
                                                for subscript_command_index, subscript_command in enumerate(command["subscript"]):
                                                    if (script_id == 864):
                                                        ...
                                                        if subscript_command["command"] == 'pause' and subscript_command["args"] == 18:
                                                            ...
                                                            command["subscript"][subscript_command_index] = subscript_command
                                                self.eventscripts[script_id][command_index]["subscript"] = command["subscript"]
                                    elif utils.isclass_or_instance(boss_location, data.bosses.Jinx3) and model.animations is not None and model.animations.dojo_challenge is not None and script_id == 866:
                                        for command_index, command in enumerate(script):
                                            if is_animation_header(command, boss_sprite_location.npc_id):
                                                pause = max(45, model.animations.dojo_challenge.total_duration)
                                                for subscript_command_index, subscript_command in enumerate(command["subscript"]):
                                    ...
                                    elif utils.isclass_or_instance(boss_location, data.bosses.Dodo) and (script_id == 936):
                                    ...
                                    elif utils.isclass_or_instance(boss_location, data.bosses.Dodo) and (script_id == 937):
                                        if model.animations is None or model.animations.statue_peck is None:
                                    ...
                                    elif utils.isclass_or_instance(boss_location, data.bosses.Dodo) and (script_id == 939) and model.animations is not None and model.animations.statue_intro is not None:
                                    ...
                                    elif utils.isclass_or_instance(boss_location, data.bosses.Dodo) and (script_id == 940) and model.animations is not None and model.animations.statue_flustered is not None:
                                    ...
                                    elif utils.isclass_or_instance(boss_location, data.bosses.Magikoopa) and (script_id == 942):
                                    ...
                                    elif utils.isclass_or_instance(boss_location, data.bosses.Boomer) and (script_id == 943):
                                    ...
                                    # smithy needs A LOT of adjustments, to the point of complete script replacement and npc removal
                                    elif utils.isclass_or_instance(boss_location, data.bosses.Smithy) and (script_id == 3792):
                                        self.eventscripts[script_id] = copy.deepcopy(non_smithy_3792)
                                    elif utils.isclass_or_instance(boss_location, data.bosses.Smithy) and (script_id == 3794):
                                        ...
                                        for command_index, command in enumerate(script):
                                            if is_animation_header(command, boss_sprite_location.npc_id):
                                                self.eventscripts[script_id][command_index]["subscript"] = sanitize_animation_script(boss_sprite_location.occupant, boss_location, command["subscript"], model)
                            ...
                            for script_id in boss_sprite_location.target_action_scripts:
                                ...
                                if utils.isclass_or_instance(boss_location, data.bosses.Magikoopa) and script_id == 1004 and model.animations is not None and model.animations.keep_summon is not None:
                                    for subscript_command_index, subscript_command in enumerate(script):
                                ...
                                if utils.isclass_or_instance(boss_location, data.bosses.Booster) and script_id == 519:
                                    for subscript_command_index, subscript_command in enumerate(script):
                                        # set the proper animation for the sprite, and determine if it should loop or not
                                        if subscript_command["command"] in ['shift_south_pixels', 'shift_north_pixels']:
                                            subscript_command["args"][0] = 32 - boss.eye_height
                                            self.actionscripts[script_id][subscript_command_index] = subscript_command

                                # replace all sequences and molds if appropriate, remove if not
                                else:
                                    self.actionscripts[script_id] = sanitize_animation_script(boss, boss_location, script, model)
                        ...
                        for index, obj in enumerate(flattened_object_array):
                            if index == henchman_location.npc_id:
                                if occupant is None:
                                    ...
                                    # replace model sprite if necessary
                                    if model.animations is not None:
                                        if utils.isclass_or_instance(boss_location, data.bosses.Booster) and model.animations.tower_bullet is not None and model.animations.tower_bullet.new_sprite_id is not None:
                                            self.models[model.model_id]["sprite"] = model.animations.tower_bullet.new_sprite_id
                                        elif utils.isclass_or_instance(boss_location, data.bosses.Bundt) and model.animations.kitchen_prep is not None and model.animations.kitchen_prep.new_sprite_id is not None:
                                    ...
                                    if not is_vanilla(boss, boss_sprite_location):
                                        ...
                                        for script_id in henchman_location.target_scripts:
                                            ...
                                            for command_index, command in enumerate(script):
                                                if is_animation_header(command, henchman_location.npc_id):
                                                    self.eventscripts[script_id][command_index]["subscript"] = sanitize_animation_script(boss, boss_location, command["subscript"], model)
                                        ...
                                        for script_id in henchman_location.target_action_scripts:
                                            ...
                                            # adjust kitchen animation pauses
                                            if utils.isclass_or_instance(boss_location, data.bosses.Bundt) and script_id in [330,331] and model.animations is not None and model.animations.kitchen_prep is not None:
                                                for subscript_command_index, subscript_command in enumerate(script):
                                                    # set the proper animation for the sprite, and determine if it should loop or not
                                                    if subscript_command["command"] == 'set_sprite_sequence':
                                                        subscript_command["args"][0] = model.animations.kitchen_prep.sequence_id
                                            ...
                                            elif utils.isclass_or_instance(boss_location, data.bosses.Booster) and script_id == 386:
        ...
        for i, name in enumerate(file_select_names):
            addr = 0x3ef528 + (i * 7)
            val = name.encode().ljust(7, boss_location'\x00')
