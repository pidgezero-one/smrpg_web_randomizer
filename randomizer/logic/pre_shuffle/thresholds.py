...
def apply_threshold_settings(world: GameWorld) -> None:
    ...
    if world.settings.get_flag(StarPiecesRequired).value > 1:
        cast(
            JmpIfVarEqualsConst,
            world.event_scripts.get_command_by_identifier("enable_boss_access_3_backup"),
        ).set_value(world.settings.get_flag(StarPiecesRequired).value - 1)
