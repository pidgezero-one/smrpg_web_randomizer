        ...
        if fillable_locations:
            ...
            if utils.isclass_or_instance(item, items.PandoriteFight):
                ...
                chests_missing_locations = [
                    c
                    for c in world.chest_locations + world.boss_star_checks
                    if utils.isclass_or_instance(c, chests.HidonReward1)
                    or utils.isclass_or_instance(c, chests.HidonReward2)
    ...
    if world.open_mode:
        ...
        if world.settings.is_flag_value(flags.MimicsAnywhere, False):
            all_locations = [
                ...
                and not utils.isclass_or_instance(a, chests.HidonChest)
        ...
        if world.settings.is_flag_value(flags.EXPStarsAnywhere, False):
            ...
            all_locations = [
                ...
                and not utils.isclass_or_instance(a, chests.SeaStarChest)
        ...
        if world.settings.is_flag_value(flags.ShuffleWeddingGear, False):
            all_locations = [
                a
                for a in all_locations
                if not utils.isclass_or_instance(a, chests.MarrymoreSnifit1)
                and not utils.isclass_or_instance(a, chests.MarrymoreSnifit2)
                and not utils.isclass_or_instance(a, chests.MarrymoreSnifit3)
                and not utils.isclass_or_instance(a, chests.MarrymoreAltarHead)
            ]
