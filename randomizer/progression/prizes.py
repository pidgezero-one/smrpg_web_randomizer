...
from ..data.items.items import (
    CrystalShardItem, ExtraShinyStoneItem, HammerItem, FroggieStickItem, NokNokShellItem, PunchGloveItem,
    FingerShotItem, CymbalsItem, ChompItem, MasherItem, ChompShellItem, StayVoucherItem,
...
from ..data.allies.allies import MARIO_Ally, MALLOW_Ally, GENO_Ally, BOWSER_Ally, TOADSTOOL_Ally
from ..types.prize import FrogCoinPrize, EXPStarPrize, StandardPrize, CoinPrize, SlotsPrize, BossFightPrize, CharacterPrize, StarPiecePrize, ItemPrize, SpellPrize, MimicFightInitiatorPrize, TreasureHunterNickname, ProgressiveItemPrize, WeddingGearPrize, SpecialItemPrizeType
from ..data.spells.spells import JumpSpell, FireOrbSpell, SuperJumpSpell, SuperFlameSpell, UltraJumpSpell, UltraFlameSpell, ThunderboltSpell, HPRainSpell, PsychopathSpell, ShockerSpell, SnowySpell, PsychBombSpell, GenoBeamSpell, GenoBoostSpell, GenoWhirlSpell, GenoBlastSpell, GenoFlashSpell, TerrorizeSpell, PoisonGasSpell, CrusherSpell, BowserCrushSpell, TherapySpell, GroupHugSpell, MuteSpell, SleepyTimeSpell, ComeBackSpell, StarRainSpell
...
class FirstMimicFightLauncher(MimicFightInitiatorPrize):
    ...
    def chest_grant(self) -> EventScript:
        return EventScript([
            JmpToEvent(E3124_MIMIC_1_CHEST)
        ])
...
class SecondMimicFightLauncher(MimicFightInitiatorPrize):
    ...
    def chest_grant(self) -> EventScript:
        return EventScript([
            JmpToEvent(E3126_MIMIC_2_CHEST)
        ])
...
class FrogCoin1Prize(FrogCoinPrize):
    ...
    def river_grant(self) -> EventScript:
        return EventScript([
            JmpToEvent(E2816_ASYNC_NO_ANIMATION_FROG_COIN)
        ])
