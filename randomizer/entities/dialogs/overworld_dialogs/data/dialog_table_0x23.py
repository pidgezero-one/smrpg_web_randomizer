"""Dialog data bank 2"""

dialog_data: list[str] = ["""[await]"""] * 922
dialog_data[
    0
] = """ I slept like a baby.
 Thanks![await]"""
dialog_data[
    1
] = """ Super Jump?[await]
 Oh, no, not today.[delay] It's my day off.[await]"""
dialog_data[2] = """[await]"""
dialog_data[3] = """[await]"""
dialog_data[4] = """[await]"""
dialog_data[5] = """[await]"""
dialog_data[
    6
] = """ That card you have there...[await]
 How about selling it to me for...
 [delay][delay]...oh, say[delay][delay]...100 coins?[await]
  [select]  (It’s a deal!)
  [select]  (Are you crazy?!)[await]"""
dialog_data[
    7
] = """ I heard you can get some pretty
 nice thank-you gifts if you tip the
 bellhop in the suite.[await]
 I guess karma works sometimes![await]"""
dialog_data[
    8
] = """ It's mostly just elderly guests who
 stay in this hotel these days.[await]
 They've been tipping the staff quite nicely.[await][page]
 Maybe I should get a job here.
 I wonder how I can apply?[await]"""
dialog_data[
    9
] = """ If you find a “Bright Card” and
 don't want to use it, you can sell
 it to me.[await]
 You'll have to pay 15 Frog Coins if
 you want it back, though.[await]"""
dialog_data[10] = """[await]"""
dialog_data[11] = """[await]"""
dialog_data[12] = """[await]"""
dialog_data[
    13
] = """CHEF TORTE: Ve bake a cake
 for za BIG vedding!
 It must be...masterpiece![await]"""
dialog_data[
    14
] = """APPRENTICE: Yeah, the happy
 couple ordered something special.
 Wait until they see THIS![await]"""
dialog_data[
    15
] = """SNIFIT 1: It's not like the
 developers HAD to leave this
 door locked.[await]"""
dialog_data[16] = """[await]"""
dialog_data[17] = """[await]"""
dialog_data[18] = """[await]"""
dialog_data[19] = """[await]"""
dialog_data[20] = """[await]"""
dialog_data[21] = """[await]"""
dialog_data[22] = """[await]"""
dialog_data[23] = """[await]"""
dialog_data[24] = """[await]"""
dialog_data[25] = """[await]"""
dialog_data[26] = """[await]"""
dialog_data[27] = """[await]"""
dialog_data[28] = """[await]"""
dialog_data[
    29
] = """[page]
 Hya hya hya! Guess I'm one heck
 of a party animal, huh?[await][page]
 Hya hya[delay]...h[delay][delay]..[delay]ya..[await][pause][end]"""
dialog_data[30] = """[await]"""
dialog_data[31] = """[await]"""
dialog_data[32] = """[await]"""
dialog_data[33] = """[await]"""
dialog_data[34] = """[await]"""
dialog_data[35] = """[await]"""
dialog_data[36] = """[await]"""
dialog_data[37] = """[await]"""
dialog_data[38] = """[await]"""
dialog_data[39] = """[await]"""
dialog_data[40] = """[await]"""
dialog_data[41] = """[await]"""
dialog_data[42] = """[await]"""
dialog_data[43] = """[await]"""
dialog_data[44] = """[await]"""
dialog_data[45] = """[await]"""
dialog_data[46] = """[await]"""
dialog_data[47] = """             Got the Brooch![await]"""
dialog_data[48] = """              Got the Shoes![await]"""
dialog_data[49] = """               Got the Ring![await]"""
dialog_data[50] = """            Found the Crown![await]"""
dialog_data[51] = """[await]"""
dialog_data[52] = """[await]"""
dialog_data[53] = """[await]"""
dialog_data[54] = """[await]"""
dialog_data[55] = """[await]"""
dialog_data[56] = """[await]"""
dialog_data[
    57
] = """ zzz...[delay_30]
 Signal Ring...[delay_30] star pieces...[delay_30]
 zzz...[await]"""
dialog_data[58] = """[await]"""
dialog_data[
    59
] = """ The composer for this tune is a
 pretty eccentric guy. [await][pause]He'll give you
 special items if you help him
 compose a song.[await]
 Why don't you go visit him in
 Tadpole Pond sometime?[await]"""
dialog_data[
    60
] = """[page]
 Well then, how about...[delay][delay]
 5 Frog Coins! 5! That’s major![await]
 [select]  (DEAL!)
 [select]  (What else do you have?)[await]"""
dialog_data[
    61
] = """RAZ: We're getting married today,
 but the people who booked the
 chapel before us are late.[await]"""
dialog_data[62] = """RAINI: It's a lovely day outside,\n isn't it?[await]"""
dialog_data[
    63
] = """ Uh oh...
 Don't tell the hotel I snuck out
 of work for this![await]"""
dialog_data[
    64
] = """RAZ: Oh NO![delay_60]
 I forgot to sign the guest book for
 `TOWER_BOSS_1` & `MARRYMORE_CHARACTER`'s wedding![await]"""
dialog_data[
    65
] = """RAINI: I think the back door to the
 chapel was left unlocked.[await][pause] But Raz
 is afraid of getting caught
 sneaking in.[await]"""
dialog_data[
    66
] = """ `TOWER_BOSS_1` and `MARRYMORE_CHARACTER`
 getting married sounds like an
 obscure fanfic.[await]
 Even so, invites to this wedding
 are highly exclusive.[await]"""
dialog_data[
    67
] = """ Don't listen to that guy,
 `MARRYMORE_CHARACTER` x `RANDOM_BOSS_NAME_1`
 is a much better ship.[await]"""
dialog_data[
    68
] = """ We were about to begin the
 ceremony when some kids, wearing
 dark clothes, kicked us out![await]"""
dialog_data[
    69
] = """ Don't listen to that guy, he's just
 mad because he ships
 `RANDOM_CHARACTER_NAME` and `TOWER_BOSS_1`.[await]"""
dialog_data[
    70
] = """ I was hoping for this seed to solve
 the romantic tension between
 `RANDOM_BOSS_NAME_2` and `RANDOM_BOSS_NAME_3`.[await]
 Oh well, can't win 'em all.[await]"""
dialog_data[
    71
] = """ Really? `MARRYMORE_CHARACTER` and `TOWER_BOSS_1`
 getting married?[delay_30] Are these devs
 even trying anymore?[await]"""
dialog_data[
    72
] = """ Huh?[delay_30] A bathroom?[delay_30] No, I'm just
 cold![await]"""
dialog_data[
    73
] = """ The chapel kitchen has been making
 some pretty weird cakes lately.[await]
 I'm not sure if all of them are even
 really cakes.[await]"""
dialog_data[74] = """[await]"""
dialog_data[75] = """[await]"""
dialog_data[76] = """[await]"""
dialog_data[77] = """[await]"""
dialog_data[78] = """[await]"""
dialog_data[79] = """[await]"""
dialog_data[80] = """[await]"""
dialog_data[81] = """[await]"""
dialog_data[82] = """[await]"""
dialog_data[83] = """[await]"""
dialog_data[84] = """[await]"""
dialog_data[85] = """[await]"""
dialog_data[86] = """[await]"""
dialog_data[87] = """[await]"""
dialog_data[88] = """[await]"""
dialog_data[89] = """[await]"""
dialog_data[90] = """[await]"""
dialog_data[91] = """[await]"""
dialog_data[92] = """[await]"""
dialog_data[93] = """[await]"""
dialog_data[94] = """[await]"""
dialog_data[95] = """[await]"""
dialog_data[96] = """[await]"""
dialog_data[97] = """[await]"""
dialog_data[98] = """[await]"""
dialog_data[99] = """[await]"""
dialog_data[100] = """[await]"""
dialog_data[101] = """[await]"""
dialog_data[102] = """[await]"""
dialog_data[103] = """[await]"""
dialog_data[104] = """[await]"""
dialog_data[105] = """[await]"""
dialog_data[106] = """[await]"""
dialog_data[107] = """[await]"""
dialog_data[108] = """[await]"""
dialog_data[
    109
] = """ Have you been to the hotel? I heard
 the SUITE has a new gift item and
 room service menu.[await]"""
dialog_data[
    110
] = """ I heard some strange rumours about
 today's wedding cake. But I'd have
 to visit the kitchen to confirm.[await]"""
dialog_data[
    111
] = """ Welcome to Marrymore![await]
 Have you visited our hotel?[await]
 Our Suite's room service menu has
 some new and exciting items.[await]"""
dialog_data[
    112
] = """ I have no idea what on earth is
 going on in that chapel kitchen.[await]
 Everything they make turns out
 evil.[await]"""
dialog_data[113] = """[await]"""
dialog_data[114] = """[await]"""
dialog_data[
    115
] = """ I'm just glad it's not Sunday.
 I hate mowing the lawn on Sundays.[await]"""
dialog_data[
    116
] = """ OK, get a little closer, now...[await]
 Ready? All right, 1..2..[await]
 Hey, you there![await]"""
dialog_data[
    117
] = """ Why the long face?
 This isn't a funeral, you know.[await]"""
dialog_data[118] = """[await]"""
dialog_data[119] = """[await]"""
dialog_data[120] = """[await]"""
dialog_data[121] = """[await]"""
dialog_data[122] = """[await]"""
dialog_data[
    123
] = """ These massage chairs they installed
 in the chapel pews are great![await]"""
dialog_data[124] = """[await]"""
dialog_data[125] = """[await]"""
dialog_data[126] = """[await]"""
dialog_data[127] = """[await]"""
dialog_data[
    128
] = """
                   I do.[await]"""
dialog_data[
    129
] = """ I came here to watch “Maria and
 Draco”. I think I'm lost.[await]"""
dialog_data[
    130
] = """ I wasn't invited to this wedding,
 but it's not exactly like they have
 bouncers here.[await]"""
dialog_data[
    131
] = """ Reverend Dodo must have gotten
 lost on his way here.[await]"""
dialog_data[132] = """[await]"""
dialog_data[
    133
] = """ I'm a little worried about what
 the chefs might be serving for this
 wedding.[await]
 I can't Super Jump it if it turns
 out to be evil.[await]"""
dialog_data[
    134
] = """ Have you been to Booster's Tower
 since before the previous wedding
 happened?[await]
 If not, you should go check it out.
 You might find something
 important.[await]"""
dialog_data[
    135
] = """
       Hey, what are you doing?[await]"""
dialog_data[136] = """[await]"""
dialog_data[137] = """[await]"""
dialog_data[138] = """[await]"""
dialog_data[139] = """[await]"""
dialog_data[140] = """[await]"""
dialog_data[141] = """[await]"""
dialog_data[142] = """[await]"""
dialog_data[143] = """[await]"""
dialog_data[144] = """ We're finally “man and wife”.[await]"""
dialog_data[
    145
] = """ Honey, I don't want you to
 wander too far off, now![await]"""
dialog_data[
    146
] = """ Did you know that the hotel offers
 you special prizes if you stay in
 the Suite repeatedly?[await]
 I wonder what they're giving out?[await]"""
dialog_data[
    147
] = """ I stayed in the hotel for two nights
 when I only paid for one.[await][pause] And I
 didn't have enough money to cover
 the second night.[await]
 They made me work to make up the
 difference.[delay] And you know what?[await]
 The tips were great![await]"""
dialog_data[
    148
] = """
          Pipe down and relax.[await]"""
dialog_data[
    149
] = """
      I wish it were ski season![await]"""
dialog_data[
    150
] = """ I wish those two a wonderful
 life together.[await]
 Hope they don't fight too often![await]"""
dialog_data[
    151
] = """ Oh man, is it over, yet?[await]
 I hate getting my picture taken.[await]"""
dialog_data[152] = """[await]"""
dialog_data[
    153
] = """ All right,[await]
 on the count of 3, say
 “Ahhhhhhhhhhhhhhhhhhhhhhhhhh!”[await]"""
dialog_data[154] = """[await]"""
dialog_data[155] = """[await]"""
dialog_data[156] = """[await]"""
dialog_data[157] = """[await]"""
dialog_data[158] = """[await]"""
dialog_data[159] = """[await]"""
dialog_data[160] = """[await]"""
dialog_data[161] = """[await]"""
dialog_data[162] = """[await]"""
dialog_data[163] = """[await]"""
dialog_data[164] = """[await]"""
dialog_data[165] = """[await]"""
dialog_data[166] = """[await]"""
dialog_data[167] = """[await]"""
dialog_data[168] = """[await]"""
dialog_data[169] = """[await]"""
dialog_data[170] = """[await]"""
dialog_data[171] = """[await]"""
dialog_data[172] = """[await]"""
dialog_data[173] = """[await]"""
dialog_data[174] = """[await]"""
dialog_data[
    175
] = """ You'd think with our improved
 economy that we really could afford
 bazookas for the guards...[await]"""
dialog_data[176] = """[await]"""
dialog_data[177] = """[await]"""
dialog_data[178] = """[await]"""
dialog_data[179] = """[await]"""
dialog_data[180] = """[await]"""
dialog_data[
    181
] = """ Good afternoon, `MAIN_CHARACTER_NAME`![await]
 Our Vault Guard is taking the day
 off today, so feel free to help
 yourself to the treasury.[await]"""
dialog_data[
    182
] = """ Oooh! It’s good to see you back.
 Look what I found in the wallet
 you returned to me.[await][page]
 It’s yours, isn't it?[await]"""
dialog_data[183] = """[await]"""
dialog_data[184] = """[await]"""
dialog_data[185] = """[await]"""
dialog_data[
    186
] = """ Sorry, I don't have any advice
 today.[await]"""
dialog_data[187] = """[await]"""
dialog_data[188] = """[await]"""
dialog_data[189] = """[await]"""
dialog_data[190] = """[await]"""
dialog_data[191] = """[await]"""
dialog_data[192] = """[await]"""
dialog_data[
    193
] = """ If you want more Frog Coins, you
 can get some from scaling the
 Land's End cliff under 11 seconds.[await]"""
dialog_data[194] = """\n            Got a Frog Coin![await]"""
dialog_data[195] = """[await]"""
dialog_data[196] = """[await]"""
dialog_data[197] = """[await]"""
dialog_data[198] = """[await]"""
dialog_data[199] = """[await]"""
dialog_data[200] = """[await]"""
dialog_data[201] = """[await]"""
dialog_data[202] = """[await]"""
dialog_data[203] = """[await]"""
dialog_data[204] = """[await]"""
dialog_data[205] = """[await]"""
dialog_data[
    206
] = """ HA![delay] `MARRYMORE_CHARACTER` and `TOWER_BOSS_1`
 are getting married![delay] Pa' Mole owes
 me 100 coins![await]"""
dialog_data[
    207
] = """
     “Bandit's Way” is now open[await]"""
dialog_data[
    208
] = """
      “Forest Maze” is now open[await]"""
dialog_data[
    209
] = """
       “Pipe Vault” is now open[await]"""
dialog_data[
    210
] = """
     Check out “Booster Tower”![await]"""
dialog_data[
    211
] = """
   “Marrymore Chapel” is now open[await]"""
dialog_data[
    212
] = """
            “Sea” is now open[await]"""
dialog_data[
    213
] = """
      Check out “Seaside Town”![await]"""
dialog_data[
    214
] = """
      Check out “Belome Temple”![await]"""
dialog_data[
    215
] = """
     “Bowser's Keep” is now open[await]"""
dialog_data[
    216
] = """
           “Gate” is now open[await]"""
dialog_data[217] = """[await]"""
dialog_data[218] = """     “Monstro Town” is now open[await]"""
dialog_data[219] = """    “Barrel Volcano” is now open[await]"""
dialog_data[220] = """    “Moleville Mines” is now open[await]"""
dialog_data[221] = """[await]"""
dialog_data[222] = """[await]"""
dialog_data[223] = """[await]"""
dialog_data[224] = """[await]"""
dialog_data[225] = """[await]"""
dialog_data[226] = """[await]"""
dialog_data[227] = """[await]"""
dialog_data[228] = """[await]"""
dialog_data[229] = """[await]"""
dialog_data[230] = """[await]"""
dialog_data[231] = """[await]"""
dialog_data[232] = """[await]"""
dialog_data[233] = """[await]"""
dialog_data[234] = """[await]"""
dialog_data[235] = """[await]"""
dialog_data[236] = """[await]"""
dialog_data[237] = """[await]"""
dialog_data[238] = """[await]"""
dialog_data[239] = """[await]"""
dialog_data[240] = """[await]"""
dialog_data[241] = """[await]"""
dialog_data[242] = """[await]"""
dialog_data[243] = """[await]"""
dialog_data[244] = """[await]"""
dialog_data[245] = """[await]"""
dialog_data[246] = """[await]"""
dialog_data[247] = """[await]"""
dialog_data[248] = """[await]"""
dialog_data[
    249
] = """TOAD: I suppose our security
 could use some improvement.[await]
 Ah well.[delay_15] I'm sure this castle will
 never be invaded again.[await]"""
dialog_data[250] = """[await]"""
dialog_data[
    251
] = """ It would really stink if another
 baddie invaded the castle.[await]"""
dialog_data[252] = """[await]"""
dialog_data[253] = """[await]"""
dialog_data[254] = """[await]"""
dialog_data[255] = """[await]"""
dialog_data[256] = """[await]"""
dialog_data[257] = """[await]"""
dialog_data[
    258
] = """[page]
 You're KILLIN' me here!
 All rightee then, how about 10?
 That’s TEN Frog Coins![await]
  [select]  (It’s a deal!)
  [select]  (Nope)[await]"""
dialog_data[
    259
] = """[page]
 Ho ho!...`MAIN_CHARACTER_TITLE_SHORT`. Wise `MAIN_CHARACTER_GENDER_CASUAL` doesn't
 think 10 Frog Coins is enough![await]
 You don't know what you're
 missing, but...OH, WELL...[await]"""
dialog_data[260] = """[await]"""
dialog_data[261] = """[await]"""
dialog_data[262] = """[await]"""
dialog_data[263] = """[await]"""
dialog_data[264] = """[await]"""
dialog_data[265] = """[await]"""
dialog_data[266] = """[await]"""
dialog_data[267] = """[await]"""
dialog_data[268] = """[await]"""
dialog_data[
    269
] = """ See that chest up there?[await][pause] Alanim
 could have kept it permanently
 missable.[delay] But he didn't.[await]
 What a great guy![await]"""
dialog_data[
    270
] = """ I would never snoop through the
 Princess' room.[await][pause] But I bet she has
 some cool stuff.[await]"""
dialog_data[271] = """[await]"""
dialog_data[272] = """[await]"""
dialog_data[273] = """[await]"""
dialog_data[274] = """[await]"""
dialog_data[275] = """[await]"""
dialog_data[276] = """[await]"""
dialog_data[
    277
] = """ Here in Marrymore, the townsfolk\n like to loiter around the chapel.[await]"""
dialog_data[
    278
] = """ Our wishes have come true,
 so we will be having our
 wedding in Marrymore![await]
 We'll be gone for a while!
                         Raz & Raini[await]"""
dialog_data[
    279
] = """ What’s that?
 You want your card back?[delay]
 Okay, well...it’s 15 Frog Coins.[await]
 [select]  (Here you go)
 [select]  (Forget it)[await]"""
dialog_data[280] = """[await]"""
dialog_data[
    281
] = """ Please enjoy my song.[await][pause] I composed
 it entirely with tadpoles.[await]"""
dialog_data[
    282
] = """ I'm grateful that absolutely nothing
 weird is going on here in Marrymore
 today.[await]"""
dialog_data[
    283
] = """ The chapel's entrance is locked
 down like Fort Knox.[await]
 It would be highly unusual for any
 weirdoes to be in there.[await]"""
dialog_data[
    284
] = """[page]
 Here, take it.
 Anyway, it felt good just to
 carry it around.[await]"""
dialog_data[
    285
] = """[page]
 Oh yeah, well...
 I want to hold onto it, so...
 you can forget about paying up.[await]"""
dialog_data[286] = """[await]"""
dialog_data[
    287
] = """YOSHI:
       (There’s nothing in here.)[await]"""
dialog_data[
    288
] = """
         There’s nothing inside.[await]"""
dialog_data[289] = """[await]"""
dialog_data[290] = """[await]"""
dialog_data[
    291
] = """ (Hey there! What’s happening?)[await]
 [select]  (I want to store my Cookies)
 [select]  (I want my Cookies back)
 [select]  (How’s it going?)[await]"""
dialog_data[
    292
] = """[page]
 (I'm storing [0x7024] for you!
  Right now you can hold [0x7000] items.
  [await][pause]So, how many do you want?)[await][pause][end]"""
dialog_data[293] = """ [0x7024][end]"""
dialog_data[
    294
] = """[page]
 [0x7024][end]"""
dialog_data[
    295
] = """[page]
 (We're getting along better
  than ever!)[await]"""
dialog_data[
    296
] = """ (You gave me [0x7024]. So I'm
  storing [0x7000] for you!)[await]"""
dialog_data[
    297
] = """[page]

          (All right, see ya!)[await]"""
dialog_data[
    298
] = """ (I gave you [0x7024]. I'll store
  the other [0x7000] for you!)[await]"""
dialog_data[
    299
] = """[page]
 (You don't have any Cookies
  stored right now. For starters,
  try and store 5 or more.[await][pause][end]"""
dialog_data[
    300
] = """ (Sorry. If I store [0x7024] more[await]
  it'll hit the 200 and over zone!
  That’s more than I can hold.)[await]"""
dialog_data[
    301
] = """[page]

      (Out of Cookies to store!)[await]"""
dialog_data[
    302
] = """[page]

   (Out of Cookies to take back!)[await]"""
dialog_data[
    303
] = """ (Okay, here’s [0x7024]. You don't
  have any more to spare, so use
  what’s left wisely!)[await]"""
dialog_data[
    304
] = """[page]

      (You have too many items!)[await]"""
dialog_data[
    305
] = """[page]
 (Sorry.[await]
  I'm already storing 200 and
  I can't store anymore.)[await]"""
dialog_data[
    306
] = """BOSHI: You lookin' at me?[await]
 [select]  (Yeah! Let’s race)
 [select]  (Uh, no...)[await]"""
dialog_data[
    307
] = """[page]
 (The odds on Yoshi are
  [0x7000] : 1. What will you do?)[await]
 [select]  (I'll race Yoshi)
 [select]  (I'll hop off and watch Yoshi)[await]"""
dialog_data[
    308
] = """[page]
 (This time, the odds on Yoshi
  are [0x7000].5 : 1 Well?)[await]
  [select]  (I'll race Yoshi)
  [select]  (I'll hop off and watch Yoshi)[await]"""
dialog_data[
    309
] = """[page]
 (So how many Cookies
  are you gonna wager?[await]
  You can wager up to 10.)[await]"""
dialog_data[
    310
] = """[page]

         (Nice weather, eh?)[await]"""
dialog_data[
    311
] = """BOSHI: (Count me in for [0x7024]![await]
 To the starting gate, group!)[await]"""
dialog_data[
    312
] = """ (You're carrying a lot of things![await]
  I'll store this [0x7000] Cookie
  for you. So, your total is [0x7024].)[await]"""
dialog_data[
    313
] = """[page]
BOSHI: (No Cookies, NO RACE.)[await]"""
dialog_data[314] = """[await]"""
dialog_data[
    315
] = """YOSHI: (Looks like he wants some
 Cookies. Wanna give him some?)[await]
 [select]  (Sure, why not?)
 [select]  (No, let’s not)[await]"""
dialog_data[
    316
] = """[page]

  (How many should we give him?)[await]"""
dialog_data[317] = """[await]"""
dialog_data[318] = """[await]"""
dialog_data[
    319
] = """[page]
 (Right now you have [0x7024].
  And I'm storing [0x7000]. How
  many more do you want stored?)[await][pause][end]"""
dialog_data[
    320
] = """[page]
 (Hm...looks like you don't
  have enough. Next time, remember
  to bring 5 or more, OK?)[await]"""
dialog_data[
    321
] = """
  How many do you want stored?)[await][pause][end]"""
dialog_data[
    322
] = """[page]
 Want to give him some Cookies?[await]
 [select]  (Sure)
 [select]  (No way)[await]"""
dialog_data[323] = """[await]"""
dialog_data[324] = """[await]"""
dialog_data[
    325
] = """ (Phew! Now that’s what I
   call generous!)[await]"""
dialog_data[326] = """[await]"""
dialog_data[327] = """[await]"""
dialog_data[328] = """[await]"""
dialog_data[329] = """[await]"""
dialog_data[
    330
] = """[page]

       Hey, you're a little short![await]"""
dialog_data[331] = """[await]"""
dialog_data[332] = """[await]"""
dialog_data[
    333
] = """BOSHI:
    (Line it up, ON THE DOUBLE!)[await]"""
dialog_data[
    334
] = """ Those chests over there haven't
 been refilled, but some of the
 others in this castle might be.[await]"""
dialog_data[
    335
] = """ If you haven't played with the giant
 egg yet, you still can.[await]"""
dialog_data[
    336
] = """ The castle's last invaders left
 their pet birds here in our
 hallways.[await] But don't worry, we're
 taking good care of them.[await]"""
dialog_data[
    337
] = """ Please don't step on our citizen's
 heads. [delay_60]...Although, I guess if you
 do, I won't stop you.[await]"""
dialog_data[
    338
] = """ There's a weird guy hanging out
 just off the edge of the town
 square. I wonder what he's up to?[await]"""
dialog_data[
    339
] = """ It's worth looking everywhere, even
 in places you've already visited, if
 you get stuck.[await]
 You never know who might have
 some hot tips for you.[await]"""
dialog_data[
    340
] = """ Have you been to the volcano
 beyond the Hot Springs yet?[await]"""
dialog_data[341] = """[await]"""
dialog_data[342] = """[await]"""
dialog_data[343] = """[await]"""
dialog_data[344] = """[await]"""
dialog_data[345] = """[await]"""
dialog_data[346] = """[await]"""
dialog_data[347] = """[await]"""
dialog_data[348] = """[await]"""
dialog_data[349] = """[await]"""
dialog_data[350] = """[await]"""
dialog_data[351] = """[await]"""
dialog_data[352] = """[await]"""
dialog_data[353] = """[await]"""
dialog_data[354] = """[await]"""
dialog_data[355] = """[await]"""
dialog_data[356] = """[await]"""
dialog_data[357] = """[await]"""
dialog_data[358] = """[await]"""
dialog_data[
    359
] = """ Have you tried the Dream Cushion
 at our Inn yet? It seems kinda
 weird to me.[await]"""
dialog_data[
    360
] = """ If you have both keys to the castle,
 you can go all the way through it
 and leave through the back door.[await][page]
 The new ruler won't like it if you
 do that, though, so you might have
 to fight them next time you visit.[await]"""
dialog_data[
    361
] = """ If you get stuck, be sure to
 talk to townspeople.[await][pause] They might
 give you some info to help point
 you in the right direction.[await]"""
dialog_data[
    362
] = """ You're looking for Star Pieces?[await]
 Hm...[delay_30] I think I heard Garro, our
 local sculptor, mention something
 about those.[await]"""
dialog_data[
    363
] = """ I heard Marrymore's hotel gives out
 gifts if you stay in their suite
 multiple times.[await][page]
 I wonder if our inn offers any
 freebies with the Dream Cushion?[delay_30] It
 might help our tourism sector![await]"""
dialog_data[
    364
] = """ I heard a rumour about a giant egg
 in the castle.[delay_30] I wonder what's
 going to hatch from it?![await]"""
dialog_data[
    365
] = """ Looking for special items? I think
 some people in the castle might
 have some they can give you.[await]
 Just be careful, the birds in there
 are pretty strong.[await]"""
dialog_data[
    366
] = """ The statues in the castle look
 shinier than they used to. Maybe
 Garro stocked up on gold paint.[await]"""
dialog_data[
    367
] = """ You might find something nice if
 you try to walk off the east side
 of our town square.[await]
 ...But only if you get into a fight
 with the castle's owner first.[await]
 Why?[delay_30] I don't know.[delay] We have some
 weird laws here.[await]"""
dialog_data[
    368
] = """ Garro lives in the north part of
 town, just west of the castle.[await][pause] He
 can bring you to the castle's
 statue polisher.[await]"""
dialog_data[369] = """[await]"""
dialog_data[370] = """[await]"""
dialog_data[371] = """[await]"""
dialog_data[372] = """[await]"""
dialog_data[373] = """[await]"""
dialog_data[374] = """[await]"""
dialog_data[375] = """[await]"""
dialog_data[376] = """[await]"""
dialog_data[377] = """[await]"""
dialog_data[378] = """[await]"""
dialog_data[379] = """[await]"""
dialog_data[380] = """[await]"""
dialog_data[381] = """[await]"""
dialog_data[382] = """[await]"""
dialog_data[383] = """[await]"""
dialog_data[384] = """[await]"""
dialog_data[385] = """[await]"""
dialog_data[386] = """[await]"""
dialog_data[387] = """[await]"""
dialog_data[388] = """[await]"""
dialog_data[389] = """[await]"""
dialog_data[390] = """[await]"""
dialog_data[391] = """[await]"""
dialog_data[392] = """[await]"""
dialog_data[393] = """[await]"""
dialog_data[394] = """[await]"""
dialog_data[395] = """[await]"""
dialog_data[396] = """[await]"""
dialog_data[397] = """[await]"""
dialog_data[398] = """[await]"""
dialog_data[399] = """[await]"""
dialog_data[400] = """[await]"""
dialog_data[401] = """[await]"""
dialog_data[402] = """[await]"""
dialog_data[403] = """[await]"""
dialog_data[404] = """[await]"""
dialog_data[405] = """[await]"""
dialog_data[406] = """[await]"""
dialog_data[407] = """[await]"""
dialog_data[408] = """[await]"""
dialog_data[409] = """[await]"""
dialog_data[410] = """[await]"""
dialog_data[411] = """[await]"""
dialog_data[412] = """[await]"""
dialog_data[413] = """[await]"""
dialog_data[414] = """[await]"""
dialog_data[415] = """[await]"""
dialog_data[
    416
] = """ There's a heck of a lot going on in
 the castle.[await][pause] But Garro can show you
 secrets about it that you can't
 find on your own![await]"""
dialog_data[
    417
] = """   (Oops[delay][delay]...[delay][delay]I've lost my dentures)[await][pause][end]"""
dialog_data[418] = """ Now[delay][delay]...[delay][delay]PARTY TIME!![await]"""
dialog_data[
    419
] = """ Hello.
 How may I help you today?[await]
 [select]  (I'd like to buy some things)
 [select]  (I'd like to rest awhile)
 [select]  (I'm fine, thank you)[await]"""
dialog_data[
    420
] = """[page]
 Yes, I see.[await]
 We here at the Marrymore Hotel
 are offering a very special
 deal for a limited time only.[await][page]
 If you choose to stay in our lovely
 deluxe suite, you shall receive a
 once in a lifetime gift![await][page]
 Now which room will you choose?[await]
 [select]  (The regular room)  10 coins
 [select]  (The SUITE!)       200 coins
 [select]  (Oh wait, forget it)[await]"""
dialog_data[
    421
] = """[page]
 Oh, EXCELLENT CHOICE, `MAIN_CHARACTER_HONORIFIC`![await]
 We shall attend to you in a minute.[await]"""
dialog_data[
    422
] = """[page]
 We thank you very much!
 You have stayed with us [0x7000]
 time(s).[await][page]
 Like always, the bellhop shall
 guide your way.[await]
 Please enjoy your stay![await]"""
dialog_data[
    423
] = """ Oh, hello there! Are you visiting?[await]
 We don't get tourists here very
 often. I guess our town is a little
 bit out of the way.[await][page]
 If you're looking for something fun
 to do, you should visit our
 volcano![await]"""
dialog_data[
    424
] = """[page]
 I'm sorry `MAIN_CHARACTER_HONORIFIC`.
 You seem to be a day late and
 a few coins short! BYE![await]"""
dialog_data[425] = """[await]"""
dialog_data[
    426
] = """[page]
 CONGRATULATIONS!!  You, `MAIN_CHARACTER_HONORIFIC`,
 have checked in our lovely suite,
 a grand total of [0x7000] times![await][page]
 In order to show our appreciation,
 we are proud to present you with
 this special commemorator’s gift![await]"""
dialog_data[
    427
] = """All the coins, even the Frog Coins,
            have been taken![await]"""
dialog_data[
    428
] = """ `MAIN_CHARACTER_HONORIFIC_CAP`![await]
 You have stayed over 200 times
 for the price of just ONE night![await][page]
 I shall receive 100 coins for each
 night you have extended your stay![await][pause][end]"""
dialog_data[
    429
] = """[page]
 ...Well! It’s no surprise that
 you're unable to pay!![await]
 I'll take what you have on you,
 and you'll work for the rest![await]"""
dialog_data[430] = """[await]"""
dialog_data[431] = """[await]"""
dialog_data[432] = """[await]"""
dialog_data[433] = """[await]"""
dialog_data[434] = """[await]"""
dialog_data[435] = """[await]"""
dialog_data[436] = """[await]"""
dialog_data[437] = """[await]"""
dialog_data[438] = """[await]"""
dialog_data[439] = """[await]"""
dialog_data[440] = """[await]"""
dialog_data[441] = """[await]"""
dialog_data[442] = """[await]"""
dialog_data[443] = """[await]"""
dialog_data[444] = """[await]"""
dialog_data[
    445
] = """
         Where's the Crown?![await]"""
dialog_data[446] = """[await]"""
dialog_data[447] = """[await]"""
dialog_data[448] = """[await]"""
dialog_data[449] = """[await]"""
dialog_data[450] = """[await]"""
dialog_data[451] = """[await]"""
dialog_data[
    452
] = """BOOSTER: Found something, huh?[await]
 But wait a sec...I've only
 counted [0x7000] thing(s).
 You need to find [0x7024] more![await]"""
dialog_data[453] = """[await]"""
dialog_data[
    454
] = """ Have you heard about the Casino?[await]
 The guy who owns it has some
 pretty sketchy connections. [await]"""
dialog_data[455] = """[await]"""
dialog_data[
    456
] = """[page]
 Yes, I see.
 Which room do you prefer?[await]
 [select]  (The regular room)  10 coins
 [select]  (The suite)         200 coins
 [select]  (Oh, forget it)[await]"""
dialog_data[
    457
] = """ `MAIN_CHARACTER_HONORIFIC_CAP`![await]
 You seem to have stayed [0x7000]
 times for the price of just...[await][page]
 ONE NIGHT! I'll have to receive
 100 coins for each night you have
 extended your stay.[await]"""
dialog_data[
    458
] = """ (I can only store 200 cookies.[await][page]
  I'll store [0x7000], but....[await]
  I think I'll nibble on the rest
  of the [0x7024].)[await]"""
dialog_data[459] = """[await]"""
dialog_data[460] = """[await]"""
dialog_data[461] = """[await]"""
dialog_data[462] = """[await]"""
dialog_data[463] = """[await]"""
dialog_data[464] = """[await]"""
dialog_data[465] = """[await]"""
dialog_data[466] = """[await]"""
dialog_data[467] = """[await]"""
dialog_data[468] = """[await]"""
dialog_data[469] = """[await]"""
dialog_data[470] = """[await]"""
dialog_data[471] = """[await]"""
dialog_data[472] = """[await]"""
dialog_data[473] = """[await]"""
dialog_data[474] = """[await]"""
dialog_data[475] = """[await]"""
dialog_data[476] = """[await]"""
dialog_data[477] = """[await]"""
dialog_data[478] = """[await]"""
dialog_data[479] = """\n        He's holding a leaflet...[await]"""
dialog_data[
    480
] = """      Come to Grate Guy Casino!
    Find us a quick turn down the
  north pipe of central Bean Valley.[await]
   Don't forget your Bright Card![await]"""
dialog_data[481] = """[await]"""
dialog_data[482] = """[await]"""
dialog_data[483] = """[await]"""
dialog_data[
    484
] = """ Keep working!
 You're not getting any breaks.[await]"""
dialog_data[485] = """[await]"""
dialog_data[
    486
] = """ Hey `MAIN_CHARACTER_MOLE_GREETING`, step right up!
 10 coins to do some Goomba
 thumping. How about it?[await]
 [select]  (Sure, why not?)
 [select]  (I'll save it for another day)[await]"""
dialog_data[
    487
] = """[page]
 You know the rules?[await]
 [select]  (Sure do)
 [select]  (Nope)[await]"""
dialog_data[
    488
] = """[page]
 Thump a Goomba, get 1 pt.
 Thump Spikey, and lose 1 pt.
 Thump a gold Goomba, get 3 pts.[await][page]
 If you get more than [0x7000] pts.,
 you get a prize. Want a recap?[await]
  [select]  (No thanks)
  [select]  (Just once more)[await]"""
dialog_data[489] = """[await]"""
dialog_data[490] = """[await]"""
dialog_data[491] = """[await]"""
dialog_data[492] = """[await]"""
dialog_data[493] = """[await]"""
dialog_data[494] = """[await]"""
dialog_data[
    495
] = """KNIFE GUY: So which hand is
 it in? Touch the hand you think
 is holding the yellow ball.[end]"""
dialog_data[496] = """[await]"""
dialog_data[497] = """[await]"""
dialog_data[498] = """[await]"""
dialog_data[499] = """[await]"""
dialog_data[500] = """[await]"""
dialog_data[501] = """[await]"""
dialog_data[502] = """[await]"""
dialog_data[503] = """[await]"""
dialog_data[504] = """[await]"""
dialog_data[
    505
] = """SNIFIT 1: Hello there.[await]
 Booster’s busy right now, so he
 can't play.[await][page]
 Come back some other time, or you
 can try to force your way in...[await]"""
dialog_data[
    506
] = """
           Booster the First[await]"""
dialog_data[
    507
] = """
          Booster the Second[await]"""
dialog_data[
    508
] = """
           Booster the Third[await]"""
dialog_data[
    509
] = """
          Booster the Fourth[await]"""
dialog_data[
    510
] = """
           Booster the Fifth[await]"""
dialog_data[
    511
] = """
           Booster the Sixth[await]"""
dialog_data[512] = """[await]"""
dialog_data[513] = """[await]"""
dialog_data[514] = """[await]"""
dialog_data[515] = """[await]"""
dialog_data[
    516
] = """ I wonder when my next spouse will
 fall from the sky?[await]"""
dialog_data[
    517
] = """SNIFIT 2: Please refrain
 from bothering Booster.[await]"""
dialog_data[518] = """[await]"""
dialog_data[519] = """[await]"""
dialog_data[
    520
] = """HINOPIO: Our thermostat is busted,
 so I'm offering a discount inn rate
 of 30 coins a night.[await]
  [select] (I'll take it)
  [select] (Yikes, no thanks)[await]"""
dialog_data[521] = """[await]"""
dialog_data[
    522
] = """ You don't have enough coins.[await]
 Come back when you get more.[await]"""
dialog_data[523] = """[await]"""
dialog_data[524] = """[await]"""
dialog_data[525] = """[await]"""
dialog_data[
    526
] = """TOADOFSKY: Finally!
 My song is nearly complete.[await][page]
 But I must confess that you
 contributed a lot to it.[await][page]
 We can all take credit for it![await][page]
 Now you must think of a final
 phrase that will wrap it all up.[await][page]
 Let your creative juices flow and
 show me whatcha got![await]"""
dialog_data[527] = """[await]"""
dialog_data[528] = """[await]"""
dialog_data[529] = """[await]"""
dialog_data[
    530
] = """ I heard your record for jumping
 stands at [0x7000] right now.[await]"""
dialog_data[531] = """ Think you can do `SUPER_JUMP_PRIZE_1_CAP` in a row?[await]"""
dialog_data[
    532
] = """ Wow, you did it!
 You made over `SUPER_JUMP_PRIZE_1_CAP` jumps.[await][page]
 All right, here’s your prize.[await]"""
dialog_data[533] = """[await]"""
dialog_data[
    534
] = """ According to my source, it's
 impossible to jump more than
 100 times.[await][page]
  You've gotten `SUPER_JUMP_PRIZE_2_CAP`, so
  here's a final prize for you![await]"""
dialog_data[
    535
] = """ I've got no more prizes for you.
 Well done![await]"""
dialog_data[536] = """[await]"""
dialog_data[537] = """[await]"""
dialog_data[538] = """[await]"""
dialog_data[
    539
] = """ Some of my friends are hiding away
 in chests around the world.[await]
 If you fight them, they might give
 you Star Pieces.[await][page]
 And you should remember to visit
 them again afterwards.[await][pause] They might
 have extra goodies for you.[await]"""
dialog_data[540] = """[await]"""
dialog_data[541] = """[await]"""
dialog_data[542] = """[await]"""
dialog_data[543] = """[await]"""
dialog_data[544] = """[await]"""
dialog_data[545] = """[await]"""
dialog_data[
    546
] = """ Are you looking for Toadofsky?[delay_15]
 He's probably busy right now.[await][page]
 To pass the time...[delay_15] have you taken
 a ride on the minecart in Moleville?[await]
 I'd love to try it once I grow
 a pair of legs.[await]"""
dialog_data[547] = """[await]"""
dialog_data[
    548
] = """ Are you looking for Toadofsky?[delay_15]
 He'll be back later.[await][page]
 To pass the time...[delay_15] have you been
 to the temple in Land's End?[await]
 I hear it's quite the attraction
 for treasure hunters.[await]"""
dialog_data[549] = """ My favorite song?[await]"""
dialog_data[
    550
] = """ My favorite song?[await][page]
 It’s Frogfucius' suite #18.
 ♪“So La Mi Re Do Re Do Re”.
 Toadofsky’s fond of it, too![await]"""
dialog_data[
    551
] = """ My favorite song?[await][page]
 It’s the Moleville miners' song,
 ♪“Mi Do So Do Re La Ti Do”.
 It’s got soul![await]"""
dialog_data[552] = """[await]"""
dialog_data[553] = """[await]"""
dialog_data[
    554
] = """ It’s the song that the Monstro
 Town star dances to.[await]"""
dialog_data[555] = """[await]"""
dialog_data[556] = """[await]"""
dialog_data[557] = """[await]"""
dialog_data[558] = """[await]"""
dialog_data[559] = """ Oops, I forgot the rest.[await]"""
dialog_data[
    560
] = """ I heard your composition, and it's
 pretty good.[await]"""
dialog_data[561] = """[await]"""
dialog_data[562] = """[await]"""
dialog_data[563] = """[await]"""
dialog_data[564] = """[await]"""
dialog_data[565] = """[await]"""
dialog_data[566] = """[await]"""
dialog_data[567] = """[await]"""
dialog_data[568] = """[await]"""
dialog_data[569] = """[await]"""
dialog_data[570] = """[await]"""
dialog_data[571] = """[await]"""
dialog_data[572] = """[await]"""
dialog_data[573] = """[await]"""
dialog_data[574] = """[await]"""
dialog_data[575] = """[await]"""
dialog_data[576] = """[await]"""
dialog_data[577] = """[await]"""
dialog_data[578] = """[await]"""
dialog_data[579] = """[await]"""
dialog_data[580] = """[await]"""
dialog_data[581] = """[await]"""
dialog_data[582] = """[await]"""
dialog_data[583] = """[await]"""
dialog_data[584] = """[await]"""
dialog_data[585] = """[await]"""
dialog_data[586] = """[await]"""
dialog_data[587] = """[await]"""
dialog_data[588] = """[await]"""
dialog_data[589] = """[await]"""
dialog_data[590] = """[await]"""
dialog_data[591] = """[await]"""
dialog_data[592] = """[await]"""
dialog_data[593] = """[await]"""
dialog_data[594] = """[await]"""
dialog_data[595] = """[await]"""
dialog_data[596] = """[await]"""
dialog_data[597] = """[await]"""
dialog_data[598] = """ You're on your way to composing![await]"""
dialog_data[
    599
] = """
       So La Mi Re Do Re Do Re [await]"""
dialog_data[
    600
] = """TOADOFSKY: You want a music
 lesson from MOI?[await]
  [select] (Yes)
  [select] (Nope)[await]"""
dialog_data[
    601
] = """ Let’s start by reading “notes”.
 See the five lines?[await][page]
 Between the second and third lines
 from the top, lies “Do”.[await][page]
 Keep going up, and the notes are
 “Do Re Mi”. Go down and you'll
 get “Do Ti La So Fa”.[await]"""
dialog_data[602] = """ Well, be that way![await]"""
dialog_data[603] = """[await]"""
dialog_data[604] = """[await]"""
dialog_data[605] = """[await]"""
dialog_data[
    606
] = """ It’s not what I'm looking for.
 It won't do at all.[await]"""
dialog_data[607] = """ No, that’s not the right melody.[await]"""
dialog_data[
    608
] = """ That sort of resembles the melody
 I'm thinking of.[await]"""
dialog_data[
    609
] = """ That’s remarkably similar to what
 I have in mind.[await]"""
dialog_data[
    610
] = """ Now that is amazingly close to my
 dream composition![await]"""
dialog_data[611] = """FROGFUCIUS: Are you stuck?[await]\n[end]"""
dialog_data[612] = """ An item awaits you in your home.[await]"""
dialog_data[
    613
] = """ Unfinished business awaits you
 in Bandit's Way.[await]"""
dialog_data[
    614
] = """ Have you visited the Mushroom
 Kingdom lately?[await]"""
dialog_data[
    615
] = """ Have you traversed through the
 secret tunnel in Land's End?[await]"""
dialog_data[
    616
] = """ My contemporary, Toadofsky,
 awaits a visit from you.[await]"""
dialog_data[617] = """ Treasure awaits you in Rose Town.[await]"""
dialog_data[
    618
] = """ It may prove worth your while to
 visit Yo'ster Isle.[await]"""
dialog_data[
    619
] = """ There seems to be some unclaimed
 treasure in the town of Moleville.[await]"""
dialog_data[
    620
] = """ When was the last time you
 ventured into the Mines?[await]"""
dialog_data[
    621
] = """ Have you visited the tall tower
 to the west yet?[await]"""
dialog_data[
    622
] = """ Something awaits you in Seaside
 Town.[await]"""
dialog_data[
    623
] = """ The ever-friendly Monstro Town
 may have something for you
 to find.[await]"""
dialog_data[
    624
] = """ Bean Valley may have some tasks
 for you to take care of.[await]"""
dialog_data[
    625
] = """ It may help to pay a visit to
 the Nimbus Land town square.[await]"""
dialog_data[
    626
] = """ There may be some secrets left
 to uncover in Nimbus Castle.[await]"""
dialog_data[
    627
] = """ There may be something of interest
 to you in Mushroom Way.[await]"""
dialog_data[
    628
] = """ You are likely to find more treasure
 in the Sunken Ship.[await]"""
dialog_data[
    629
] = """ You may find something new if you
 venture into the Kero Sewers.[await]"""
dialog_data[
    630
] = """ The Forest Maze has much to
 discover.[await]"""
dialog_data[
    631
] = """ It may be worthwhile to pay a
 visit to Marrymore.[await]"""
dialog_data[
    632
] = """ Land's End may have something of
 interest to you.[await]"""
dialog_data[
    633
] = """ Belome Temple may have something
 for you to investigate.[await]"""
dialog_data[
    634
] = """ It would be wise to pay a visit
 to Booster Hill.[await]"""
dialog_data[
    635
] = """ Something awaits you in Barrel
 Volcano.[await]"""
dialog_data[
    636
] = """ You may find something to do in
 Bowser's Keep.[await]"""
dialog_data[
    637
] = """ There is still work to be done in
 the Factory.[await]"""
dialog_data[
    638
] = """ Try your luck in Grate Guy's
 Casino.[await]"""
dialog_data[
    639
] = """ Try double-checking whatever
 you can. I may have more advice
 for you later.[await]"""
dialog_data[
    640
] = """FROGFUCIUS: Eh!?[await]
 That delicious smell!
 You have some Cricket Jam, yes?[await][page]
 Oh, but I have no pie to eat it
 with...[await]"""
dialog_data[
    641
] = """ You may find something new in
 Midas River.[await]"""
dialog_data[
    642
] = """ Rose Way may be small, but it may
 have something new for you to
 find.[await]"""
dialog_data[
    643
] = """ Treasure awaits you in the Pipe
 Vault.[await]"""
dialog_data[
    644
] = """ Unclaimed items remain in
 Booster Pass.[await]"""
dialog_data[
    645
] = """ Check at the top of Star Hill.
 It is worth checking to see if a
 star piece is there or not.[await]"""
dialog_data[
    646
] = """ Perhaps take some time to rest
 in Marrymore, and something may
 come to you.[await]"""
dialog_data[
    647
] = """ An amphibian salesman in Seaside
 Town may be seeking your business.[await]"""
dialog_data[
    648
] = """ There are some unopened chests
 remaining in the Sea.[await]"""
dialog_data[
    649
] = """ The beanstalks may bring you some
 new fortune.[await]"""
dialog_data[
    650
] = """ I haven't heard much news about
 new treasures lately.[await][pause] Say... Have
 you been practicing Super Jumps?[await]"""
dialog_data[
    651
] = """ I recommend exploring the obstacle
 courses deep in Bowser's Keep.[await]"""
dialog_data[652] = """[await]"""
dialog_data[653] = """[await]"""
dialog_data[654] = """[await]"""
dialog_data[655] = """[await]"""
dialog_data[656] = """[await]"""
dialog_data[657] = """[await]"""
dialog_data[658] = """[await]"""
dialog_data[659] = """[await]"""
dialog_data[660] = """[await]"""
dialog_data[661] = """[await]"""
dialog_data[662] = """[await]"""
dialog_data[663] = """[await]"""
dialog_data[664] = """[await]"""
dialog_data[665] = """[await]"""
dialog_data[666] = """[await]"""
dialog_data[667] = """[await]"""
dialog_data[668] = """[await]"""
dialog_data[669] = """[await]"""
dialog_data[670] = """[await]"""
dialog_data[671] = """[await]"""
dialog_data[672] = """[await]"""
dialog_data[673] = """[await]"""
dialog_data[674] = """[await]"""
dialog_data[675] = """[await]"""
dialog_data[676] = """         Need the key to get in[await]"""
dialog_data[
    677
] = """
     Need the “Shed Key” to get in[await]"""
dialog_data[678] = """[await]"""
dialog_data[679] = """[await]"""
dialog_data[680] = """[await]"""
dialog_data[681] = """[await]"""
dialog_data[682] = """[await]"""
dialog_data[683] = """[await]"""
dialog_data[684] = """[await]"""
dialog_data[685] = """               It’s locked[await]"""
dialog_data[
    686
] = """      Tried saying the password:[delay]
              [filename][await]"""
dialog_data[
    687
] = """                  No luck...[delay]
    Booster changed the password.[delay]
                 Oh, well...[await]"""
dialog_data[688] = """[await]"""
dialog_data[689] = """[await]"""
dialog_data[690] = """[await]"""
dialog_data[691] = """[await]"""
dialog_data[692] = """[await]"""
dialog_data[693] = """[await]"""
dialog_data[694] = """[await]"""
dialog_data[
    695
] = """      Take a look at Booster's
   family portraits in order from
     the oldest to the youngest.[await]"""
dialog_data[696] = """  Remember to respect your elders.[await]"""
dialog_data[697] = """[await]"""
dialog_data[698] = """[await]"""
dialog_data[699] = """[await]"""
dialog_data[700] = """[await]"""
dialog_data[701] = """[await]"""
dialog_data[702] = """[await]"""
dialog_data[703] = """[await]"""
dialog_data[
    704
] = """YARIDOVICH: Finally![await]
 It took you long enough to get
 here![await]"""
dialog_data[
    705
] = """ This is...the inn.
 You may stay...for free.[await]
  [select] (Thanks)
  [select] (I'll pass)[await]"""
dialog_data[706] = """[await]"""
dialog_data[
    707
] = """ The workers...in the left
 building...they're acting...
 quite strange.[await]"""
dialog_data[708] = """[await]"""
dialog_data[
    709
] = """ If you...want to enter the ocean,
 jump into...the whirlpools."""
dialog_data[
    710
] = """ If you cannot...enter the ocean,
 you may have to...do something
 else first.[await]"""
dialog_data[
    711
] = """ You will find Yaridovich...
 in his house. He is...the most
 respected person here.[await]"""
dialog_data[
    712
] = """ It would be wise... to avoid
 the shed. It's...a scary place.[await]"""
dialog_data[713] = """[await]"""
dialog_data[
    714
] = """ In the middle...of the ocean is a
 pirate...ship.[await]
 You'll find a...I mean, there’s a...[await]"""
dialog_data[
    715
] = """ A treasure chest...behind some...
 boxes. It's very easy...to miss it.[await]"""
dialog_data[
    716
] = """ If you...survive...the pirate
 ship...we will...we will...[await]"""
dialog_data[
    717
] = """ We will...make it...worth your
 while.[await]"""
dialog_data[718] = """ I'm...just a...customer.[await]"""
dialog_data[719] = """[await]"""
dialog_data[
    720
] = """ You...may not enter here.
 This is...a restricted area.[await]"""
dialog_data[
    721
] = """ There is nothing of interest here.
 Don't...bother looking inside.[await]"""
dialog_data[
    722
] = """                   Hey![await]
          Someone’s in there.[await]"""
dialog_data[723] = """[await]"""
dialog_data[724] = """[await]"""
dialog_data[725] = """[await]"""
dialog_data[726] = """[await]"""
dialog_data[727] = """[await]"""
dialog_data[728] = """[await]"""
dialog_data[729] = """[await]"""
dialog_data[730] = """[await]"""
dialog_data[731] = """[await]"""
dialog_data[732] = """[await]"""
dialog_data[733] = """[await]"""
dialog_data[734] = """[await]"""
dialog_data[735] = """[await]"""
dialog_data[736] = """[await]"""
dialog_data[737] = """[await]"""
dialog_data[738] = """[await]"""
dialog_data[739] = """               It’s locked[await]"""
dialog_data[740] = """[await]"""
dialog_data[741] = """[await]"""
dialog_data[742] = """[await]"""
dialog_data[743] = """[await]"""
dialog_data[744] = """[await]"""
dialog_data[745] = """[await]"""
dialog_data[746] = """[await]"""
dialog_data[747] = """[await]"""
dialog_data[748] = """[await]"""
dialog_data[749] = """[await]"""
dialog_data[750] = """[await]"""
dialog_data[751] = """[await]"""
dialog_data[752] = """[await]"""
dialog_data[753] = """[await]"""
dialog_data[754] = """[await]"""
dialog_data[755] = """[await]"""
dialog_data[756] = """[await]"""
dialog_data[757] = """[await]"""
dialog_data[758] = """[await]"""
dialog_data[
    759
] = """DR. TOPPER: Give up?[await]
 [select]  (Still working on it)
 [select]  (I GIVE UP!)[await]"""
dialog_data[760] = """[await]"""
dialog_data[761] = """[await]"""
dialog_data[762] = """[await]"""
dialog_data[763] = """[await]"""
dialog_data[764] = """[await]"""
dialog_data[765] = """[await]"""
dialog_data[766] = """[await]"""
dialog_data[767] = """[await]"""
dialog_data[768] = """[await]"""
dialog_data[769] = """[await]"""
dialog_data[770] = """[await]"""
dialog_data[
    771
] = """ I don't think I'm going to find any
 more items today.[await][pause] But if you
 change your mind about the ones
 I have, feel free to come again.[await]"""
dialog_data[772] = """[await]"""
dialog_data[773] = """[await]"""
dialog_data[
    774
] = """ Check back with me after you kill
 some time by... I don't know,
 exploring a volcano, or something.[await]"""
dialog_data[775] = """[await]"""
dialog_data[
    776
] = """ I'm all out of inventory, and won't
 be selling any more.[await][pause] Thanks for
 your business![await]"""
dialog_data[
    777
] = """ Item #2: A “Mystery Egg”.
 I have no idea what it does!
 It sort of grows on ya, huh?[await]
 It’s yours for 200 coins.
  [select] (Okay)
  [select] (No thanks)[await]"""
dialog_data[778] = """ That’s all I've got for now.[await]"""
dialog_data[
    779
] = """ You need more coins![await]
 I gotta eat, you know![await]"""
dialog_data[
    780
] = """ Item #1: A “Lucky Jewel”!
 It’s sure to bring you plenty of
 good luck.[await]
 I'll sell it to you for 100 coins.
  [select] (It’s a deal)
  [select] (I'll pass)[await]"""
dialog_data[
    781
] = """ Nice doing business with you.
 Here you go![await]"""
dialog_data[
    782
] = """ Oops, I didn't realize I was out
 of inventory.[await]"""
dialog_data[
    783
] = """ Item #3: A “Metal Plate”.[await]
 Don't know what it’s used for,
 but I'll sell it for 300 coins.
  [select] (I'll take it)
  [select] (No thanks)[await]"""
dialog_data[
    784
] = """ Check back with me after you kill
 some time by... I don't know,
 liberating a beach, or something.[await]"""
dialog_data[
    785
] = """ Please rest up before you go.
 It'll only cost you 15 coins.[await]
  [select] (Okay)
  [select] (No)[await]"""
dialog_data[
    786
] = """ Okay, some other time then.
 Have a safe trip.[await]"""
dialog_data[
    787
] = """ You don't have enough coins?[await][page]
 That’s ok, just pay me whatever
 you can.
 I know you need to rest.[await]"""
dialog_data[788] = """[await]"""
dialog_data[789] = """[await]"""
dialog_data[790] = """[await]"""
dialog_data[791] = """[await]"""
dialog_data[
    792
] = """ELDER: Hello, `MAIN_CHARACTER_NAME`![await][pause]
 Have you met my, uh, long-term
 houseguest, who's taken over
 my second floor?[await]"""
dialog_data[793] = """[await]"""
dialog_data[794] = """[await]"""
dialog_data[
    795
] = """ Speaking of items, it looks like
 you've got too many and can't
 carry them all.[await]"""
dialog_data[
    796
] = """ Oops, sorry I'm all out of items.[await][page]
 In other words,
          “Come back later!”[await]
 So...good day to you![await]"""
dialog_data[
    797
] = """ Hey, hey, HEY!![await]
 I'm the Mushroom Boy![await][page]
 Did you know that hidden amongst
 the regular mushrooms, are some
 special ones? Unreal huh?[await][page]
 I'll give you items in return for
 those special mushrooms.[await][page]
 But you've got to taste them to
 find out what kind they are.[await]
 So I'll take a nibble first.[await][page]
 I'm hoping for a special mushroom,
 but even if it isn't, at least I'll
 get a meal out of it.[await]"""
dialog_data[
    798
] = """ You have any mushrooms?
  [select] (Yes)
  [select] (Nope)[await]"""
dialog_data[
    799
] = """ Hey, far out!
 Munch munch munch.[await]"""
dialog_data[
    800
] = """ That was just a regular mushroom!
 Thanks for the snack.[await]"""
dialog_data[
    801
] = """ Ooh![await]
 This one’s a “Berry Mushroom”![await][page]
 I'll give you a “Maple Syrup”.
 Oh boy, I'm stuffed.[await]"""
dialog_data[
    802
] = """ All right![await]
 Like, this is unreal!
 It’s a “Rippin Mushroom”.[await][page]
 So I'll give you a “Rock Candy”.
 Hey thanks a lot![await]"""
dialog_data[
    803
] = """ Whooeee![await]
 This is most excellent!
 It’s a “Flower Mushroom”.[await][page]
 I'll give you a “Flower Tab” for it.
 Hey, thanks, dude.[await]"""
dialog_data[
    804
] = """ See you around`MAIN_CHARACTER_MBOY_GREETING`.
 Come again, all right?[await]"""
dialog_data[805] = """ Hey you don't have any mushrooms.[await]"""
dialog_data[806] = """      Received a “Maple Syrup”[await]"""
dialog_data[807] = """      Received a “Rock Candy”[await]"""
dialog_data[808] = """       Received a “Flower Tab”[await]"""
dialog_data[809] = """[await]"""
dialog_data[810] = """[await]"""
dialog_data[811] = """[await]"""
dialog_data[812] = """[await]"""
dialog_data[813] = """[await]"""
dialog_data[814] = """[await]"""
dialog_data[815] = """[await]"""
dialog_data[816] = """[await]"""
dialog_data[817] = """[await]"""
dialog_data[818] = """[await]"""
dialog_data[
    819
] = """         Attention, Civilians![await]
         Short on Frog Coins?[await]
 Climb our cliff in under 11 seconds,
      and win FIVE Frog Coins![await][page]
  Claim as many times as you like!
            Stop on by soon.
                      ~The Sky Troopas[await]"""
dialog_data[820] = """[await]"""
dialog_data[821] = """[await]"""
dialog_data[822] = """[await]"""
dialog_data[823] = """[await]"""
dialog_data[824] = """[await]"""
dialog_data[825] = """ Don't you dig these vibes?[await]"""
dialog_data[826] = """[await]"""
dialog_data[827] = """[await]"""
dialog_data[828] = """[await]"""
dialog_data[829] = """[await]"""
dialog_data[830] = """[await]"""
dialog_data[831] = """[await]"""
dialog_data[832] = """[await]"""
dialog_data[833] = """[await]"""
dialog_data[834] = """[await]"""
dialog_data[
    835
] = """ There's no hidden chests in this
 world, but some monsters live in
 chests.[await][page]
 If you're interested, you should
 talk to the tenant upstairs,
 he knows all about them.[await]"""
dialog_data[836] = """[await]"""
dialog_data[837] = """[await]"""
dialog_data[838] = """[await]"""
dialog_data[839] = """[await]"""
dialog_data[840] = """[await]"""
dialog_data[841] = """[await]"""
dialog_data[842] = """[await]"""
dialog_data[843] = """[await]"""
dialog_data[844] = """[await]"""
dialog_data[845] = """[await]"""
dialog_data[846] = """[await]"""
dialog_data[847] = """[await]"""
dialog_data[848] = """[await]"""
dialog_data[
    849
] = """  [select] Breeze
  [select] Gust
  [select] Blast[await]"""
dialog_data[850] = """[await]"""
dialog_data[851] = """[await]"""
dialog_data[852] = """[await]"""
dialog_data[853] = """[await]"""
dialog_data[854] = """[await]"""
dialog_data[855] = """[await]"""
dialog_data[856] = """[await]"""
dialog_data[857] = """[await]"""
dialog_data[858] = """[await]"""
dialog_data[859] = """[await]"""
dialog_data[860] = """[await]"""
dialog_data[861] = """[await]"""
dialog_data[862] = """[await]"""
dialog_data[863] = """[await]"""
dialog_data[864] = """[await]"""
dialog_data[865] = """[await]"""
dialog_data[
    866
] = """ Sorry, we're on vacation![await]
 If you find our flags, show them to
 our mushroom assistant.[await]
                  ~The 3 MUSTY FEARS[await]"""
dialog_data[867] = """[await]"""
dialog_data[868] = """[await]"""
dialog_data[869] = """[await]"""
dialog_data[870] = """[await]"""
dialog_data[871] = """[await]"""
dialog_data[872] = """[await]"""
dialog_data[873] = """[await]"""
dialog_data[874] = """[await]"""
dialog_data[875] = """[await]"""
dialog_data[876] = """[await]"""
dialog_data[877] = """[await]"""
dialog_data[878] = """[await]"""
dialog_data[879] = """[await]"""
dialog_data[880] = """[await]"""
dialog_data[881] = """[await]"""
dialog_data[882] = """[await]"""
dialog_data[883] = """[await]"""
dialog_data[884] = """[await]"""
dialog_data[885] = """[await]"""
dialog_data[886] = """[await]"""
dialog_data[887] = """[await]"""
dialog_data[888] = """[await]"""
dialog_data[889] = """[await]"""
dialog_data[890] = """[await]"""
dialog_data[891] = """[await]"""
dialog_data[892] = """[await]"""
dialog_data[893] = """[await]"""
dialog_data[894] = """[await]"""
dialog_data[895] = """[await]"""
dialog_data[896] = """[await]"""
dialog_data[897] = """[await]"""
dialog_data[898] = """[await]"""
dialog_data[899] = """[await]"""
dialog_data[900] = """[await]"""
dialog_data[
    901
] = """JAGGER: Now this should be
 interesting. Can you beat THE
 master, `MAIN_CHARACTER_NAME`?[await]"""
dialog_data[902] = """[await]"""
dialog_data[903] = """[await]"""
dialog_data[904] = """[await]"""
dialog_data[905] = """[await]"""
dialog_data[906] = """[await]"""
dialog_data[907] = """[await]"""
dialog_data[908] = """[await]"""
dialog_data[909] = """[await]"""
dialog_data[910] = """[await]"""
dialog_data[911] = """[await]"""
dialog_data[
    912
] = """  You will enter combat against me?[await]\n  [select] (Yes)\n  [select] (Uh...)[await]"""
dialog_data[913] = """[await]"""
dialog_data[914] = """[await]"""
dialog_data[915] = """[await]"""
dialog_data[916] = """[await]"""
dialog_data[
    917
] = """TOADOFSKY:
           Okay, here we go![await][page]
 Listen to this, it’s our song![await]"""
dialog_data[
    918
] = """TOADOFSKY: So what do you think?[await]
  [select] (Let’s hear it again)
  [select] (I want to recompose it)
  [select] (Never mind)[await]"""
dialog_data[919] = """ All right, let’s hear it again.[await]"""
dialog_data[
    920
] = """ To recompose the song, just jump
 on the froggies.[await]"""
dialog_data[921] = """[await]"""
