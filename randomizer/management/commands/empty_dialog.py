...
Usage:
    manage.py empty_dialog 803                    # Single dialog
    manage.py empty_dialog DI0803_NO_ADVICE       # By name
    manage.py empty_dialog 800-810                # Range (inclusive)
    manage.py empty_dialog 800 801 802            # Multiple
    manage.py empty_dialog 800-805 810 815-820    # Mixed
"""
...
def main(args: list[str]):
    dialog_ids = parse_dialog_ids(args)

    if not dialog_ids:
        raise CommandError("No valid dialog IDs provided.")
...
class Command(BaseCommand):
    help = "Empty dialog(s) by ID and rename to DIXXXX_EMPTY."

    def add_arguments(self, parser):
        parser.add_argument("dialog_ids", nargs="+", metavar="DIALOG_ID",
                            help="Dialog numbers (803), names (DI0803_NO_ADVICE), "
                                 "or inclusive ranges (800-810). Mix freely.")

    def handle(self, *args, **options):
        main(options["dialog_ids"])
