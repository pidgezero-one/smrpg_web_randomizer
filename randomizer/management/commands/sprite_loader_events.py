...
# room 496 R496_FACTORY_GROUNDS_FIGHT_WITH_SMITHY_USES_SLEDGE: BFS ties stub
# 858 (INNER_FACTORY_4TH_ROOM) against stub 815 (DOJO) at depth 2. Both are
# false claims. The 815 path runs through a stray identifier inside
# script_3797.py, `identifier="EVENT_2064_action_queue_11"` -- leftover
# naming from a copy-paste that happens to embed script 2064's number and is
# never itself a jump target, not a real reference to event 2064. The 858
# path is real (script_3797.py conditionally does
...
# which reaches it directly (one hop) from its own entrance_event. Room 496
# has no stub of its own.
#
