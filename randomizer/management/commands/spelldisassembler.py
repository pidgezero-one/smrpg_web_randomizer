...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for index in range(0, 128):
            ...
            writeline(file, "    overworldUsable = %r" % ((tmp & 0x80) == 0x80))

            offset += 1

            tmp = rom[offset]
            spelltype = (tmp & 0x01) == 0x01
            writeline(file, "    spell_type = %s" % ("SpellType.Damage" if not spelltype else "SpellType.Heal"))
            effecttype = tmp & 0x06
            if effecttype == 2:
            ...
            el = tmp & 0xF0
            if el == 0x10:
                writeline(file, "    element = SpellElement.Ice")
            elif el == 0x20:
                writeline(file, "    element = SpellElement.Thunder")
            elif el == 0x40:
                writeline(file, "    element = SpellElement.Fire")
            elif el == 0x80:
                writeline(file, "    element = SpellElement.Earth")
            else:
