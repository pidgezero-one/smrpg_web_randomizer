...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        for i, room in enumerate(rooms[:510]):
            ...
            room_output += (
                "\nfrom randomizer.entities.rooms.rooms.room_%i.room_%i_partition import partition"
                % (i, i)
            )
            if len(room.exit_fields) > 0:
                room_output += (
                    "\nfrom randomizer.entities.rooms.rooms.room_%i.room_%i_exits import exits"
                    % (i, i)
                )
            if len(room.event_tiles) > 0:
                room_output += (
                    "\nfrom randomizer.entities.rooms.rooms.room_%i.room_%i_events import events"
                    % (i, i)
                )
            if len(room.objects) > 0:
                room_output += (
                    "\nfrom randomizer.entities.rooms.rooms.room_%i.room_%i_objects import objects"
                    % (i, i)
                )
            room_output += "\n\nroom = Room("
            room_output += "\n    partition=partition,"
