...
jmp_cmds = [0x09, 0x10, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x38, 0x39, 0x50, 0x51, 0x5D, 0x64]
        ...
        for key, bank in banks.items():
            ...
            #        writeline(file, '    [')
                    ...
                    for c in script_content:
                        ...
                        addr_ = ptrs[i]+offset+offset_2
                        addr = hex(addr_)
            #            writeline(file, '''        {'id': 'command_%s', 'addr': %s, 'data': [%s]},''' % (addr, addr, byte_string))
                        this_script.append({'id': 'command_%s' % addr, 'addr': addr_, "data": list(c[0])})
                        offset_2 += len(c[0])
            ...
            #    writeline(file, '    [')
                ...
                for c in script_content:
                    ...
                    addr = hex(addr_)
            #        writeline(file, '''        {'id': 'command_%s', 'addr': %s, 'data': [%s], 'length': %i},''' % (addr, addr, byte_string, len(c[0])))
                    this_script.append({'id': 'command_%s' % addr, 'addr': addr_, "data": list(c[0]), "length": len(c[0])})
                    offset_2 += len(c[0])
                scripts.append(script_content)
            #    writeline(file, '    ]')
                data.append(this_script)
            ...
            for index, script in enumerate(data):
                for cmd_index, command in enumerate(script):
                    if command["data"][0] in jmp_cmds or command["data"][0] in jmp_cmds_1:
                        ...
                        dest = (command["addr"] & 0xFF0000) + (address[1] << 8) + address[0]
                        # print(hex(dest), len(data))

                        found = None
                        for search_index, search_script in enumerate(data):
                            for search_cmd_index, search_command in enumerate(search_script):
                        ...
                        data[index][cmd_index]["data"] = command["data"]
            ...
            for index, script in enumerate(data):
                ...
                for cmd in script:
                    print(cmd)
                    byte_string = [string_byte(b) for b in cmd["data"]]
                    byte_string = ", ".join(byte_string)
                    writeline(file, '''        {'id': '%s', 'data': [%s]},''' % (cmd["id"], byte_string))
