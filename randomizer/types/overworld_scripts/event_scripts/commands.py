...
from randomizer.types.overworld_scripts.constants.coords import COORD_F
...
from randomizer.types.overworld_scripts.variables.classes import ByteVar, Flag, ShortVar
...
class StopAllBackgroundEvents(EventScriptCommandNoArgs):
...
class ReturnFD(EventScriptCommandNoArgs):
...
class JmpIf316DIs3(EventScriptCommandWithJmps):
    _opcode: int = 0x41
    _size: int = 3
...
class SetVarToConst(EventScriptCommand):
    ...
    def render(self) -> bytearray:
        ...
        elif self.address == PRIMARY_TEMP_7000:
...
class CopyVarToVar(EventScriptCommand):
    ...
    def set_addresses(self, from_var=None, to_var=None):
        ...
        if self.from_var != PRIMARY_TEMP_7000 and self.to_var != PRIMARY_TEMP_7000:
            self._size = 3
        else:
            self._size = 2
    ...
    def render(self) -> bytearray:
        ...
        elif self.from_var == PRIMARY_TEMP_7000 and isinstance(self.to_var, ByteVar):
            return super().render(0xB5, self.to_var)
        elif self.to_var == PRIMARY_TEMP_7000 and isinstance(self.from_var, ShortVar):
            return super().render(0xBA, self.from_var)
        elif self.from_var == PRIMARY_TEMP_7000 and isinstance(self.to_var, ShortVar):
...
class _ActionQueue(ActionQueuePrototype):
    pass


class ActionQueueAsync(_ActionQueue):
    def __init__(
        self,
        target: AreaObject,
        subscript: Sequence[ActionScriptCommand] = [],
        identifier: Optional[str] = None,
    ) -> None:
        super().__init__(target, False, subscript, identifier)


class ActionQueueSync(_ActionQueue):
    def __init__(
        self,
        target: AreaObject,
        subscript: Sequence[ActionScriptCommand] = [],
...
class _StartEmbeddedActionScript(StartEmbeddedActionScriptPrototype):
    pass


class StartAsyncEmbeddedActionScript(_StartEmbeddedActionScript):
    def __init__(
        self,
        target: AreaObject,
        prefix: int,
...
class StartSyncEmbeddedActionScript(_StartEmbeddedActionScript):
    def __init__(
        self,
        target: AreaObject,
        prefix: int,
...
class _SetActionScript(EventScriptCommand):
    _size: int = 4
    _target: AreaObject
    _sync: bool
...
class SetAsyncActionScript(_SetActionScript):
    def __init__(
        self,
        target: AreaObject,
        action_script_id: int,
...
class SetSyncActionScript(_SetActionScript):
    def __init__(
        self,
        target: AreaObject,
        action_script_id: int,
...
class SetTempAsyncActionScript(_SetTempActionScript):
    def __init__(
        self,
        target: AreaObject,
        action_script_id: int,
...
class SetTempSyncActionScript(_SetTempActionScript):
    def __init__(
        self,
        target: AreaObject,
        action_script_id: int,
...
class EnableObjectTriggerInSpecificLevel(EventScriptCommand):
    _opcode: int = 0xF3
    _size: int = 3
    _object: AreaObject
    _level_id: UInt16
    ...
    def render(self) -> bytearray:
        arg_raw = self.level_id + (self.object << 9) + (1 << 15)
        arg: int = UInt16(arg_raw)
        return super().render(arg)
...
class CreatePacketAtObjectCoords(EventScriptCommandWithJmps):
    _opcode: int = 0x3E
    _size: int = 5
    _packet_id: UInt8
    _object: AreaObject
...
class UnfreezeCamera(EventScriptCommandNoArgs):
...
class Set7000ToObjectCoord(EventScriptCommand):
    _size: int = 2
    _object: AreaObject
    _coord: Coord
...
class EnableControls(EventScriptCommand):
    ...
    def set_enabled_buttons(self, enabled_buttons: List[ControllerInput]) -> None:
        self._enabled_buttons = set(enabled_buttons)

    def __init__(
        self,
        enabled_buttons: List[ControllerInput] = [],
        identifier: Optional[str] = None,
    ):
        super().__init__(identifier)
...
class EnableControlsUntilReturn(EventScriptCommand):
    ...
    def __init__(
        self,
        enabled_buttons: "set[ControllerInput]" = set(),
        identifier: Optional[str] = None,
    ):
        super().__init__(identifier)
        self.set_enabled_buttons(enabled_buttons)
...
class AddToInventory(EventScriptCommand):
    ...
    def set_item(self, item: Union[Type[Item], int]) -> None:
    ...
    def __init__(
        self, item: Union[Type[Item], int], identifier: Optional[str] = None
    ) -> None:
        super().__init__(identifier)
        self.set_item(item)
    ...
    def render(self) -> bytearray:
        if isinstance(self.item, ByteVar):
            ...
            return super().render(0x50, self.item().item_id)
...
class StoreItemAt70A7QuantityTo7000(EventScriptCommandNoArgs):
...
class StoreCharacterEquipmentTo7000(EventScriptCommandNoArgs):
    ...
    def set_equip_slot(self, equip_slot: Type[Equipment]) -> None:
        assert equip_slot == Weapon or equip_slot == Armor or equip_slot == Accessory
...
class PrioritySet(EventScriptCommand):
    _opcode: int = 0x81
    _size: int = 4
    _mainscreen: List[Layer]
    _subscreen: List[Layer]
    _colour_math: List[Layer]

    @property
    def mainscreen(self) -> List[Layer]:
        return self._mainscreen

    def set_mainscreen(self, mainscreen: List[Layer]) -> None:
        for layer in mainscreen:
            assert 0 <= layer <= 2 or layer == 4
        self._mainscreen = mainscreen

    @property
    def subscreen(self) -> List[Layer]:
        return self._subscreen

    def set_subscreen(self, subscreen: List[Layer]) -> None:
        for layer in subscreen:
            assert 0 <= layer <= 2 or layer == 4
        self._subscreen = subscreen

    @property
    def colour_math(self) -> List[Layer]:
        return self._colour_math

    def set_colour_math(self, colour_math: List[Layer]) -> None:
    ...
    def __init__(
        self,
        mainscreen: List[Layer],
        subscreen: List[Layer],
        colour_math: List[Layer],
        identifier: Optional[str] = None,
    ) -> None:
        super().__init__(identifier)
