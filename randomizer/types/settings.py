...
class CategorizationFlag(Flag):
    ...
    _options: dict[CategorizationOption, bool] = {}
    _default: dict[CategorizationOption, bool] = {}
    ...
    def enable(self, option: CategorizationOption):
        if option not in self.options:
            raise FlagError(f"Option {option} is not a valid option for this flag.")
        self.options[option] = True

    def disable(self, option: CategorizationOption):
        if option not in self.options:
            raise FlagError(f"Option {option} is not a valid option for this flag.")
        self.options[option] = False

    @property
    def default(self) -> dict[CategorizationOption, bool]:
        return self._default
    
    @property
    def options(self) -> dict[CategorizationOption, bool]:
        return self._options or self.default.copy()
    
    def reset(self):
        self._options = self.default.copy()

class CategorizationFlagWithOrdinance(Flag):
    """For thing like choosing your starting party where order matters"""

    type = FlagType.CATEGORIZATION_WITH_ORDINANCE
    _options: dict[CategorizationOption, Optional[int]] = {}
    _default: dict[CategorizationOption, Optional[int]] = {}

    @property
    def default(self) -> dict[CategorizationOption, Optional[int]]:
        return self._default
    
    @property
    def options(self) -> dict[CategorizationOption, Optional[int]]:
        return self._options or self.default.copy()
...
class SelectOneFlag(Flag):
    ...
    choices: list[CategorizationOption] = []
    _selected: CategorizationOption
    _default: CategorizationOption

    @property
    def selected(self) -> CategorizationOption:
        return self._selected or self._default
    
    def select(self, choice: CategorizationOption):
        if choice not in self.choices:
            raise FlagError(f"Choice {choice} is not a valid option for this flag.")
        self._selected = choice

    @property
    def default(self) -> CategorizationOption:
        return self._default
...
class StartingCharacters(CategorizationFlagWithOrdinance):
    _name = "Starting Characters"
    _options: dict[CategorizationOption, Optional[int]] = {}
    _default: dict[CategorizationOption, Optional[int]] = {}
    # TODO disassemble allies and set default as just randomchar = 1
...
class KeroSewersGate(SelectOneFlag):
...
class BoosterHillGate(SelectOneFlag):
...
class LandsEndGate(SelectOneFlag):
...
class NimbusGate(SelectOneFlag):
...
<br>Selecting a remake-specific boss will do nothing if the remake flag is not enabled."""
    _id = "pool"
    # TODO form categorization enum from boss fight objects
    # TODO include remake
...
<br>
<br><bold>Numbers only</bold>: Attack, defense, speed, and HP values are randomized. Elemental weaknesses/resistances and status immunities remain unchanged.
<br>
...
class FastText(BooleanFlag):
    _name = "Fast text"
    _description = "Dialog text will render a bit faster."
    _id = "fasttext"
