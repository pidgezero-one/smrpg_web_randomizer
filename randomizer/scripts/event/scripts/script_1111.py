...
script = EventScript(
    [
        ...
        JmpIfVarEqualsConst(
            PRIMARY_TEMP_7000, 1, ["EVENT_1111_remove_one_from_inventory_10"]
        ),
        StoreItemAmountTo7000(CricketJam),
        JmpIfVarEqualsConst(
            PRIMARY_TEMP_7000, 1, ["EVENT_1111_check_if_pie_exchanged"]
        ),
        JmpToEvent(
            E0947_FROGFUCIUS_HINT_MAIN_CHECKS, identifier="EVENT_1111_run_dialog_6"
        ),
        Return(),
        RemoveOneOfItemFromInventory(
            CricketPie, identifier="EVENT_1111_remove_one_from_inventory_10"
        ),
