"""A0488_FOREST_MAZE_AREA_RECRUITABLE_CHARACTER"""

from randomizer.scripts.action.script_imports import *

script = ActionScript(
    [
        JmpIfBitSet(FOREST_LIBERATED, ["ACTION_488_clear_bit_10"]),
        JmpIfBitSet(DIRECTIONAL_7045_5, ["ACTION_488_clear_bit_10"]),
        JmpIfBitSet(DIRECTIONAL_7045_7, ["ACTION_488_clear_bit_10"]),
        JmpIfBitSet(DIRECTIONAL_7046_0, ["ACTION_488_clear_bit_10"]),
        JmpIfBitSet(DIRECTIONAL_7045_0, ["ACTION_488_shift_to_xy_coords_24"]),
        JmpIfBitSet(DIRECTIONAL_7045_1, ["ACTION_488_shift_to_xy_coords_36"]),
        JmpIfBitSet(DIRECTIONAL_7045_2, ["ACTION_488_clear_bit_10"]),
        JmpIfBitSet(DIRECTIONAL_7045_3, ["ACTION_488_shift_to_xy_coords_48"]),
        JmpIfBitSet(DIRECTIONAL_7045_4, ["ACTION_488_shift_to_xy_coords_60"]),
        JmpIfBitSet(DIRECTIONAL_7046_1, ["ACTION_488_shift_to_xy_coords_13"]),
        ClearBit(DIRECTIONAL_7045_5, identifier="ACTION_488_clear_bit_10"),
        VisibilityOff(),
        Return(),
        ShiftToXYCoords(x=8, y=61, identifier="ACTION_488_shift_to_xy_coords_13"),
        SetSpriteSequence(index=8, is_sequence=True, looping=True, mirror_sprite=True),
        Pause(2, identifier="ACTION_488_pause_15"),
        JmpIfObjectWithinRangeSameZ(
            comparing_npc=MARIO,
            usually=0,
            tiles=5,
            destinations=["ACTION_488_set_animation_speed_18"]),
        Jmp(["ACTION_488_pause_15"]),
        SetSequenceSpeed(FAST, identifier="ACTION_488_set_animation_speed_18"),
        SetSpriteSequence(index=0, is_sequence=True, looping=True, mirror_sprite=True),
        WalkSoutheastSteps(2),
        WalkSoutheastPixels(8),
        VisibilityOff(),
        Return(),
        ShiftToXYCoords(x=8, y=47, identifier="ACTION_488_shift_to_xy_coords_24"),
        WalkSoutheastPixels(8),
        SetSpriteSequence(index=9, is_sequence=True, looping=True, mirror_sprite=True),
        Pause(2, identifier="ACTION_488_pause_27"),
        JmpIfObjectWithinRangeSameZ(
            comparing_npc=MARIO,
            usually=0,
            tiles=5,
            destinations=["ACTION_488_set_animation_speed_30"]),
        Jmp(["ACTION_488_pause_27"]),
        SetSequenceSpeed(FAST, identifier="ACTION_488_set_animation_speed_30"),
        SetSpriteSequence(index=1, is_sequence=True, looping=True, mirror_sprite=True),
        WalkNortheastSteps(2),
        WalkNortheastPixels(8),
        VisibilityOff(),
        Return(),
        ShiftToXYCoords(x=8, y=47, identifier="ACTION_488_shift_to_xy_coords_36"),
        WalkSoutheastPixels(8),
        SetSpriteSequence(index=6, is_sequence=True, looping=True, mirror_sprite=True),
        Pause(2, identifier="ACTION_488_pause_39"),
        JmpIfObjectWithinRangeSameZ(
            comparing_npc=MARIO,
            usually=0,
            tiles=5,
            destinations=["ACTION_488_set_animation_speed_42"]),
        Jmp(["ACTION_488_pause_39"]),
        SetSequenceSpeed(FAST, identifier="ACTION_488_set_animation_speed_42"),
        SetSpriteSequence(index=1, is_sequence=True, looping=True, mirror_sprite=True),
        WalkNortheastSteps(2),
        WalkNortheastPixels(8),
        VisibilityOff(),
        Return(),
        ShiftToXYCoords(x=8, y=47, identifier="ACTION_488_shift_to_xy_coords_48"),
        WalkSoutheastPixels(8),
        SetSpriteSequence(index=9, is_sequence=True, looping=True, mirror_sprite=True),
        Pause(2, identifier="ACTION_488_pause_51"),
        JmpIfObjectWithinRangeSameZ(
            comparing_npc=MARIO,
            usually=0,
            tiles=5,
            destinations=["ACTION_488_set_animation_speed_54"]),
        Jmp(["ACTION_488_pause_51"]),
        SetSequenceSpeed(FAST, identifier="ACTION_488_set_animation_speed_54"),
        SetSpriteSequence(index=1, is_sequence=True, looping=True, mirror_sprite=True),
        WalkNortheastSteps(2),
        WalkNortheastPixels(8),
        VisibilityOff(),
        Return(),
        ShiftToXYCoords(x=3, y=47, identifier="ACTION_488_shift_to_xy_coords_60"),
        SetSequenceSpeed(FAST),
        SetSpriteSequence(
            index=6, sprite_offset=1, is_sequence=True, looping=True, mirror_sprite=True
        ),
        Pause(52),
        SetSequenceSpeed(NORMAL),
        SetSpriteSequence(index=3, sprite_offset=1, looping=False, mirror_sprite=True),
        Pause(16),
        SetSequenceSpeed(FAST),
        SetSpriteSequence(index=1, is_sequence=True, looping=True),
        WalkNorthwestSteps(2),
        VisibilityOff(),
        Return(),
    ]
)
