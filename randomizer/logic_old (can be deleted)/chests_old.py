...
def randomize_all(world):
                ...
                if enemy.overworld_sprite is not None:
                    ...
                    if shuffled_boss.name is "CountDown":
                        ...
                        forced_coins = [
                            ...
                            if isinstance(chest, chests.BanditsWayCroco)
                        ]
    ...
    if world.open_mode:
            ...
            for chest in world.chest_locations:
                ...
                for i in world.items:
                    if chest.item.index == i.index:
                        tiered_item = i
                if (
                    (chest.item in coins and not coins_allowed)
                    or (chest.item in stars and not stars_allowed)
                    or (chest.item == items.Flower and not flowers_allowed)
                    or (chest.item == items.RecoveryMushroom and not mushrooms_allowed)
                    or (chest.item == items.FrogCoin and not frogcoins_allowed)
                    or (tiered_item and tiered_item.hard_tier > tiers_allowed)
                ):
        ...
        # Empty chests.
            ...
            flags.ChestShuffleBiased
        ) or world.settings.is_flag_enabled(flags.ChestShuffleChaos):
            finished_chests = []
            ...
            ratio_coins = len(
                [
                    chest
                    for chest in world.chest_locations
                    if not isinstance(chest, chests.Reward) and chest.item in coins
                ]
            )
            ratio_frogcoins = (
                len(
                    [
                        chest
                        for chest in world.chest_locations
                        if not isinstance(chest, chests.Reward)
                        and chest.item == items.FrogCoin
                    ]
                )
                - 2
            )
            ratio_mushrooms = len(
                [
                    ...
                    for chest in world.chest_locations
                    if not isinstance(chest, chests.Reward)
                    and chest.item == items.RecoveryMushroom
                ]
            )
            ratio_flowers = (
                len(
            ...
            ratio_stars = len(
                [
                    chest
                    for chest in world.chest_locations
                    if not isinstance(chest, chests.Reward) and chest.item in stars
                ]
            )
            ratio_items = len(
                [
                    ...
                    for chest in world.chest_locations
                    if not isinstance(chest, chests.Reward)
                    and chest.item not in coins
                    and chest.item not in stars
                    and chest.item
                    not in [
                        items.FrogCoin,
                        items.RecoveryMushroom,
                        items.Flower,
                        items.YouMissed,
                    ]
                ]
            ...
            if stars_allowed:
                if world.settings.is_flag_enabled(flags.ChestRandomizeStars):
                    eligible_chests = [
                        chest
                        for chest in world.chest_locations
                        if chest.item_allowed(items.BanditsWayStar)
                    ]
                    ...
                    while len(finished_chests) < num_stars:
                        ...
                        if biased:
                            chest.item = random.choice(
                                [
                                    star
                                    for star in stars
                                    if star.hard_tier == chest.access
                                ]
                            )
                        else:
                        ...
                        if isinstance(chest, chests.BanditsWayStarChest) or isinstance(
                            chest, chests.BanditsWayDogJump
                        ):
                            for c in eligible_chests:
                                if isinstance(
                                    c, chests.BanditsWayStarChest
                                ) or isinstance(c, chests.BanditsWayDogJump):
            ...
            if world.settings.is_flag_enabled(flags.MonstroTownLite):
                ...
                monstro_locations = [
                    i
                    for i in world.chest_locations
                    if isinstance(
                        i,
                        (
                            chests.CulexReward,
            ...
            if chance <= 3 and len(monstro) > 0:
                ...
                item = monstro[1]
                location = [
                    i for i in monstro_locations if isinstance(i, chests.SuperJumps100)
                ][0]
                location.item = item
                monstro.remove(item)
            ...
            if include_key_items:
                key_item_locations = [
                    l
                    for l in world.key_locations
                    if keys.item_location_filter(world, l)
                ]

                # Get items to place only from vanilla key item locations, not including other chests/rewards.
                required_items = keys.Inventory(
                    [
                        l.item
                        for l in key_item_locations
                        if l.item.shuffle_type == items.ItemShuffleType.Required
                    ]
                )
                extra_items = keys.Inventory(
                    [
                        l.item
                        for l in key_item_locations
                        if l.item.shuffle_type == items.ItemShuffleType.Extra
                    ]
                )
                ...
                if not world.settings.is_flag_enabled(flags.ChestExcludeRewards):
                    chest_locations = [
                        ...
                        if l not in finished_chests
                        and keys.item_location_filter(world, l)
                    ]
                else:
                    chest_locations = [
                        l
                        ...
                        if l not in finished_chests
                        and keys.item_location_filter(world, l)
                        and not isinstance(l, chests.Reward)
                    ]
            ...
            if not world.settings.is_flag_enabled(flags.ChestExcludeRewards):
                eligible_wallet_locations = [
                    chest
                    for chest in chests_plus_leftovers
                    if chest not in finished_chests
                ]
            ...
            eligible_empty_locations = [
                chest
                for chest in chests_plus_leftovers
                if chest not in finished_chests
                and not isinstance(chest, chests.Reward)
                and chest.item_allowed(items.YouMissed)
            ]
            ...
            eligible_chests = [
                ...
                for chest in chests_plus_leftovers
                if not isinstance(
                    chest, (chests.Reward, chests.BowserDoorReward, KeyItemLocation)
                )
                and chest not in finished_chests
            ]
            eligible_rewards = [
                chest
                for chest in chests_plus_leftovers
            ...
            eligible_items = [
                i
                for i in world.items
                if i.index not in excluded_items
                and not i.is_key
            ...
            while len(eligible_chests) > 0:
                ...
                if biased:
                    ...
                    adjusted_denominator += (
                        ...
                        + adjusted_ratio_flowers
                        + adjusted_ratio_mushrooms
                        + adjusted_ratio_frogcoins
                    )
                    selection = random.randint(1, adjusted_denominator)
                    if (
                        flowers_allowed
                        and chest.item_allowed(items.Flower)
                        and selection < adjusted_ratio_flowers
                    ):
                        chest.item = items.Flower
                    elif (
                        mushrooms_allowed
                        and chest.item_allowed(items.RecoveryMushroom)
                        and selection
                        < adjusted_ratio_flowers + adjusted_ratio_mushrooms
                    ):
                        chest.item = items.RecoveryMushroom
                    elif (
                        frogcoins_allowed
                        and chest.item_allowed(items.FrogCoin)
                        and selection
                        < adjusted_ratio_flowers
                        + adjusted_ratio_mushrooms
                        + adjusted_ratio_frogcoins
                        ...
                        coins_allowed
                        and selected_tier <= 2
                        and chest.item_allowed(items.Coins150)
                        and selection
                        < adjusted_ratio_flowers
                        + adjusted_ratio_mushrooms
                        + adjusted_ratio_frogcoins
                        ...
                        while not proceed_repeat_item:
                            ...
                            possible_items = [
                                i
                                for i in items_for_chest
                                if i.hard_tier == selected_tier
                            ]
                    ...
                    if (
                        flowers_allowed
                        and chest.item_allowed(items.Flower)
                        and selection < ratio_flowers / 1.5
                    ):
                        chest.item = items.Flower
                    elif (
                        mushrooms_allowed
                        and chest.item_allowed(items.RecoveryMushroom)
                        and selection < ratio_flowers / 1.5 + ratio_mushrooms / 1.5
                    ):
                        chest.item = items.RecoveryMushroom
                    elif (
                        frogcoins_allowed
                ...
                while len(eligible_rewards) > 0:
                    ...
                    if isinstance(chest, chests.CricketJamReward):
                        ...
                        while not proceed_repeat_item:
                            ...
                            if (
                                check_item not in items_already_in_chests
                                or not check_item.is_equipment
                            ):
        ...
        if world.settings.is_flag_enabled(flags.ReplaceItems):
            ...
            for chest in [
                i for i in world.chest_locations if not isinstance(i, chests.Reward)
            ]:
                if (
                    chest.item.hard_tier == 1
                    and not chest.item.is_key
                    and chest.item.price > 0
                ):
                    if (
                        chest.item_allowed(items.Coins150)
                        and not chest.item.frog_coin_item
                    ):
                        chest.item = closest_coins(chest.item.price)
                    elif (
                        chest.item_allowed(items.FrogCoin) and chest.item.frog_coin_item
                    ):
                        chest.item = items.FrogCoin
