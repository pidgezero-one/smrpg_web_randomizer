...
def _randomize_item(item, safetychecks_on=True):
    ...
    if item.world.settings.is_flag_value(
        flags.EquipmentProperties, EquipmentPropertiesOptions.RANDOM
    ):
        ...
        if random.randint(1, 3) == 1:
            ...
            num_down = random.choices([0, 1, 2, 3, 4, 5], weights=[1, 5, 10, 10, 5, 1])[
                0
            ]
        ...
        for attr in up_vals:
            setattr(
                item, attr, utils.mutate_normal(up_vals[attr], minimum=1, maximum=127)
            )
        ...
        if odds > 0:
            ...
            if item.effect_type in [
                items.EffectType.Buffs,
                items.EffectType.ElementalImmunity,
            ]:
                KO_odds_factor *= 1.5
            ...
            if item.effect_type in [
                items.EffectType.Normal,
                items.EffectType.Buffs,
                items.EffectType.StatusProtection,
            ]:
                ...
                items.EffectType.FewEffects,
                items.EffectType.ElementalResistance,
            ]:
            ...
            if safetychecks_on and isinstance(
                item,
                (
                    items.FearlessPin,
                    items.AntidotePin,
            ...
            for i in range(0, 7):
                ...
                if i == 4 and not item.world.settings.is_flag_enabled(
    ...
    elif item.world.settings.is_flag_value(
        flags.EquipmentProperties, EquipmentPropertiesOptions.some
    ):
    ...
    if not item.world.settings.is_flag_value(
        flags.EquipmentCharacters, EquipmentCharactersOptions.vanilla
    ):
        ...
        if item.world.open_mode or (not item.is_weapon or Geno not in item.equip_chars):
            ...
            if item.world.settings.is_flag_value(
                flags.EquipmentCharacters, EquipmentCharactersOptions.equip_all
            ):
                item.equip_chars = list({Mario, Mallow, Geno, Bowser, Peach})
            elif item.world.settings.is_flag_value(
                flags.EquipmentCharacters, EquipmentCharactersOptions.RANDOM
            ) or (
                ...
                for _ in range(num_equippable):
                    ...
                    if (
                        not item.world.open_mode
                        and item.is_weapon
                        and Geno in char_choices
                    ):
...
def randomize_all(world):
        ...
        if not item.world.settings.is_flag_enabled(flags.EquipmentNoSafety):
            ...
            if instant_ko_items < 4:
                top_armor = [
                    item
                    for item in world.items
                    if (item.is_armor or item.is_accessory)
                    and item.tier == 1
                    and not item.prevent_ko
                ]
    ...
    for item in world.items:
        if item.is_equipment:
            ...
            item.rank_order_reverse = (
                ranks_reverse.index(item) + 1 if item in ranks_reverse else 0
            )
