    ...
    if henchman.pack_number is not None:
        ...
        for f in henchman_pack.formations:
            ...
            if utils.isclass_or_instance(
                boss, bosses.MackBoss
            ) or utils.isclass_or_instance(boss, bosses.BoosterBoss):
...
def randomize_all(world):
    ...
    if world.open_mode:
        ...
        if world.settings.is_flag_enabled(flags.BossShuffle):
            ...
            shuffled_bosses = [
                l.item.related_class for l in world.boss_fight_placements
            ]

            original_bosses = [
                l.original_item.related_class for l in world.boss_fight_placements
            ]
            stat_inheritance = copy.deepcopy(original_bosses)

            if world.settings.is_flag_value(
                flags.BossShuffleScaleStats, BossScaleOptions.RANDOM
            ):
            ...
            for location, boss, original in zip(
                locations, shuffled_bosses, original_bosses
            ):
                if utils.isclass_or_instance(location, bosses.Birdetta):
                    pack = world.get_formation_pack_by_index(boss.pack_number)
                    target_formation = world.get_enemy_formation_by_index(
                        pack.formations[0].index
                    )
                    ...
                    if utils.isclass_or_instance(
                        ...
                        target_formation_birdetta.required_battlefield = (
                            bosses.Battlefields.Birdo
                        )
                        ...
                        (
                            ...
                            bosses.GrateGuyBoss,
                            bosses.JohnnyBoss,
                            bosses.YaridovichBoss,
                            bosses.Belome2Boss,
                            bosses.CulexBoss,
                            bosses.DodoBoss,
                            bosses.ValentinaBoss,
                            bosses.CzarBoss,
                            bosses.ChesterBoss,
                            bosses.MagikoopaBoss,
                            bosses.BoomerBoss,
                            bosses.ClerkBoss,
                            bosses.ManagerBoss,
                            bosses.DirectorBoss,
                        ...
                        target_formation.required_battlefield = (
                            bosses.Battlefields.NimbusCastle
                        )
                    else:
                        exclude_shelly = True
                        ...
                        for index, member in enumerate(target_formation.members):
                            if member.enemy.sprite is not None:
                                ...
                                world.get_enemy_instance(
                                    enemies.EmptyEnemy
                                ).sprite_sub = True
                                world.get_enemy_instance(
                                    enemies.EmptyEnemy
                                ).formation_id = pack.formations[0].index
                            if not member.hidden_at_start:
                                summons.append(0x28 + member.index)
                                target_formation.members[index].hidden_at_start = True
                        world.get_enemy_instance(enemies.Shelly).position = len(
                            target_formation.members
                        )
                        world.get_enemy_instance(
                            enemies.Shelly
                        ).vanilla = utils.isclass_or_instance(boss, bosses.BirdettaBoss)
                        target_formation.members.append(
                            FormationMember(
                                len(target_formation.members),
                                False,
                        ...
                        world.get_enemy_instance(
                            enemies.Shelly
                        ).summon_event = target_formation.event_at_start
            ...
            if not world.settings.is_flag_value(
                flags.BossShuffleScaleStats, BossScaleOptions.vanilla
            ):
                ...
                for location in locations:
                    ...
                    formation = world.get_enemy_formation_by_index(
                        pack.formations[0].index
                    )
                    elist = [
                        e
                        for e in formation.stat_total_enemies
                        if not (
                            utils.isclass_or_instance(e, enemies.Shelly)
                            and exclude_shelly
                        )
                    ]
                    # HP
                    ...
                    if any(
                        e for e in elist if utils.isclass_or_instance(e, enemies.Exor)
                    ):
                        hp = 0
                        eyes = 0
                        for e in elist:
                            if utils.isclass_or_instance(e, enemies.Exor):
                                ...
                                e, (enemies.LeftEye, enemies.RightEye)
                            ):
                                eyes += e.hp
                        hp += int(eyes / 2)
                        xp = sum(
                    ...
                    # For Cloaker/Domino, count average HP of each phase of the fight.
                        ...
                        e
                        for e in elist
                        if utils.isclass_or_instance(e, enemies.Cloaker)
                    ):
                        dudes = 0
                        sneks = 0
                        for e in elist:
                            if utils.isclass_or_instance(
                                e, (enemies.Cloaker, enemies.Domino)
                            ):
                                dudes += e.hp
                            elif utils.isclass_or_instance(
                                e, (enemies.Earthlink, enemies.MadAdder)
                            ):
                                sneks += e.hp
                    ...
                    # For Dodo/Valentina, count 40% of Dodo's HP.
                        ...
                        e
                        for e in elist
                        if utils.isclass_or_instance(e, enemies.Valentina)
                    ):
                        dodo = 0
                        valentina = 0
                        for e in elist:
                    ...
                    # For King Calimari, need special exp calc
                        ...
                        e
                        for e in elist
                        if utils.isclass_or_instance(e, enemies.KingCalamari)
                    ):
                    ...
                    # For Megasmilax, need special exp calc
                        ...
                        e
                        for e in elist
                        if utils.isclass_or_instance(e, enemies.Megasmilax)
                    ):
                        hp = sum(e.hp for e in elist)
                        xp = sum(
                            e.xp
                    ...
                    # For Axems, need special exp calc
                        ...
                        e
                        for e in elist
                        if utils.isclass_or_instance(e, enemies.AxemRangers)
                    ):
                        hp = sum(e.hp for e in elist)
                        xp = sum(
                            e.xp
                    ...
                    # For Belome 2, need special exp calc
                        ...
                        e
                        for e in elist
                        if utils.isclass_or_instance(e, enemies.Belome2)
                    ):
                        hp = sum(e.hp for e in elist)
                        xp = sum(
                            e.xp
                            ...
                            if utils.isclass_or_instance(
                                e, (enemies.Belome2, enemies.MarioClone)
                            )
                        )
                        coins = sum(
                            e.coins
                    ...
                    # For Culex, need special exp calc
                        ...
                        e for e in elist if utils.isclass_or_instance(e, enemies.Culex)
                    ):
                        hp = sum(e.hp for e in elist)
                        xp = sum(
                            e.xp
                    ...
                    # For Johnny, need special exp calc
                        ...
                        e for e in elist if utils.isclass_or_instance(e, enemies.Johnny)
                    ):
                        hp = sum(e.hp for e in elist)
                        xp = 0
                        coins = 0
                    ...
                    if any(
                        e
                        for e in elist
                        if utils.isclass_or_instance(e, enemies.Smithy1)
                    ):
                        attack = 0
                        defense = 0
                        magic_attack = 0
                        ...
                        attack = (
                            (attack / 4)
                            + world.get_enemy_instance(enemies.Smithy1).attack
                        ) / 2
                        defense = (
                            (defense / 4)
                            + world.get_enemy_instance(enemies.Smithy1).defense
                        ) / 2
                        magic_attack = (
                            (magic_attack / 4)
                            + world.get_enemy_instance(enemies.Smithy1).magic_attack
                        ) / 2
                        magic_defense = (
                            (magic_defense / 4)
                            + world.get_enemy_instance(enemies.Smithy1).magic_defense
                        ) / 2
                        evade = int(round(statistics.mean(e.evade for e in elist)))
                        magic_evade = int(
                            round(statistics.mean(e.magic_evade for e in elist))
                        )
                        ...
                        magic_attack = int(
                            round(statistics.mean(e.magic_attack for e in elist))
                        )
                        magic_defense = int(
                            round(statistics.mean(e.magic_defense for e in elist))
                        )
                        evade = int(round(statistics.mean(e.evade for e in elist)))
                        magic_evade = int(
                            round(statistics.mean(e.magic_evade for e in elist))
                ...
                for boss, stats, original in zip(
                    shuffled_bosses, location_stats, stat_inheritance
                ):
                    # skip if vanilla
                    if boss == original:
                        continue
                    ...
                    for i, enemy in enumerate(formation.stat_scaling_enemies):
                        ...
                        no_raise_defense = (
                            utils.isclass_or_instance(enemy, enemies.BahamuttMagikoopa)
                            or utils.isclass_or_instance(enemy, enemies.BahamuttChester)
                            or utils.isclass_or_instance(enemy, enemies.JinxClone)
                        )
                        ...
                        enemy.magic_defense = min(
                            int(
                                round(
                                    stats["magic_defense"] * enemy.ratio_magic_defense
                                )
                            ),
                            enemy.magic_defense
                        ...
                        if formation.index == 309:
                            if utils.isclass_or_instance(
                                enemy, (enemies.Cloaker2, enemies.Domino2)
                            ):
                        ...
                        # Otherwise give the first enemy all the XP/coins, except for Hammer Bros that need half.
                            ...
                            if (
                                utils.isclass_or_instance(enemy, enemies.Henchman)
                                and original_xp_sum > 0
                            ):
                                enemy.xp = max(
                                    1, round(enemy.xp / original_xp_sum * stats["xp"])
                                )
            ...
            for location, boss in zip(locations, shuffled_bosses):
                ...
                if world.settings.is_flag_enabled(flags.MimicsAnywhere) and (
                    utils.isclass_or_instance(location, bosses.Pandorite)
                    or utils.isclass_or_instance(location, bosses.Hidon)
                    or utils.isclass_or_instance(location, bosses.BoxBoy)
                ):
                ...
                if utils.isclass_or_instance(location, bosses.Director):
                    unique_henchmen = [
                        h
                        for h in boss.unique_henchmen
                        if h.model.animations is not None
                        and h.model.animations.factory_pierce is not None
                    ] + [
                        h
                        for h in boss.unique_henchmen
                        if h.model.animations is None
                        or h.model.animations.factory_pierce is None
                    ]
                    repeatable_henchmen = [
                        h
                        for h in boss.repeatable_henchmen
                        if h.model.animations is not None
                        and h.model.animations.factory_pierce is not None
                    ] + [
                        h
                ...
                if not is_vanilla(boss, location):
                    ...
                    for index, henchman in enumerate(location.unique_henchmen):
                        for index2, model in enumerate(henchman):
                            if (
                                ...
                                requires_henchman_with_pack = (
                                    location.unique_henchmen[index][index2].fill_type
                                    is not bosses.HenchmanType.NPC_ONLY
                                )
                                eligible_repeatable_henchmen = [
                                    r
                                    for r in repeatable_henchmen
                                    if (
                                        (
                                            requires_henchman_with_pack
                                            and r.pack_number is not None
                                        )
                                        or not requires_henchman_with_pack
                                    )
                                ]

                                if index < len(unique_henchmen) and not (
                                    requires_henchman_with_pack
                                    and unique_henchmen[index].pack_number is None
                                ):
                                    new_unique_henchman = unique_henchmen[index]
                                    new_henchman = new_unique_henchman
                                    location.unique_henchmen[index][
                                        index2
                                    ].occupant = new_henchman
                                ...
                                # otherwise, repeatable henchmen can fill the npc slot if permitted
                                    ...
                                    new_henchman = random.choice(
                                        eligible_repeatable_henchmen
                                    )
                                    if location.unique_henchmen[index][
                                        index2
                                    ].repeatable_allowed:
                                        location.unique_henchmen[index][
                                            index2
                                        ].occupant = new_henchman
                                        set_henchman_run_flags(
                                            world, boss, new_henchman
                                        )
                                    elif location.unique_henchmen[index][
                                        index2
                                    ].remove_if_empty:
                                        location.unique_henchmen[index][
                                            index2
                    ...
                    for index, henchman in enumerate(location.repeatable_henchmen):
                        ...
                        if index == 0 and utils.isclass_or_instance(
                            location, bosses.Punchinello
                        ):
                        ...
                        for index2, model in enumerate(henchman):
                            if (
                                ...
                                if new_henchman is not None:
                                    location.repeatable_henchmen[index][
                                        index2
                                    ].occupant = new_henchman
                                    set_henchman_run_flags(world, boss, new_henchman)
                                elif location.repeatable_henchmen[index][
                                    index2
                                ].remove_if_empty:
                                    location.repeatable_henchmen[index][
                                        index2
                    ...
                    # update run flags
                    for index, henchman in enumerate(
                        location.unique_henchmen + location.repeatable_henchmen
                    ):
                        for index2, model in enumerate(henchman):
                            if (
                                model.occupant is not None
                                and model.occupant.pack_number is not None
                                and model.fill_type != HenchmanType.NPC_ONLY
                            ):
                                can_run_away = model.can_run_away
                                pack_number = model.occupant.pack_number
                                pack = world.get_formation_pack_by_index(pack_number)
                                formations = [
                                    world.get_enemy_formation_by_index(f.index)
                    ...
                    if not utils.isclass_or_instance(
                        location, bosses.Boomer
                    ) and utils.isclass_or_instance(boss, bosses.BoomerBoss):
            ...
            for boss_location in world.boss_locations:
                ...
                if 353 not in fight_builders:
                    fight_builders[353] = {"jumps": [], "executions": []}
                if 355 not in fight_builders:
                    fight_builders[355] = {"jumps": [], "executions": []}
                ...
                if formation.required_battlefield is None:
                    if utils.isclass_or_instance(boss_location, bosses.Mokura):
                        cmds = [
                            new_command(353, "set_short", [0x700E, boss.pack_number]),
                            new_command(353, "start_battle_700E"),
                        ]
                    else:
                        ...
                        if utils.isclass_or_instance(boss_location, bosses.Pandorite):
                            where_is_pandorite = [
                                ...
                                if utils.isclass_or_instance(c.item, PandoriteFight)
                            ]
                            ...
                            where_is_hidon = [
                                c
                                for c in world.chest_locations
                                if utils.isclass_or_instance(c.item, HidonFight)
                            ]
                            ...
                            where_is_boxboy = [
                                c
                                for c in world.chest_locations
                                if utils.isclass_or_instance(c.item, BoxBoyFight)
                            ]
                        ...
                        for t, t_r in fields:
                            if bl_identifier in t_r:
                                ...
                                break
                        if (
                            boss_location.battlefield is None
                        ):  # default to BV underground if still empty
                            boss_location.battlefield = (
                                bosses.Battlefields.BeanValleyUnderground
                            )
                        cmds = [
                            new_command(
                                353,
                ...
                if world.settings.is_flag_value(
                ...
                ) and utils.isclass_or_instance(boss, bosses.HammerBroBoss):
                    map_bits = [[0x7065, 4], [0x706D, 4]]
                    dialog = [2256]
                elif utils.isclass_or_instance(boss, bosses.BowyerBoss):
                    if world.settings.is_flag_value(
                        flags.PipeVaultGate, PipeVaultGating.bowyer
                    ):
                        map_bits.append([0x7055, 7])
                        dialog = [2258]
                    if world.settings.is_flag_value(
                        flags.Moleville1Gate, PipeVaultGating.bowyer
                    ):
                        map_bits.append([0x707B, 3])
                        dialog = [2269]
                elif world.settings.is_flag_value(
                    flags.BoosterTowerGate, BoosterTowerGating.punchinello
                ) and utils.isclass_or_instance(boss, bosses.PunchinelloBoss):
                    map_bits = [[0x7053, 6]]
                    dialog = [2259]
                elif world.settings.is_flag_value(
                    flags.MarrymoreGate, MarrymoreGating.kggg
                ) and utils.isclass_or_instance(boss, bosses.GrateGuyBoss):
                    map_bits = [[0x704C, 7]]
                    dialog = [2260]
                elif world.settings.is_flag_value(
                    flags.SeaGate, SeaGating.bundt
                ) and utils.isclass_or_instance(boss, bosses.BundtBoss):
                    map_bits = [[0x7067, 4], [0x706F, 3]]
                    dialog = [2261]
                elif world.settings.is_flag_value(
                    flags.BelomeTempleGate, BelomeTempleGating.yarid
                ) and utils.isclass_or_instance(boss, bosses.YaridovichBoss):
                    map_bits = [[0x7052, 2]]
                    dialog = [2263]
                elif world.settings.is_flag_value(
                    flags.MonstroTownGate, MonstroTownGating.belome2
                ) and utils.isclass_or_instance(boss, bosses.Belome2Boss):
                    map_bits = [[0x7067, 7], [0x706F, 6]]
                    dialog = [2267]
                elif world.settings.is_flag_value(
                    flags.BarrelVolcanoGate, BarrelVolcanoGating.valentina
                ) and utils.isclass_or_instance(boss, bosses.ValentinaBoss):
                    map_bits = [[0x7090, 5], [0x7070, 1], [0x7068, 2]]
                    dialog = [2268]
                elif world.settings.is_flag_value(
                    flags.BowsersKeepGate, BowsersKeepGating.axem
                ) and utils.isclass_or_instance(boss, bosses.AxemRangersBoss):
                    map_bits = [[0x7068, 3]]
                    dialog = [2264]
                    ...
                    extra_dialog_cmd = utils.new_command(
                        355,
                        "run_dialog",
                        [
                            2265,
                    ...
                    flags.YaridovichGate, YaridovichGating.johnny
                ) and utils.isclass_or_instance(boss, bosses.JohnnyBoss):
                    map_bits = [[0x7057, 1]]
                    dialog = [2262]
                ...
                if not is_vanilla(boss, boss_location):
                    ...
                    if utils.isclass_or_instance(boss_location, bosses.Valentina):
                        ...
                        for statue_location in boss_location.statue_locations:
                            ...
                            world.rooms[room_id].objects[npc_id].model.occupant = statue

                            # flip directions where necessary
                            eligible_directions = statue.directions
                            current_direction = (
                                world.rooms[room_id].objects[npc_id].direction
                            )
                            ...
                            if statue.details.horizontal_pixel_shift > 0:
                                horizontal_shift = 0xFF & (
                                    statue.details.horizontal_pixel_shift
                                )
                            elif statue.details.horizontal_pixel_shift < 0:
                                horizontal_shift = 0xFF & (
                                    0xFF + statue.details.horizontal_pixel_shift + 1
                                )
                            if statue.details.north_facing_horizontal_pixel_shift > 0:
                                reverse_horizontal_shift = 0xFF & (
                                    statue.details.north_facing_horizontal_pixel_shift
                                )
                                reverse_horizontal_shift_b = 0xFF & (
                                    0xFF
                                    + statue.details.north_facing_horizontal_pixel_shift
                                    + 1
                                )
                            ...
                            if statue.details.vertical_pixel_shift > 0:
                                vertical_shift = 0xFF & (
                                    statue.details.vertical_pixel_shift
                                )
                            elif statue.details.vertical_pixel_shift < 0:
                                vertical_shift = 0xFF & (
                                    0xFF + statue.details.vertical_pixel_shift + 1
                                )
                            if (horizontal_shift != 0 or vertical_shift != 0) and (
                                new_direction == RadialDirection.SOUTHWEST
                                or new_direction == RadialDirection.SOUTHEAST
                            ):
                                cmd = new_animation(
                                    statue_location.sequence_setter,
                                    "action_queue_async",
                                ...
                                reverse_horizontal_shift != 0 or vertical_shift != 0
                            ) and new_direction == RadialDirection.NORTHWEST:
                                cmd = new_animation(
                                    statue_location.sequence_setter,
                                    "action_queue_async",
                                    npc_id,
                                ...
                                reverse_horizontal_shift_b != 0 or vertical_shift != 0
                            ) and new_direction == RadialDirection.NORTHEAST:
                                cmd = new_animation(
                                    statue_location.sequence_setter,
                                    "action_queue_async",
                                    npc_id,
                    ...
                    for boss_sprite_location in boss_location.boss_locations:
                        ...
                        if utils.isclass_or_instance(boss_location, bosses.Gunyolk):
                            ...
                            for i in [1, 2, 3, 4, 5, 6]:
                                world.rooms[470].objects[i].visible = False
                            world.rooms[470].objects[npc_id].y = 85

                        elif utils.isclass_or_instance(boss_location, bosses.Smithy):
                            # hide composite NPCs that aren't used if shuffled
                            world.rooms[509].objects[npc_id].x = 6
                            world.rooms[509].objects[npc_id].y = 18
                            for i in range(5, 15):
                                world.rooms[509].objects[i].model.occupant = npcs.Empty
                                world.rooms[509].objects[i].visible = False

                        elif utils.isclass_or_instance(
                            boss_location, bosses.MegaSmilax
                        ):
                            # move NPCs in megasmilax's room
                            world.rooms[254].objects[0].visible = False
                            world.rooms[254].objects[1].z = 1
                        ...
                        models = [
                            occupant.attack_model,
                            occupant.big_model,
                            occupant.small_model,
                        ]
                        models = [m for m in models if m is not None]

                        if room_id in [
                            205
                        ]:  # rooms that just don't work nicely for models with 6 min vram
                            is_current = False
                            for m in models:
                                sid = m.sprite_id
                                ...
                                if (
                                    is_current
                                    and utils.get_min_vram_from_animation(
                                        model_sprite, 0
                                    )
                                    < 6
                                ):
                        ...
                        # replace the models
                        world.rooms[room_id].objects[npc_id].model.occupant = model

                        # set Z half for short npcs in booster's portrait game room so that theyre visible when giving you the item
                        if (
                            utils.isclass_or_instance(boss_location, bosses.Booster)
                            and room_id
                            == Rooms._195_BOOSTER_TOWER_6F_AREA_02_BOOSTERS_ANCESTOR_GAME_ROOM
                        ):
                            if (
                                utils.isclass_or_instance(model, npcs.MimicFace)
                                or utils.isclass_or_instance(model, npcs.Jinx)
                                or utils.isclass_or_instance(model, npcs.MokuraCloud)
                            ):
                                world.rooms[room_id].objects[npc_id].z_half = True
                        ...
                        for script_id in boss_sprite_location.target_scripts:
                            ...
                            if (
                                utils.isclass_or_instance(
                                    boss_location, bosses.Punchinello
                                )
                                and script_id == 860
                            ):
                                pause = 10
                                for command_index, command in enumerate(script):
                                    if is_animation_header(command, npc_id):
                                        if (
                                            ...
                                            if (
                                                model.animations.mines_punch.contact_frame
                                                or 0
                                            ) > 0:
                                                pause = (
                                                    model.animations.mines_punch.contact_frame
                                                ...
                                                pause = (
                                                    model.animations.mines_punch.total_duration
                                                    if model.animations.mines_punch.total_duration
                                                    is not None
                                                    else 30
                                                )
                                        for (
                            ...
                            if (
                                utils.isclass_or_instance(
                                    boss_location, bosses.Magikoopa
                                )
                                and model.animations is not None
                                and model.animations.keep_summon is not None
                                and script_id == 941
                            ):
                                if (
                                    model.animations.keep_summon.contact_frame
                                    is not None
                                ):
                                    world.eventscripts[script_id][1]["args"][0] = (
                                        model.animations.keep_summon.contact_frame + 16
                                    )

                            # shift to the right if croco is at booster
                            if (
                                utils.isclass_or_instance(boss_location, bosses.Booster)
                                and model.tower_entrance_horizontal_shift != 0
                                and room_id == 202
                            ):
                                ...
                                world.actionscripts[519].insert(
                                    0,
                                    {
                                        "identifier": "dummy",
                                        "command": order,
                            ...
                            dojo_duration = (
                                0
                                if (
                                    model.animations is None
                                    or model.animations.dojo_challenge is None
                                )
                                else (
                                    model.animations.dojo_challenge.total_duration or 0
                                )
                            )
                            if (
                                utils.isclass_or_instance(boss_location, bosses.Jagger)
                                and model.animations is not None
                                ...
                                utils.isclass_or_instance(boss_location, bosses.Jinx1)
                                and model.animations is not None
                                and model.animations.dojo_challenge is not None
                            ):
                                for command_index, command in enumerate(script):
                                ...
                                utils.isclass_or_instance(boss_location, bosses.Jinx2)
                                and model.animations is not None
                                and model.animations.dojo_challenge is not None
                            ):
                                for command_index, command in enumerate(script):
                                ...
                                utils.isclass_or_instance(boss_location, bosses.Jinx3)
                                and model.animations is not None
                                and model.animations.dojo_challenge is not None
                            ):
                                for command_index, command in enumerate(script):
                            ...
                            if utils.isclass_or_instance(
                                boss_location, bosses.Dodo
                            ) and (script_id == 936):
                                if (
                                    model.animations is None
                                    or model.animations.statue_peck is None
                                ):
                                    ...
                                    peck_duration = (
                                        model.animations.statue_peck.contact_frame
                                    )
                                    if peck_duration > 19 or peck_duration is None:
                                        raise Exception(
                                            "%s statue peck animation contact frame is illegal value"
                                            % boss.name
                                        )
                                    animation_wait = 15 + 16 - peck_duration
                                    animation_duration = peck_duration + 3
                                    rewritten_peck_subroutine.append(
                                        {
                                            "identifier": "dummy",
                                            "command": "pause",
                                            "args": [animation_wait],
                                        ...
                                        rewritten_peck_subroutine.append(
                                            {
                                                ...
                                                "command": "set_animation_speed",
                                                "args": [
                                                    model.animations.statue_peck.speed,
                                                    [_0x10Flags.SEQUENCE],
                                                ],
                                            }
                                        )
                                    rewritten_peck_subroutine.append(
                                        {
                                            ...
                                            "command": "set_sprite_sequence",
                                            "args": [
                                                model.animations.statue_peck.sequence_id,
                                                0,
                                                [_0x08Flags.LOOPING_OFF],
                                            ],
                                        }
                                    )  # no support for increased sprite #, but no use case for it yet
                                    rewritten_peck_subroutine.append(
                                        {
                                            "identifier": "dummy",
                                            "command": "pause",
                                    ...
                                    world.eventscripts[script_id][0][
                                        "subscript"
                                    ] = copy.deepcopy(
                                        [{**s} for s in rewritten_peck_subroutine]
                                    )
                                ...
                                utils.isclass_or_instance(boss_location, bosses.Dodo)
                                and (script_id == 939)
                                and model.animations is not None
                                and model.animations.statue_intro is not None
                            ):
                                ...
                                if (
                                    model.animations.statue_intro.total_duration
                                    is not None
                                ):
                                    intro_duration = min(
                                        model.animations.statue_intro.total_duration, 66
                                    )
                                    if intro_duration < 66:
                                        rewritten_intro_subroutine.append(
                                            {
                                                "identifier": "dummy",
                                                "command": "pause",
                                                "args": [66 - intro_duration],
                                            }
                                        )
                                    rewritten_intro_subroutine.append(
                                        {
                                            ...
                                            "command": "set_sprite_sequence",
                                            "args": [
                                                model.animations.statue_intro.sequence_id,
                                                0,
                                                [
                                                    _0x08Flags.LOOPING_OFF,
                                                    _0x08Flags.MIRROR_SPRITE,
                                ...
                                world.eventscripts[script_id][0][
                                    ...
                                    [{**s} for s in rewritten_intro_subroutine]
                                )
                                ...
                                and (script_id == 940)
                                and model.animations is not None
                                and model.animations.statue_flustered is not None
                            ):
                                ...
                                rewritten_recoil_subroutine.append(
                                    {
                                        "identifier": "dummy",
                                        "command": "set_sprite_sequence",
                                        "args": [
                                            model.animations.statue_flustered.sequence_id,
                                            0,
                                            [
                                                _0x08Flags.LOOPING_OFF,
                                                _0x08Flags.MIRROR_SPRITE,
                                ...
                                world.eventscripts[script_id][0][
                                    ...
                                    [{**s} for s in rewritten_recoil_subroutine]
                                )

                            elif utils.isclass_or_instance(
                                boss_location, bosses.Chester
                            ) and (script_id == 2190):
                                ...
                                if (
                                    ...
                                    rewritten_chester_subscript.append(
                                        {
                                            "identifier": "dummy",
                                            "command": "set_sprite_sequence",
                                            "args": [
                                    ...
                                    if (
                                        model.animations.keep_challenge.total_duration
                                        is not None
                                    ):
                                        rewritten_chester_subscript.append(
                                            {
                                ...
                                world.eventscripts[script_id][0][
                                    ...
                                    [{**s} for s in rewritten_chester_subscript]
                                )
                                ...
                                if (
                                    ...
                                    rewritten_keep_subscript = [
                                        ...
                                        },
                                        {
                                            ...
                                            "command": "set_sprite_sequence",
                                            "args": [
                                                model.animations.keep_summon.sequence_id,
                                                0,
                                                [
                                                    _0x08Flags.MIRROR_SPRITE,
                                                    _0x08Flags.READ_AS_SEQUENCE,
                                        ...
                                        },
                                        {
                                            ...
                                            "command": "pause",
                                            "args": [
                                                model.animations.keep_summon.total_duration
                                                if model.animations.keep_summon.total_duration
                                                is not None
                                                else 39
                                            ],
                                        },
                                ...
                                rewritten_keep_event = [
                                    {
                                        "identifier": "EVENT_942_action_queue_async",
                                        "command": "action_queue",
                                        "args": [AreaObjects.NPC_1, False],
                                        "subscript": rewritten_keep_subscript,
                                    },
                                    {
                                ...
                                world.eventscripts[script_id] = copy.deepcopy(
                                    [{**s} for s in rewritten_keep_event]
                                )

                            # boomer will need pause adjustments
                            elif utils.isclass_or_instance(
                                boss_location, bosses.Boomer
                            ) and (script_id == 943):
                                ...
                                if utils.isclass_or_instance(boss, bosses.MimicBoss):
                                    rewritten_chandelier_subscript.extend(
                                        boss.challenge_script
                                    )
                                    rewritten_chandelier_subscript.append(
                                        {
                                            "identifier": "dummy",
                                            "command": "pause",
                                            "args": [45],
                                    ...
                                    model.animations is not None
                                    and model.animations.chandelier_challenge
                                    is not None
                                ):
                                    rewritten_chandelier_subscript.append(
                                        {
                                            ...
                                            "command": "set_sprite_sequence",
                                            "args": [
                                                model.animations.chandelier_challenge.sequence_id,
                                                0,
                                                [
                                                    _0x08Flags.LOOPING_OFF,
                                                    _0x08Flags.READ_AS_SEQUENCE,
                                    ...
                                    if (
                                        model.animations.chandelier_challenge.total_duration
                                        is not None
                                    ):
                                        rewritten_chandelier_subscript.append(
                                            {
                                ...
                                world.eventscripts[script_id][0][
                                    ...
                                    [{**s} for s in rewritten_chandelier_subscript]
                                )

                            # smithy needs A LOT of adjustments, to the point of complete script replacement and npc removal
                            elif utils.isclass_or_instance(
                                boss_location, bosses.Smithy
                            ) and (script_id == 3792):
                                world.eventscripts[script_id] = copy.deepcopy(
                                    [{**s} for s in non_smithy_3792]
                                )
                            elif utils.isclass_or_instance(
                                boss_location, bosses.Smithy
                            ) and (script_id == 3794):
                                world.eventscripts[script_id] = copy.deepcopy(
                                    [{**s} for s in non_smithy_3794]
                                )
                                if (
                                    model.animations is not None
                                    and model.animations.endgame_challenge is not None
                                ):
                                    if (
                                        model.animations.endgame_challenge.total_duration
                                        is not None
                                    ):
                                        challenge_duration = (
                                            model.animations.endgame_challenge.total_duration
                                        ...
                                        if challenge_duration > 55:
                                            world.eventscripts[945][0]["args"] = [
                                                challenge_duration
                                            ]
                                            world.eventscripts[946][0][
                                                "subscript"
                                            ].insert(
                                                0,
                                                {
                                            ...
                                            "subscript": [
                                                {
                                                    ...
                                                    "command": "set_sprite_sequence",
                                                    "args": [
                                                        model.animations.endgame_challenge.sequence_id,
                                                        0,
                                                        [
                                                            _0x08Flags.READ_AS_SEQUENCE,
                                                            _0x08Flags.LOOPING_OFF,
                        ...
                        for script_id in boss_sprite_location.target_action_scripts:
                            ...
                            if (
                                ...
                                if boss.eye_height > 17:
                                    initial_shift = {
                                        "identifier": "ACTION_519_initial_shift",
                                        "command": "shift_south_pixels",
                                        "args": [boss.eye_height - 17],
                                    }
                                elif boss.eye_height < 17:
                                    initial_shift = {
                                        "identifier": "ACTION_519_initial_shift",
                                        "command": "shift_north_pixels",
                                        "args": [17 - boss.eye_height],
                                    }
                                if initial_shift is not None:
                                    world.actionscripts[script_id].insert(
                                        0, initial_shift
                                    )
                            ...
                            # replace all sequences and molds if appropriate, remove if not
                                ...
                                world.actionscripts[
                                    ...
                                    boss, boss_location, script, model
                                )
                        ...
                        for script_id in boss_sprite_location.target_scripts:
                            min_vram = max(
                                min_vram,
                                utils.get_min_vram_size_for_eventscript(
                                    world,
                                    npc_id,
                                    model.sprite_id,
                    ...
                    for u in (
                        boss_location.unique_henchmen
                        + boss_location.repeatable_henchmen
                    ):
                        for henchman_location in u:
                            ...
                            if room_id in [
                                ...
                                411,
                                437,
                            ] and not utils.isclass_or_instance(boss, bosses.CzarBoss):
                                world.rooms[room_id].objects[npc_id].z = 0

                            if occupant is None:
                                # remove this NPC if necessary when boss has nothing to fill
                                if (
                                    henchman_location.remove_if_empty
                                ):  # is gunyolk npc 0 getting an occupant when it shouldnt be?
                                    world.rooms[room_id].objects[npc_id].visible = False
                                    world.rooms[room_id].objects[
                                        npc_id
                                    ].model.occupant = npcs.Empty

                                # adjust trampoline animation in volcano
                                if (
                                    utils.isclass_or_instance(
                                        boss_location, bosses.AxemRangers
                                    )
                                    and script_id == 3344
                                    and room_id == 393
                                ):
                                    ...
                                    world.eventscripts[script_id] = [
                                        com
                                        for com in world.eventscripts[script_id]
                                        if not is_animation_header(com, npc_id)
                                    ]
                                    for cmd_index, cmd in enumerate(
                                        world.eventscripts[script_id]
                                    ):
                                        if is_animation_header(cmd, 0):
                                            for subs_index, subc in enumerate(
                                                cmd["subscript"]
                                            ):
                                                if (
                                                    subc["command"]
                                ...
                                # print(boss, boss_location, model, model.sprite_id)
                                world.rooms[room_id].objects[
                                    npc_id
                                ].model.occupant = model

                                if not has_vanilla_henchmen(boss, boss_location):
                                    # update models
                                    ...
                                    for (
                                        ...
                                        if (
                                            utils.isclass_or_instance(
                                                boss_location, bosses.Bundt
                                            )
                                            and script_id in [330, 331]
                                            and model.animations is not None
                                            and model.animations.kitchen_prep
                                            is not None
                                        ):
                                            for (
                                                subscript_command_index,
                                                ...
                                                if (
                                                    ...
                                                    cmd_flags = [
                                                        ...
                                                        for f in cmd_flags
                                                        if f
                                                        is not _0x08Flags.LOOPING_OFF
                                                    ]
                                                    if (
                                                        model.animations is not None
                                                        and model.animations.kitchen_prep.total_duration
                                                        is not None
                                                    ):
                                                        cmd_flags.append(
                                                            _0x08Flags.LOOPING_OFF
                                        ...
                                        # overwrite snifit 3's bullet script
                                            ...
                                            if (
                                                model.animations is None
                                                or model.animations.tower_bullet is None
                                            ):
                                                world.actionscripts[script_id] = [
                                                    {
                                                        "identifier": "ACTION_386_face_southeast_0",
                                                        "command": "face_southeast",
                                                    },
                                                    {
                                                        "identifier": "ACTION_386_pause_1",
                                                        "command": "pause",
                                                        "args": [18],
                                                    },
                                                    {
                                                        "identifier": "ACTION_386_face_southwest_2",
                                                        "command": "face_southwest",
                                                    },
                                                    {
                                                        "identifier": "ACTION_386_pause_3",
                                                        "command": "pause",
                                                        "args": [18],
                                                    },
                                                    {
                                                        "identifier": "ACTION_386_pause_init",
                                                        "command": "pause",
                                                        "args": [56],
                                                    },
                                                    {
                                                        "identifier": "ACTION_386_set_bit_18",
                                                        "command": "set_bit",
                                                        "args": [0x7043, 3],
                                                    },
                                                    {
                                                        "identifier": "ACTION_386_pause_second",
                                                        "command": "pause",
                                                        "args": [40],
                                                    },
                                                    {
                                                        "identifier": "ACTION_386_jmp_27",
                                                        "command": "jmp",
                                                        "args": [
                                                            "ACTION_386_pause_init"
                                                        ],
                                                    },
                                                ]
                                            elif (
                                                model.animations.tower_bullet.total_duration
                                                is None
                                            ):
                                                world.actionscripts[script_id] = [
                                                    {
                                                    ...
                                                    },
                                                    {
                                                        ...
                                                        "command": "set_sprite_sequence",
                                                        "args": [
                                                            model.animations.tower_bullet.sequence_id,
                                                            0,
                                                            [
                                                                _0x08Flags.READ_AS_SEQUENCE
                                                            ],
                                                ...
                                                if (
                                                    model.animations.tower_bullet.contact_frame
                                                    is not None
                                                ):
                                                    contact = (
                                                        model.animations.tower_bullet.contact_frame
                                                ...
                                                if (
                                                    model.animations.tower_bullet.speed
                                                    is not None
                                                ):
                                                    world.actionscripts[
                                                        script_id
                                                ...
                                                world.actionscripts[script_id].extend(
                                                    [
                                                        ...
                                                        {
                                                            "identifier": "ACTION_386_pause_intermediate",
                                                            "command": "pause",
                                                            "args": [contact - 8],
                                                        },
                                                        {
                                        ...
                                        # overwrite poundette background animation
                                            ...
                                            utils.isclass_or_instance(
                                                boss_location, bosses.Director
                                            )
                                            and script_id in [962, 963, 964]
                                            and model.animations is not None
                                            and model.animations.factory_pierce
                                            is not None
                                        ):
                                            scr = []
                                            ...
                                            for (
                                                ...
                                                if (
                                                    ...
                                                    subs["args"][
                                                        ...
                                                        model.animations.factory_pierce.sequence_id
                                                    )
                                                    if (
                                                        model.animations.factory_pierce.contact_frame
                                                        is not None
                                                    ):
                                                        if (
                                                            model.animations.factory_pierce.speed
                                                            is not None
                                                        ):
                                                            scr.append(
                                                                {
                                                                    "identifier": "ACTION_%i_spd"
                                                                    % script_id,
                                                                    "command": "set_animation_speed",
                                                                    "args": [
                                                                        model.animations.factory_pierce.speed,
                                                        ...
                                                        if initial_wait > 0:
                                                            scr.append(
                                                                {
                                                                    "identifier": "ACTION_%i_wait_init"
                                                                    % script_id,
                                                                    "command": "pause",
                                                                    "args": [
                                                                        initial_wait
                                                        ...
                                                        scr.append(
                                                            {
                                                                "identifier": "ACTION_%i_wait_post"
                                                                % script_id,
                                                                "command": "pause",
                                                                "args": [
                                                                    model.animations.factory_pierce.contact_frame
                                                        ...
                                                        subs_reset = copy.deepcopy(
                                                            subscript_command
                                                        )
                                                        subs["args"][2] = [
                                                            s
                                                            for s in subs["args"][2]
                                                            if s
                                                            != _0x08Flags.LOOPING_OFF
                                                        ]
                                                        scr.append(subs)
                    ...
                    uniq = [
                        item
                        for sublist in boss_location.unique_henchmen
                        for item in sublist
                    ] + [
                        item
                        for sublist in boss_location.repeatable_henchmen
                        for item in sublist
                    ]
                    if not world.settings.is_flag_enabled(
                        ...
                        incoming_dialogs.extend(
                            boss_location.boss.optional_dialog_replacements
                        )
                        for loc in boss_location.boss_locations:
                            incoming_dialogs.extend(
                                loc.occupant.optional_dialog_replacements
                            )
                    for loc in uniq:
                        targeted_dialogs.extend(loc.dialogs)
                                ...
                                if d_data == bosses.EMPTY_DIALOG:
                                    ...
                                    for e_index in [
                                        ...
                                        for cmd in world.eventscripts[e_index]:
                                            if (
                                                cmd["command"] == "run_dialog"
                                                and cmd["args"][0] == d_id
                                            ):
                                                if utils.isclass_or_instance(
                                                    boss, bosses.CzarBoss
                                                ):
                ...
                for u in (
                    boss_location.unique_henchmen + boss_location.repeatable_henchmen
                ):
                    ...
                    for henchman_location in u:
                        ...
                        if occupant is None:
                            ...
                            henchman_location.fill_type == HenchmanType.EVENT
                            or henchman_location.fill_type
                            ...
                            fight_builders[henchman_location.event_id][
                                "executions"
                            ].extend(copy.deepcopy([{**s} for s in cmds]))
                            jmp = new_command(
                                henchman_location.event_id,
                                "jmp_if_7000_equals_short",
                            ...
                            world.rooms[room_id].objects[
                                npc_id
                            ].battle_pack = occupant.pack_number
            ...
            for e in fight_builders:
                ...
                world.eventscripts[e] = copy.deepcopy(
                    [{**s} for s in fight_builders[e]["jumps"]]
                ) + copy.deepcopy([{**s} for s in fight_builders[e]["executions"]])
            for e in sequence_setters:
                sequence_setters[e].append(new_command(e, "pause", [10]))
                world.eventscripts[e] = (
                    copy.deepcopy([{**s} for s in sequence_setters[e]])
                    + world.eventscripts[e]
                )
...
def randomize_music(world):
    ...
    if world.settings.is_flag_enabled(flags.BossShuffleMusic):
        ...
        music_choices = [
            m
            for m in world.music_pool
            if m.name not in world.settings.get_flag(flags.ShuffledMusic).disabled
        ]
