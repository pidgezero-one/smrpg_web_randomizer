dialog_data = [""]*168 # Increase to add more data
dialog_data[0] = '''SNIFIT 3: Um...
 Nice weather we're having.[await]'''
dialog_data[1] = '''SNIFIT 3: You wanna fight?[await]'''
dialog_data[2] = '''
           Got “Beetlemania”![await]'''
dialog_data[3] = '''GARDENER: I'll plant the seed...[await]'''
dialog_data[4] = '''GARDENER: Then give it fertilizer.[await]'''
dialog_data[5] = '''           Got “Beetlemania”![end]'''
dialog_data[6] = '''Got “Beetlemania”[end]'''
dialog_data[7] = '''
            Got a Star Piece![await]'''
dialog_data[8] = '''[await]'''
dialog_data[9] = ''' Can you believe there are people
 who call themselves gamers but
 don't own a single pair of jorts?[await]'''
dialog_data[10] = '''GARDENER: ••[delay]••[delay]••[delay]
          Nothing's happening.[await]'''
dialog_data[11] = '''SNIFIT 4: After years of
 hard work and effort, I'm now
 Snifit 4![await][page]
 
     I'll wear the number proudly![await]'''
dialog_data[12] = '''SNIFIT 5: Phew...![await]
 I've been training to become one
 of Booster's Snifits for 3 years![await]'''
dialog_data[13] = '''SNIFIT 6: If only Ma and Pa could
 see me now.[await]'''
dialog_data[14] = '''SNIFIT 7:
     Yee haw![await]
          Hurrah!
               I'm a Snifit![await][page]

         ••[delay]••[delay]••[delay]I'm••speechless![await]'''
dialog_data[15] = '''APPRENTICE: Shriek!
 The boss only wants 7 Snifits![await]
 All my training was in vain![await][page]

               What a rip![await]'''
dialog_data[16] = '''APPRENTICE: Harrumph!
 I'm off to the arcade![await]'''
dialog_data[17] = '''GARDENER: W••[delay]••[delay]wow![await]'''
dialog_data[18] = '''GARDENER: This is amazing![await]'''
dialog_data[19] = '''GARDENER: You think it could use
 a little more fertilizer?[await]'''
dialog_data[20] = '''GARDENER: Oops, maybe not, huh?[await]'''
dialog_data[21] = '''GARDENER: This will get me in
 Happy Gardener Monthly magazine![await]'''
dialog_data[22] = '''

     (Wish I could run faster.)[await]'''
dialog_data[23] = '''

    Wish I had some Cricket Jam.[await]'''
dialog_data[24] = '''
      I wanna be a great plumber
         like my brother Mario.[await]'''
dialog_data[25] = '''

   If I could JUST get that melody![await]'''
dialog_data[26] = '''
               The hunger!
           Oh, for some food! [await]'''
dialog_data[27] = '''
     I wanna be the best treasure
          hunter in the world.[await]'''
dialog_data[28] = '''

   I vant to be a vald class baker.[await]'''
dialog_data[29] = '''

        I hope I become famous.[await]'''
dialog_data[30] = '''

        I hope she'll say “yes”.[await]'''
dialog_data[31] = '''

    I wish he'd pop THE question.[await]'''
dialog_data[32] = '''

   I wish I weren't such a crybaby.[await]'''
dialog_data[33] = '''

Please let Mallow find his way home.[await]'''
dialog_data[34] = '''MALLOW: This one's private![delay]
 Who do you think you are,
 spying on other people's wishes?[await]'''
dialog_data[35] = '''MALLOW: C'mon, our quest for the
 Star Pieces is important![await][page]
 Everyone's counting on us!
 We've got to hurry![await]'''
dialog_data[36] = '''MALLOW: Sorry about that.
 It's just that I'm embarrassed
 about my wish.[await]'''
dialog_data[37] = '''MALLOW: Why, this wish is...[await][page]
 It's from my mom and dad.[await][page]
 They're wishing for me to come
 home![await]'''
dialog_data[38] = '''MALLOW: 
 This isn't the time to be crying![await]
 They're waiting for me to come
 home![await]'''
dialog_data[39] = '''MALLOW: Come on Mario, we've got
 a busy schedule to keep. I'll make
 you eat dust if you don't hurry.[await]'''
dialog_data[40] = '''MALLOW: 
                Sniffle...[await]'''
dialog_data[41] = ''' What was that loud noise outside?
       Check out Booster Pass![await]'''
dialog_data[42] = '''
  You noticed something in the bush.[await]'''
dialog_data[43] = '''
Picked up a “[0x70A7]”[await]'''
dialog_data[44] = '''GARDENER: Hey!
 I haven't finished yet.[await]'''
dialog_data[45] = '''GARDENER: Short attention span?[await]'''
dialog_data[46] = '''GARDENER: Hi, I'm the gardener.[delay][delay][delay]
 See that huge planter?[await][page]
 I've spent many years seeking the
 legendary “Seed” and “Fertilizer”
 to fill it with, but had no luck.[await][page]
 Sigh...if I could only find them![await]'''
dialog_data[47] = '''GARDENER: Incredible!
 I can't believe it! It's the
 legendary “Seed” I've read about
 in the ancient gardening texts.
 These are so rare! Can you even
 begin to comprehend what a huge
 discovery this is? I've spent my
 whole life searching for one!
 It's too much, I tell ya![delay_24][end]'''
dialog_data[48] = '''GARDENER: Egads!
 Do you know what you have there?
 That fertilizer comes from the
 Shy Away clan, famous for its
 miraculous feats of gardening. I
 looked for it beyond the cliffs,
 even tried Bean Valley, but failed.
 I've traveled around the world and
 waited a lifetime for some of that
 fertilizer you've got there![delay_24][end]'''
dialog_data[49] = '''GARDENER: Well, that's my story.[await]
 Say...will you give me the “Seed”?
  [select]  (Sure)
  [select]  (Nope)[await]'''
dialog_data[50] = '''GARDENER: Yes, you might say I
 have fertilizer on the brain.[await]
 Will you give it to me?
  [select]  (Sure)
  [select]  (Sorry)[await]'''
dialog_data[51] = '''GARDENER: You changed your mind?[await]
  [select]  (Yes)
  [select]  (No)[await]'''
dialog_data[52] = '''GARDENER: Well, `MAIN_CHARACTER_NAME`?[await]
 Will you give me the fertilizer?
  [select]  (Yes)
  [select]  (No)[await]'''
dialog_data[53] = '''GARDENER: I finally have it![await][page]
 This is extraordinary![await]'''
dialog_data[54] = ''' May I have the fertilizer?
  [select]  (Yes)
  [select]  (No)[await]'''
dialog_data[55] = '''GARDENER: Sigh...
 What ever happened to granting
 an old man's wishes?[await]'''
dialog_data[56] = '''GARDENER: Oh thank you![await]
 I finally have both the “Seed” and
 the “Fertilizer”.[await]
 I'll try them out right away.[await]'''
dialog_data[57] = '''GARDENER: I'd be so happy if I
 only had the “Seed”![await][page]
 I'm sooo disappointed.[await]'''
dialog_data[58] = '''GARDENER: I finally have it![await]
 But without the “Seed”,
 it's useless.[await]'''
dialog_data[59] = ''' You're giving me the fertilizer too?
  [select]  (Yes)
  [select]  (No)[await]'''
dialog_data[60] = '''GARDENER: Gals don't care for
 tightwads, you know![await]'''
dialog_data[61] = '''

   I wish for everyone to be happy.[await]'''
dialog_data[62] = '''DEALER: 
         I will draw one card.[await]'''
dialog_data[63] = '''DEALER: 
        I'll play with this hand.[await]'''
dialog_data[64] = '''DEALER: 
 Draw or stay, `MAIN_CHARACTER_NAME`?[await]'''
dialog_data[65] = '''            Your total is [0x7000].
  [select] (Draw)
  [select] (Stay)[await]'''
dialog_data[66] = '''DEALER: 
 I will first deal you two cards.[await]'''
dialog_data[67] = '''            You got a(n) [0x7000].[await]'''
dialog_data[68] = '''      Oh no! You went over 21.[await]'''
dialog_data[69] = '''DEALER:
      Please show me your hand.[await]'''
dialog_data[70] = '''DEALER: 
                I have [0x7000].[await]'''
dialog_data[71] = '''DEALER: 
 `MAIN_CHARACTER_NAME`, you have [0x7000].[await]'''
dialog_data[72] = '''DEALER: I broke 21, so you won![await]
 Here's your Frog Coin.[await]'''
dialog_data[73] = '''DEALER: You won this round.[await]
 Here is your Frog Coin.[await]'''
dialog_data[74] = '''DEALER: You lost this round.[await]
 I'll take your Frog Coin.[await]'''
dialog_data[75] = '''DEALER: It's a tie.[await]
 I'll keep your Frog Coin.[await]'''
dialog_data[76] = '''DEALER: No Frog Coins?[await]
 Sorry, you can't play then.[await]'''
dialog_data[77] = '''DEALER: 
            Your first card.[await]'''
dialog_data[78] = '''DEALER: 
          Your second card.[await]'''
dialog_data[79] = '''DEALER: 
 Welcome to the Blackjack table.[await]
 Play a round with me, `MAIN_CHARACTER_HONORIFIC`?
  [select] (Yes)
  [select] (No)[await]'''
dialog_data[80] = ''' Another game, `MAIN_CHARACTER_HONORIFIC`?
  [select] (Sure)
  [select] (No)[await]'''
dialog_data[81] = '''DEALER: If you would like to play,
 please go to the other side of
 the table.[await]'''
dialog_data[82] = '''DOORMAN: Sorry, this casino is
 for members only.[await]'''
dialog_data[83] = '''DOORMAN: You don't have a
 “Bright Card”?[await]
 I'm sorry, I can't let you in.[await]'''
dialog_data[84] = '''DOORMAN: Please enjoy yourself.[await]'''
dialog_data[85] = '''DOORMAN: We hope everything was
 to your liking?[await]'''
dialog_data[86] = ''' (Hit right/left on the Control Pad)[await]'''
dialog_data[87] = ''' Okay, `MAIN_CHARACTER_NAME`, this is in
 celebration of your [0x7000] win(s)![await]'''
dialog_data[88] = ''' Wanna try it again?
  [select] (Yes)
  [select] (No)[await]'''
dialog_data[89] = ''' Welcome to the Grate Guy Casino![await]
 Wanna play the slot machines?
  [select] (Yes)
  [select] (No)[await]'''
dialog_data[90] = ''' Require instructions?
  [select] (No)
  [select] (Yes)[await]'''
dialog_data[91] = ''' I'll take your 10 coins.[await]
 Please enjoy yourself.[await]'''
dialog_data[92] = ''' Please come back with more coins.[await]'''
dialog_data[93] = ''' The slot machine costs 10 coins
 per game.[await][page]
 Hit it once to start the game.[await][page]
 Each consecutive hit will freeze
 one of the pictures. Match all 3
 and you might win a Frog Coin.[await][page]
 Do you want to try it out?
  [select] (Yes)
  [select] (No)[await]'''
dialog_data[94] = '''DEALER: Do you want to continue?
  [select] (Yes)
  [select] (No)[await]'''
dialog_data[95] = '''           [delay]You [delay]missed![delay_40][end]'''
dialog_data[96] = '''          Talk to the flower[await]
     and the star gate will open.[await]'''
dialog_data[97] = '''             To Marrymore[await]'''
dialog_data[98] = '''              To Star Hill[await]'''
dialog_data[99] = '''

    Can't wait to start a family.[await]'''
dialog_data[100] = '''

         I hope my baby's cute![await]'''
dialog_data[101] = '''

      May Mallow fix the Road![await]'''
dialog_data[102] = '''
    Need a “Shiny Stone” to get in[await]'''
dialog_data[103] = ''' It's really weird.
 Sometimes I hear the guy next door.[await][page]
 He's always mumbling about a
 Crystal-this and an Evil-that.[await]'''
dialog_data[104] = '''JAGGER: Sensei, the new regimen
 will strengthen us, right?[await]'''
dialog_data[105] = '''JINX: Master!
 Share your wisdom with us![await]'''
dialog_data[106] = ''' I think our neighbor moved away.[await]'''
dialog_data[107] = '''                 Caution![await]
    If you fall out the back of the
   castle, you won't be able to get
       back in without a battle.[await]
  Make sure you have everything you
        need before proceeding![await]
   ~The SMRPG Randomizer Dev Team[await]'''
dialog_data[108] = ''' Have you ever visited the tower in
 the south? I snuck in one time.[await][page]
 Keep your eyes peeled in there for
 crevices in the walls. You can find
 some nifty stuff![await]'''
dialog_data[109] = ''' Some of the monsters in this world
 that grant you star pieces can be
 fought multiple times.[await][pause] But you'll
 only get one star piece, tops.[await][page]
 There's some exceptions, though.[await]
 If the monster has a slightly
 different name... it's a different
 monster![await]'''
dialog_data[110] = ''' Garro's got his work cut out for
 him.[await][pause] He's always having to make
 new statues of whichever
 delinquent has invaded the castle.[await]'''
dialog_data[111] = ''' I heard a rumour that there's some
 treasures that King and Queen
 Nimbus hid away.[await]
 Maybe you'll find those if you
 rescue them.[await]'''
dialog_data[112] = ''' The invaders left, but we decided to
 keep the giant egg.[await]'''
dialog_data[113] = ''' A couple of the chests in the
 castle have been refilled. Have you
 found them all?[await]'''
dialog_data[114] = ''' There might be some new things you
 can find around Nimbus Land now
 that our town has been liberated.[await]
 Why not have a look around?[await]'''
dialog_data[115] = ''' Our castle doesn't have much
 security, so it gets invaded pretty
 often.[await]
 I guess it keeps things interesting
 around here, right?[await]'''
dialog_data[116] = ''' Have you been to the volcano yet?
 I'll tell you a secret.[await][page]
 Before the first stairway, there's
 a room on the other side of the
 lava that you can jump to.[await][pause] It's a
 little hard to see. But maybe you'll
 find something interesting in there.[await]'''
dialog_data[117] = ''' If you find an invincibility star in
 the volcano, try to let the timer
 run out while standing in lava.[await][page]
 If one of you levels up, something
 funny may happen when you regain
 control.[await][pause] Keep holding Y and your
 D-pad.[await]'''
dialog_data[118] = ''' You need “Castle Key 1“ to open
 the middle room...[await]'''
dialog_data[119] = ''' There's three invaders in the
 castle.[await][pause] You'll need a key to fight
 the second one, and another key to
 fight the third.[await][pause] The keys could be
 anywhere in the world![await][page]
 How'd we lose them, you ask?[delay_30]
 Er... [delay_30]don't ask me, [delay]I have no idea![await]'''
dialog_data[120] = ''' The statues in the castle were
 chosen by the third invader.[await]'''
dialog_data[121] = ''' Sometimes, when no one's looking,
 I take a dive out the back door
 of the castle. It's so much fun![await]
 I saw some treasure chests on the
 way down.[await][pause] But you can't get to
 them without going through the
 west beanstalks.[await]'''
dialog_data[122] = ''' The guy in the left corner will give
 you something, too.[await]'''
dialog_data[123] = '''                DANGER!!!

   Beyond the cliffs is the volcano[await]'''
dialog_data[124] = ''' Please, enjoy yourselves.[await]
 The volcano is just past the
 hot springs.[await]'''
dialog_data[125] = '''KING NIMBUS: Feel free to rest in
 our Hot Springs any time you like![await]'''
dialog_data[126] = '''QUEEN NIMBUS: If you haven't been
 to the Volcano yet, you can get
 there via our Hot Springs. [await]'''
dialog_data[127] = ''' The hot springs are ahead.[await]
 But while the castle is occupied by
 invaders, we're under strict
 orders to forbid entry.[await]
 Sorry about that...[await]'''
dialog_data[128] = ''' You won't have to wait for the
 Royal Bus to be repaired if you
 want to go to Bowser's Keep.[await]
 There's other ways to get in![await]'''
dialog_data[129] = ''' It's the bus driver's day off today,
 so you can't take the Royal Bus.[await]
 If you need to go to Bowser's
 Keep, you can get there
 `BOWSERS_KEEP_CONDITION`[await]'''
dialog_data[130] = ''' I heard there was a big factory
 behind it. Is that true?[await]'''
dialog_data[131] = ''' Welcome! Come on in!
 So you want to take a rest?
 I'm sure that's why you're here.[await][page]
 It'll be [0x7024] coins a night.[await]
 [select]  (OK, I'll take a break)
 [select]  (I'll pass)[await]'''
dialog_data[132] = '''[page]
 Let me guess...you want to use
 this “Dream Cushion” to rest,
 right? That'll be 30 more coins.[await]
 [select]  (I'll try anything once)
 [select]  (Looks pretty weird)[await]'''
dialog_data[133] = '''[page]
 Seems like you don't have
 enough money. Come back again.[await]'''
dialog_data[134] = '''
              Oh, shut up.[await]'''
dialog_data[135] = '''
         Don't talk to me now.[await]'''
dialog_data[136] = ''' You want this?
 It's only 500 coins.[await]
 [select]  (Well, sure!)
 [select]  (No)[await]'''
dialog_data[137] = '''[page]

         You need more coinage.[await]'''
dialog_data[138] = ''' Want me to teach you?[await]
 [select]  (Yes, please)
 [select]  (I already know)[await]'''
dialog_data[139] = '''[page]
 High score if you hit
 the shells with the stars
 fired from the beetle.[await]
 If you get hit, recover by firing
 consecutively...Just do it![await]'''
dialog_data[140] = ''' Have you found any rare Frog
 Coins?[await][pause] They're different from
 normal Frog Coins, and they stay
 in your Special Items box.[await][page]
 If you find one, you can trade it
 with the guy upstairs for a special
 item.[await]'''
dialog_data[141] = ''' The world around us is always
 changing.[await][pause] It is wise to visit places
 more than once, especially if
 something major has happened.[await][page]
 “One man's trash is another man's
 treasure.”[await][pause] If you find items in your
 journey that seem useless, someone
 may be seeking them in the future.[await]'''
dialog_data[142] = '''GREAPER:
       Nice, you found my item![await]'''
dialog_data[143] = '''DRY BONES:
       Cool, you found my item.[await]'''
dialog_data[144] = '''THE BIG BOO:
      'Grats, you found my item![await]'''
dialog_data[145] = ''' Looks like you found our missing
 keys! Nice work![await]'''
dialog_data[146] = ''' Do you mind staying around a bit
 longer? This castle gets invaded
 so much, it's practically a joke.[await]'''
dialog_data[147] = ''' The giant egg in the next room
 stayed here. What a weird day.[await]'''
dialog_data[148] = ''' Morning!
 Did you have pleasant dreams?[await][pause][end]'''
dialog_data[149] = '''
 That's odd[delay]...[delay][delay]
 It smells like pepper in here!
 Probably just my imagination![await]'''
dialog_data[150] = '''
 It looked like you had nightmares.
 Is everything okay?[await]'''
dialog_data[151] = '''
 When you hear laughter,
 you know it's a good dream![await]'''
dialog_data[152] = '''
 You slept quietly, so I'm sure
 you had pleasant dreams![await]'''
dialog_data[153] = '''[page]
 Hey, wait a sec.
 You don't have enough money, so
 I can't let you use the cushion.[await]'''
dialog_data[154] = '''GARRO: I don't know why Dodo
 doesn't just get a new helmet that
 won't cover his eyes.[await]'''
dialog_data[155] = '''GAZ:
    Geno, come on, you can do it![await]'''
dialog_data[156] = '''YOSHI:
 (`MAIN_CHARACTER_NAME`! Good luck!)[await]'''
dialog_data[157] = '''CHANCELLOR:
 We're counting on you, `MAIN_CHARACTER_NAME`![await]'''
dialog_data[158] = ''' Boing, boing, boing! I'm trying
 really hard, so `MAIN_CHARACTER_NAME`,
 you have to try hard, too![await]'''
dialog_data[159] = '''
       I'm with you all the way![await]'''
dialog_data[160] = '''[page]

       “How to climb the vines”[await][page]
 The vines are positioned from
 the bottom left to the top right.[await][page]
 When you jump, you will be able to
 disengage yourself from the vine.[await]
 Nimbus Dept. of Transportation[await]'''
dialog_data[161] = '''                   Huh?[delay_10]
   There is a note in the pocket...[await][page]
                 It reads:[await]
           [delay]“GARY WAS HERE!
            [delay]ASH IS A LOSER!”[await]'''
dialog_data[162] = ''' If you see Samus, tell her I need her
 to find my cane.[await]'''
dialog_data[163] = '''[page]
 Here is the menu.[await]
 [select]  (Pick Me Up........10 Coins)
 [select]  (Kerokero Cola....150 Coins)
 [select]  (No thanks)[await]'''
dialog_data[164] = '''[page]
 `MAIN_CHARACTER_HONORIFIC_CAP`, you seem to be a few coins
 short. Please ring again.[await][pause][end]'''
dialog_data[165] = ''' Our guest is resting.
 Please be quiet.[await]'''
dialog_data[166] = ''' If you see Link, tell him I need him
 to find my Gravity Suit.[await]'''
dialog_data[167] = '''      Bought a “[0x70A7]”![await]'''
