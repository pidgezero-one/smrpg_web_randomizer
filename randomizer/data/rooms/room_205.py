# R205_MUSHROOM_WAY_AREA_03
# pyright: reportWildcardImportFromLibrary=false
from smrpgpatchbuilder.datatypes.levels.classes import (EventInitiator, PostBattleBehaviour, EdgeDirection, BufferType, BufferSpace, VramStore, Buffer, Partition, RoomExit, MapExit, Event, BattlePackNPC, RegularNPC, BattlePackClone)
from ...types.room import Room
from smrpgpatchbuilder.datatypes.overworld_scripts.arguments.directions import *
from . import npcs
from ..variables.room_names import *
from ..variables.overworld_area_names import *
from ..variables.music_names import *
from ..variables.event_script_names import *
from ..variables.action_script_names import *
room = Room(
    partition=Partition(
        ally_sprite_buffer_size=1,
        allow_extra_sprite_buffer=False,
        extra_sprite_buffer_size=0,
        buffers = [
            Buffer(
                buffer_type=BufferType.EMPTY_3,
                main_buffer_space=BufferSpace.BYTES_0,
                index_in_main_buffer=True
            ),
            Buffer(
                buffer_type=BufferType.THREE_SPRITES_PER_ROW,
                main_buffer_space=BufferSpace.BYTES_512,
                index_in_main_buffer=False
            ),
            Buffer(
                buffer_type=BufferType.EMPTY_3,
                main_buffer_space=BufferSpace.BYTES_0,
                index_in_main_buffer=True
            )
        ],
        full_palette_buffer=True
    ),
    music=M0013_ROADISFULLOFDANGERS,
    entrance_event=E2814_MUSHROOM_WAY_3_LOADER,
    events=[
        Event(
            event=E2812_MUSHROOM_WAY_3_UPPER_QUICK_SPINY,
            x=17,
            y=85,
            z=0,
            f=EdgeDirection.SOUTHEAST,
            height=7,
            length=2,
            nw_se_edge_active=True,
            ne_sw_edge_active=False,
            byte_8_bit_4=False),
        Event(
            event=E2812_MUSHROOM_WAY_3_UPPER_QUICK_SPINY,
            x=18,
            y=89,
            z=0,
            f=EdgeDirection.SOUTHEAST,
            height=7,
            length=3,
            nw_se_edge_active=True,
            ne_sw_edge_active=False,
            byte_8_bit_4=False),
        Event(
            event=E2811_MUSHROOM_WAY_3_LOWER_QUICK_SPINY,
            x=15,
            y=100,
            z=0,
            f=EdgeDirection.SOUTHWEST,
            height=7,
            length=5,
            nw_se_edge_active=True,
            ne_sw_edge_active=False,
            byte_8_bit_4=False),
        Event(
            event=E2811_MUSHROOM_WAY_3_LOWER_QUICK_SPINY,
            x=14,
            y=102,
            z=0,
            f=EdgeDirection.SOUTHWEST,
            height=7,
            length=5,
            nw_se_edge_active=True,
            ne_sw_edge_active=False,
            byte_8_bit_4=False),
        Event(
            event=E2807_MUSHROOM_WAY_3_EXIT_TO_WORLD_MAP,
            x=29,
            y=87,
            z=0,
            f=EdgeDirection.SOUTHWEST,
            height=7,
            length=2,
            nw_se_edge_active=True,
            ne_sw_edge_active=False,
            byte_8_bit_4=False),
        Event(
            event=E2808_MUSHROOM_WAY_BOSS_FIGHT,
            x=27,
            y=90,
            z=0,
            f=EdgeDirection.SOUTHWEST,
            height=7,
            length=5,
            nw_se_edge_active=True,
            ne_sw_edge_active=False,
            byte_8_bit_4=False),
        Event(
            event=E2808_MUSHROOM_WAY_BOSS_FIGHT,
            x=27,
            y=90,
            z=0,
            f=EdgeDirection.SOUTHEAST,
            height=7,
            length=8,
            nw_se_edge_active=True,
            ne_sw_edge_active=False,
            byte_8_bit_4=False),
        Event(
            event=E2808_MUSHROOM_WAY_BOSS_FIGHT,
            x=28,
            y=96,
            z=0,
            f=EdgeDirection.SOUTHEAST,
            height=7,
            length=5,
            nw_se_edge_active=True,
            ne_sw_edge_active=False,
            byte_8_bit_4=False),
    ],
    exits=[
        MapExit(
            x=29,
            y=87,
            z=0,
            f=EdgeDirection.SOUTHWEST,
            length=2,
            height=0,
            nw_se_edge_active=True,
            ne_sw_edge_active=False,
            byte_2_bit_2=True,
            destination=OW09_MUSHROOM_WAY,
            show_message=False,
            byte_2_bit_1=False,
            byte_2_bit_0=False),
        RoomExit(
            x=3,
            y=78,
            z=0,
            f=EdgeDirection.SOUTHEAST,
            length=2,
            height=0,
            nw_se_edge_active=True,
            ne_sw_edge_active=False,
            byte_2_bit_2=False,
            destination=R204_MUSHROOM_WAY_AREA_02,
            show_message=False,
            dst_x=20,
            dst_y=37,
            dst_z=0,
            dst_z_half=False,
            dst_f=NORTHWEST,
            x_bit_7=False),
    ],
    objects=[
        BattlePackNPC( # 0
            npc=npcs.SPIKEY_NPC_2,
            initiator=EventInitiator.ANYTHING_EXCEPT_PRESS_A,
            after_battle=PostBattleBehaviour.REMOVE_PERMANENTLY,
            battle_pack=2,
            action_script=A0400_SEQUENCE_LOOPING_ON,
            visible=False,
            x=11,
            y=55,
            z=0,
            z_half=False,
            direction=SOUTHWEST,
            face_on_trigger=False,
            cant_enter_doors=False,
            byte2_bit5=False,
            set_sequence_playback=True,
            cant_float=True,
            cant_walk_up_stairs=False,
            cant_walk_under=False,
            cant_pass_walls=True,
            cant_jump_through=False,
            cant_pass_npcs=True,
            byte3_bit5=False,
            cant_walk_through=True,
            byte3_bit7=False,
            slidable_along_walls=True,
            cant_move_if_in_air=True,
            byte7_upper2=3),
        BattlePackClone( # 1
            npc=npcs.SPIKEY_NPC_2,
            battle_pack=2,
            action_script=A0400_SEQUENCE_LOOPING_ON,
            visible=False,
            x=11,
            y=55,
            z=0,
            z_half=False,
            direction=SOUTHWEST),
        BattlePackClone( # 2
            npc=npcs.SPIKEY_NPC_2,
            battle_pack=2,
            action_script=A0400_SEQUENCE_LOOPING_ON,
            visible=False,
            x=11,
            y=55,
            z=0,
            z_half=False,
            direction=SOUTHWEST),
        BattlePackClone( # 3
            npc=npcs.SPIKEY_NPC_2,
            battle_pack=2,
            action_script=A0400_SEQUENCE_LOOPING_ON,
            visible=False,
            x=19,
            y=85,
            z=0,
            z_half=False,
            direction=SOUTHWEST),
        BattlePackClone( # 4
            npc=npcs.SPIKEY_NPC_2,
            battle_pack=2,
            action_script=A0400_SEQUENCE_LOOPING_ON,
            visible=False,
            x=17,
            y=107,
            z=0,
            z_half=False,
            direction=NORTHWEST),
        RegularNPC( # 5
            npc=npcs.TOAD_NPC,
            initiator=EventInitiator.NONE,
            event_script=E2304_BANK_1F_RETURN_EVENT_2,
            action_script=A0491_MUSHROOM_WAY_3_RECRUITABLE_CHARACTER,
            visible=False,
            x=29,
            y=92,
            z=0,
            z_half=False,
            direction=SOUTHWEST,
            face_on_trigger=False,
            cant_enter_doors=False,
            byte2_bit5=False,
            set_sequence_playback=True,
            cant_float=False,
            cant_walk_up_stairs=False,
            cant_walk_under=False,
            cant_pass_walls=False,
            cant_jump_through=False,
            cant_pass_npcs=False,
            byte3_bit5=False,
            cant_walk_through=False,
            byte3_bit7=False,
            slidable_along_walls=True,
            cant_move_if_in_air=True,
            byte7_upper2=3,
            cannot_clone=True,
            directions=VramStore.DIR2_SWSE),
        RegularNPC( # 6
            npc=npcs.LAKITU_NPC_2,
            initiator=EventInitiator.NONE,
            event_script=E2304_BANK_1F_RETURN_EVENT_2,
            action_script=A0557_MUSHROOM_WAY_LAKITU,
            visible=True,
            x=13,
            y=107,
            z=5,
            z_half=False,
            direction=SOUTHWEST,
            face_on_trigger=False,
            cant_enter_doors=False,
            byte2_bit5=False,
            set_sequence_playback=True,
            cant_float=False,
            cant_walk_up_stairs=False,
            cant_walk_under=False,
            cant_pass_walls=False,
            cant_jump_through=False,
            cant_pass_npcs=False,
            byte3_bit5=False,
            cant_walk_through=False,
            byte3_bit7=False,
            slidable_along_walls=True,
            cant_move_if_in_air=True,
            byte7_upper2=3,
            cannot_clone=True),
        RegularNPC( # 7
            npc=npcs.HAMMER_BRO_SMALL_NPC,
            initiator=EventInitiator.PRESS_A_FROM_ANY_SIDE,
            event_script=E2304_BANK_1F_RETURN_EVENT_2,
            action_script=A0492_MUSHROOM_WAY_BOSS,
            visible=True,
            x=28,
            y=91,
            z=0,
            z_half=False,
            direction=SOUTHWEST,
            face_on_trigger=True,
            cant_enter_doors=True,
            byte2_bit5=False,
            set_sequence_playback=True,
            cant_float=True,
            cant_walk_up_stairs=False,
            cant_walk_under=False,
            cant_pass_walls=True,
            cant_jump_through=False,
            cant_pass_npcs=True,
            byte3_bit5=False,
            cant_walk_through=True,
            byte3_bit7=True,
            slidable_along_walls=True,
            cant_move_if_in_air=True,
            byte7_upper2=3, cannot_clone=True, vram_size=1),
    ]
)
