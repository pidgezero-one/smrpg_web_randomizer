# R470_FACTORY_GROUNDS_AREA_04_GUN_YOLKS_ROOM
# pyright: reportWildcardImportFromLibrary=false
from smrpgpatchbuilder.datatypes.levels.classes import (EventInitiator, EdgeDirection, BufferType, BufferSpace, VramStore, Buffer, Partition, RoomExit, Event, RegularNPC, RegularClone, EffectsNpc)
from ...types.room import Room
from ...types.ally import SpriteAnimationState
from smrpgpatchbuilder.datatypes.overworld_scripts.arguments.directions import *
from . import npcs
from ..variables.room_names import *
from ..variables.overworld_area_names import *
from ..variables.music_names import *
from ..variables.event_script_names import *
from ..variables.action_script_names import *
room = Room(
    partition=Partition(
        ally_sprite_buffer_size=1,
        allow_extra_sprite_buffer=False,
        extra_sprite_buffer_size=0,
        buffers = [
            Buffer(
                buffer_type=BufferType.EMPTY_3,
                main_buffer_space=BufferSpace.BYTES_0,
                index_in_main_buffer=True
            ),
            Buffer(
                buffer_type=BufferType.THREE_SPRITES_PER_ROW,
                main_buffer_space=BufferSpace.BYTES_0,
                index_in_main_buffer=True
            ),
            Buffer(
                buffer_type=BufferType.FOUR_SPRITES_PER_ROW,
                main_buffer_space=BufferSpace.BYTES_0,
                index_in_main_buffer=True
            )
        ],
        full_palette_buffer=True
    ),
    music=M0056_FACTORY,
    entrance_event=E2601_FACTORY_4TH_ROOM_LOADER,
    events=[
        Event(
            event=E2603_FACTORY_4TH_BOSS_FIGHT,
            x=10,
            y=90,
            z=0,
            f=EdgeDirection.SOUTHEAST,
            height=7,
            length=2,
            nw_se_edge_active=True,
            ne_sw_edge_active=False,
            byte_8_bit_4=False),
    ],
    exits=[
        RoomExit(
            x=14,
            y=99,
            z=0,
            f=EdgeDirection.SOUTHEAST,
            length=3,
            height=7,
            nw_se_edge_active=True,
            ne_sw_edge_active=False,
            byte_2_bit_2=False,
            destination=R472_FACTORY_GROUNDS_AREA_03,
            show_message=False,
            dst_x=4,
            dst_y=96,
            dst_z=5,
            dst_z_half=False,
            dst_f=SOUTHEAST,
            x_bit_7=False),
    ],
    objects=[
        RegularNPC( # 0
            npc=npcs.GUNYOLK_TOP_SECTION_NPC,
            initiator=EventInitiator.NONE,
            event_script=E2304_BANK_1F_RETURN_EVENT_2,
            action_script=A0015_DO_NOTHING,
            speed=4,
            visible=True,
            x=7,
            y=81,
            z=0,
            z_half=False,
            direction=SOUTHWEST,
            face_on_trigger=False,
            cant_enter_doors=False,
            byte2_bit5=False,
            set_sequence_playback=True,
            cant_float=False,
            cant_walk_up_stairs=False,
            cant_walk_under=False,
            cant_pass_walls=False,
            cant_jump_through=False,
            cant_pass_npcs=False,
            byte3_bit5=False,
            cant_walk_through=False,
            byte3_bit7=False,
            slidable_along_walls=True,
            cant_move_if_in_air=True,
            byte7_upper2=3,
            directions=VramStore.DIR2_SWSE),
        RegularClone( # 1
            npc=npcs.GUNYOLK_TOP_SECTION_NPC,
            event_script=E2304_BANK_1F_RETURN_EVENT_2,
            action_script=A0015_DO_NOTHING,
            visible=True,
            x=6,
            y=81,
            z=0,
            z_half=False,
            direction=SOUTHWEST,
            directions=VramStore.DIR2_SWSE),
        RegularClone( # 2
            npc=npcs.GUNYOLK_TOP_SECTION_NPC,
            event_script=E2304_BANK_1F_RETURN_EVENT_2,
            action_script=A0015_DO_NOTHING,
            visible=True,
            x=6,
            y=85,
            z=0,
            z_half=False,
            direction=SOUTHWEST,
            directions=VramStore.DIR2_SWSE),
        RegularClone( # 3
            npc=npcs.GUNYOLK_TOP_SECTION_NPC,
            event_script=E2304_BANK_1F_RETURN_EVENT_2,
            action_script=A0015_DO_NOTHING,
            visible=True,
            x=7,
            y=85,
            z=0,
            z_half=False,
            direction=SOUTHWEST,
            directions=VramStore.DIR2_SWSE),
        RegularClone( # 4
            npc=npcs.GUNYOLK_OUTER_SECTION_NPC,
            event_script=E2304_BANK_1F_RETURN_EVENT_2,
            action_script=A0015_DO_NOTHING,
            visible=True,
            x=7,
            y=83,
            z=0,
            z_half=False,
            direction=SOUTHWEST,
            directions=VramStore.DIR2_SWSE),
        RegularClone( # 5
            npc=npcs.GUNYOLK_TOP_SECTION_NPC,
            event_script=E2304_BANK_1F_RETURN_EVENT_2,
            action_script=A0015_DO_NOTHING,
            visible=True,
            x=7,
            y=86,
            z=0,
            z_half=False,
            direction=SOUTHWEST,
            directions=VramStore.DIR2_SWSE),
        RegularClone( # 6
            npc=npcs.GUNYOLK_TOP_SECTION_NPC,
            event_script=E2304_BANK_1F_RETURN_EVENT_2,
            action_script=A0015_DO_NOTHING,
            visible=True,
            x=8,
            y=85,
            z=0,
            z_half=False,
            direction=SOUTHWEST,
            directions=VramStore.DIR2_SWSE),
        RegularNPC( # 7
            npc=npcs.MACHINE_MADE_DRILL_BIT_NPC,
            initiator=EventInitiator.NONE,
            event_script=E2304_BANK_1F_RETURN_EVENT_2,
            action_script=A0955_FACTORY_2ND_ROOM_CONVEYOR_ENEMIES,
            visible=True,
            x=5,
            y=79,
            z=0,
            z_half=False,
            direction=SOUTHEAST,
            face_on_trigger=False,
            cant_enter_doors=False,
            byte2_bit5=False,
            set_sequence_playback=False,
            cant_float=False,
            cant_walk_up_stairs=False,
            cant_walk_under=False,
            cant_pass_walls=False,
            cant_jump_through=False,
            cant_pass_npcs=False,
            byte3_bit5=False,
            cant_walk_through=False,
            byte3_bit7=False,
            slidable_along_walls=True,
            cant_move_if_in_air=True,
            byte7_upper2=3),
        RegularClone( # 8
            npc=npcs.MACHINE_MADE_DRILL_BIT_NPC,
            event_script=E2304_BANK_1F_RETURN_EVENT_2,
            action_script=A0954_FACTORY_2ND_ROOM_CONVEYOR_ENEMIES,
            visible=True,
            x=9,
            y=78,
            z=0,
            z_half=False,
            direction=SOUTHEAST,),
        RegularClone( # 9
            npc=npcs.MACHINE_MADE_DRILL_BIT_NPC,
            event_script=E2304_BANK_1F_RETURN_EVENT_2,
            action_script=A0954_FACTORY_2ND_ROOM_CONVEYOR_ENEMIES,
            visible=True,
            x=12,
            y=84,
            z=0,
            z_half=False,
            direction=SOUTHEAST,),
        RegularClone( # 10
            npc=npcs.MACHINE_MADE_DRILL_BIT_NPC,
            event_script=E2304_BANK_1F_RETURN_EVENT_2,
            action_script=A0954_FACTORY_2ND_ROOM_CONVEYOR_ENEMIES,
            visible=True,
            x=15,
            y=90,
            z=0,
            z_half=False,
            direction=SOUTHEAST,),
        RegularClone( # 11
            npc=npcs.MACHINE_MADE_DRILL_BIT_NPC,
            event_script=E2304_BANK_1F_RETURN_EVENT_2,
            action_script=A0956_FACTORY_2ND_ROOM_CONVEYOR_ENEMIES,
            visible=True,
            x=8,
            y=31,
            z=0,
            z_half=False,
            direction=SOUTHEAST,),
        RegularNPC( # 12
            npc=npcs.FACTORY_CHIEF_NPC,
            initiator=EventInitiator.NONE,
            event_script=E2304_BANK_1F_RETURN_EVENT_2,
            action_script=A0400_SEQUENCE_LOOPING_ON,
            speed=5,
            visible=True,
            x=8,
            y=83,
            z=0,
            z_half=False,
            direction=SOUTHEAST,
            face_on_trigger=False,
            cant_enter_doors=False,
            byte2_bit5=False,
            set_sequence_playback=True,
            cant_float=False,
            cant_walk_up_stairs=False,
            cant_walk_under=False,
            cant_pass_walls=False,
            cant_jump_through=False,
            cant_pass_npcs=False,
            byte3_bit5=False,
            cant_walk_through=False,
            byte3_bit7=False,
            slidable_along_walls=True,
            cant_move_if_in_air=True,
            byte7_upper2=3),
        RegularNPC( # 13
            npc=npcs.FACTORY_CRANE_NPC,
            initiator=EventInitiator.NONE,
            event_script=E2304_BANK_1F_RETURN_EVENT_2,
            action_script=A0958_CRANE_FOR_FINAL_FACTORY_BOSS_DEFAULT,
            speed=5,
            visible=True,
            x=1,
            y=72,
            z=6,
            z_half=False,
            direction=SOUTHWEST,
            face_on_trigger=False,
            cant_enter_doors=False,
            byte2_bit5=False,
            set_sequence_playback=True,
            cant_float=False,
            cant_walk_up_stairs=False,
            cant_walk_under=False,
            cant_pass_walls=False,
            cant_jump_through=False,
            cant_pass_npcs=False,
            byte3_bit5=False,
            cant_walk_through=False,
            byte3_bit7=False,
            slidable_along_walls=True,
            cant_move_if_in_air=True,
            byte7_upper2=3),
        RegularNPC( # 14
            npc=npcs.GREEN_SWITCH_NPC,
            initiator=EventInitiator.JUMP_ON,
            event_script=E2616_FACTORY_4TH_ROOM_GREEN_BUTTON,
            action_script=A0015_DO_NOTHING,
            visible=False,
            x=7,
            y=82,
            z=0,
            z_half=False,
            direction=SOUTHWEST,
            face_on_trigger=False,
            cant_enter_doors=False,
            byte2_bit5=False,
            set_sequence_playback=False,
            cant_float=False,
            cant_walk_up_stairs=False,
            cant_walk_under=False,
            cant_pass_walls=False,
            cant_jump_through=False,
            cant_pass_npcs=False,
            byte3_bit5=False,
            cant_walk_through=True,
            byte3_bit7=False,
            slidable_along_walls=True,
            cant_move_if_in_air=False,
            byte7_upper2=3),
        RegularNPC( # 15
            npc=npcs.MACHINE_MADE_DRILL_BIT_NPC,
            initiator=EventInitiator.NONE,
            event_script=E2304_BANK_1F_RETURN_EVENT_2,
            action_script=A0959_FINAL_FACTORY_ROOM_MASS_PRODUCED_NPC_PICKED_UP,
            visible=True,
            x=1,
            y=77,
            z=0,
            z_half=False,
            direction=SOUTHEAST,
            face_on_trigger=False,
            cant_enter_doors=False,
            byte2_bit5=False,
            set_sequence_playback=False,
            cant_float=False,
            cant_walk_up_stairs=False,
            cant_walk_under=False,
            cant_pass_walls=False,
            cant_jump_through=False,
            cant_pass_npcs=False,
            byte3_bit5=False,
            cant_walk_through=False,
            byte3_bit7=False,
            slidable_along_walls=True,
            cant_move_if_in_air=True,
            byte7_upper2=3),
    ],
    extra_sprite_actions=[
        SpriteAnimationState.LEAN_BACK,
        SpriteAnimationState.LEAN_BACK_2,
        SpriteAnimationState.LEAN_BACK,
        SpriteAnimationState.CLIMB_FRAME,
    ],
    effects_npc=EffectsNpc.UNKNOWN_1D,
)
